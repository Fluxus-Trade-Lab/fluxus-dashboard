...
import {
  STAGES, STAGE_NAMES, TRIANGLE, TAIJI, FUNNEL_LINKS, GRID_LINES,
...
/** half disc, flat side vertical; side = -1 left, +1 right */
const half = (ox, cx, cy, r, side, fill) => {
  const x = ox + cx * S, y = cy * S + PAD, rr = r * S
  const sweep = side > 0 ? 1 : 0
  return `<path d="M ${x.toFixed(1)} ${(y - rr).toFixed(1)} A ${rr} ${rr} 0 0 ${sweep} ${x.toFixed(1)} ${(y + rr).toFixed(1)} Z" fill="${fill}"/>`
}

const panels = STAGES.map((st, i) => {
  const ox = PAD + i * (S + GAP)
  const P = (x, y) => `${(ox + x * S).toFixed(1)} ${(y * S + PAD).toFixed(1)}`
  const parts = [`<rect x="${ox}" y="${PAD}" width="${S}" height="${S}" fill="#141210"/>`]
  // connective tissue goes under the circles, faint, as on the poster
  if (i === 1) {
    parts.push(...FUNNEL_LINKS.map(([x0, y0, x1, y1]) =>
      `<path d="M ${P(x0, y0)} Q ${P(x0, y1)} ${P(x1, y1)}" fill="none" stroke="#8c847a" stroke-opacity="0.55" stroke-width="1"/>`))
  }
  if (i === 3) {
    parts.push(...GRID_LINES.map(([x0, y0, x1, y1]) =>
      `<line x1="${(ox + x0 * S).toFixed(1)}" y1="${(y0 * S + PAD).toFixed(1)}" x2="${(ox + x1 * S).toFixed(1)}" y2="${(y1 * S + PAD).toFixed(1)}" stroke="#8c847a" stroke-opacity="0.55" stroke-width="1"/>`))
  }
  parts.push(
  ...
  if (i === 4) {
    const { cx, cy, r } = TAIJI
    parts.push(
      half(ox, cx, cy, r, -1, BLUE),
      half(ox, cx, cy, r, +1, RED),
      circle(ox, [cx, cy - r / 2, r / 2], RED),
      circle(ox, [cx, cy + r / 2, r / 2], BLUE),
      circle(ox, [cx, cy - r / 2, r / 6], BLUE),
      circle(ox, [cx, cy + r / 2, r / 6], RED),
    )
  }
  parts.push(
    `<text x="${ox}" y="${PAD + S + 20}" fill="#78716c" font-family="ui-monospace,Menlo,monospace" font-size="11" letter-spacing="2">${i + 1}. ${STAGE_NAMES[i].toUpperCase()}</text>`,
  )
  return parts.join('\n')
})
