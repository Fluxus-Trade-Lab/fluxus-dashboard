        ...
        <table className="w-full text-left">
          <thead>
            <tr>
              ...
              {COLUMNS.map((col) => (
                <th
                  key={col.key}
                  className="text-[11px] font-medium uppercase tracking-wide text-[var(--color-text-muted)] pb-1.5 pr-2 text-right"
                >
                  {col.label}
                </th>
              ))}
          ...
          <tbody>
            {TICKERS.map((ticker) => {
              ...
              return (
                <tr key={ticker}>
                  <td className="font-mono text-[13px] text-[var(--color-text-bold)] py-0.5 pr-2">
                    {ticker}
                  </td>
                  {COLUMNS.map((col) => {
                    ...
                    return (
                      <td
                        key={col.key}
                        className={`font-mono text-[13px] py-0.5 pr-2 text-right ${pctColor(val)}`}
                      >
                        {formatDist(val)}
                      </td>
                    )
