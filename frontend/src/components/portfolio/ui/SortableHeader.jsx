export default function SortableHeader({ label, sortKey, sortConfig, onSort, className = '' }) {
  ...
  return (
    <th
      ...
      className={`text-left px-2.5 py-2 border-b-2 border-[var(--color-border)] text-[var(--color-text-secondary)] font-semibold text-[10px] uppercase tracking-wide whitespace-nowrap ${sortable ? 'cursor-pointer select-none hover:text-[var(--color-text)]' : ''} ${className}`}
    >
