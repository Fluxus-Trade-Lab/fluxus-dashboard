...
const armMismatched = {
  ...
  entryDate: '2026-09-17', entryPrice: 180, originalQty: 300, currentQty: 100,
...
describe('OverviewTab qty/trim-log mismatch warning', () => {
  it('shows the mismatched ticker, entry date and gap%, with no share count', () => {
    ...
    // Ticker + entry date + gap% name the row without a screenshot.
    const banner = screen.getByTestId('qty-mismatch-banner')
    expect(banner.textContent).toMatch(/ARM/)
    expect(banner.textContent).toMatch(/2026-09-17/)
    ...
    // Inline per-row badge on the mismatched trade's Qty cell.
    expect(screen.getByTitle(/ARM 2026-09-17.*-13\.3%/)).toBeInTheDocument()
  })
