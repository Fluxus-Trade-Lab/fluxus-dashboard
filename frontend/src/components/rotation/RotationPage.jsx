...
export default function RotationPage() {
  ...
  const kindOf = useMemo(() => { const m = new Map(themes.map((t) => [t.group, t.kind])); return (n) => m.get(n) ?? null }, [themes])
  ...
  const flux = shown.filter((o) => o.rel?.length).map((o) => o.name)
  const note = [
    `two-week strength vs ${benchmark}, daily · y fixed at ±${Math.round(Y_MAX * 100)}%, the overflow clips, hover reads the true value`,
    seriesDates.length ? `${seriesDates[R2W_LAG] ?? seriesDates[0]} → ${seriesDates[seriesDates.length - 1]}` : null,
    boards.approx ? 'RS 0–2w ≈ this week + one week of the prior three until the ladder ships its series' : null,
    flux.length < shown.length ? `${shown.filter((o) => !o.rel?.length).map((o) => o.name).join(', ')}: no series yet` : null,
  ].filter(Boolean).join(' · ')
  ...
  return (
    <div className="rot space-y-5">
      ...
      <TerrainCard ladder={ladder.data} kindOf={kindOf} selected={names} onSelect={toggle} />
      <PointsCard boards={boards} selected={names} onSelect={toggle} />
      <FluxCard shown={shown} dates={seriesDates} stateDates={stateDates} benchmark={benchmark} picked={!!selected.length} onSelect={toggle} note={note} />
      ...
      <HowToRead>
        ...
        <p><b>Flux.</b> The two-week strength every session for up to three themes — the top of each board until you pick. Under each line, that theme's two-week state per session. The benchmark is the zero line.</p>
