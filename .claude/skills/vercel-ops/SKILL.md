...
## 09-20 更新：口径换了，旧台阶推论作废

09-13 那套「两个台阶」是在**保留期 2 周、产物 105 MB**的前提下推的。09-20 两件事同时发生，
把前提全换了，下面那节「当前时间线（09-13 判断）」**整节作废**，留着只为查history：

- **真实撞线，不是控制台读数异常**：Andy 收到 Vercel 邮件告警，Andy 的 Chrome 登录态下实测
  Deployment Storage **114.79 GB / 10 GB**（超 104.79 GB）。09-14 那行「0 B」现在确认是控制台
  显示故障，不是真归零——09-12 126.07 → 09-13 125.41 → 09-20 114.79，7 天降 10.6 GB（≈1.5
  GB/天），说明保留期到期的台阶其实一直在走，只是没到「大截」的量级。
  **教训**：控制台单次异常读数（哪怕连续复现 3 次）不能当作「问题已解决」，`storage_gb=0` 那行
  当时就该标红追查而不是「按页面原样记，下周复读」——下一次遇到同类异常读数，处理方式改成
...
- **官方文档 09-16 版新事实**（09-20 核）：Hobby 四项默认保留期均 30 天；Hobby 的保留例外只保
  ...
  View Source / Restore，REST API 只有 DELETE（删完仍进 30 天恢复期），没有 purge 入口，闸 2
  「删了不腾空间」这条结论不变，只是例外条款的具体数字要更新。
- **产物侧修法落地（T-0920-55）**：`vercel.json` 不再 `cp -r data/output` 进构建产物，改成
...
- **未核实项**：`CANCELED` 保留期只有 1 天却还活着一批的异常（09-13 记 35 个，09-13T14:36 那行
  ...
  且要在 1 天保留期落地之后重查（旧异常可能已经被新保留期自然冲掉）。
...
- **[09-20] `weekly.csv` 一行缺收尾引号，整份文件解析炸掉。** commit `57c147d6` 追加的那行 notes
  字段少了最后一个 `"`，Python `csv` 模块会把这行没闭合的引号一路吃到下一行，导致**全文件**从那
  行往后都错位（27 个字段 vs 该有的 15 个）——append-only 公箱一个字符的格式错，代价是后面所有
  读者都读错，不只是那一行。已顺手补上收尾引号；追加新行前建议 `python3 -c "import csv; ...
  assert all(len(r)==15 for r in csv.reader(open(path)))"` 过一遍再提交。
