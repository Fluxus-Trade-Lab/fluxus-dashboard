...
**⭐ 示意图必须是这一课自己的**（Andy 09-24 问「这次的教学用的示意图对吗？」，一问就查出是错的）：
09-23 我给新概念 `leaders_vs_index_on_a_red_day` 复用了 `equal_weight_split`，理由写在 commit message 里
（「它画的正是市值加权撑住、等权破位——和今天这题是同一件事」）。**逐条核就塌了**：图上四个标签
（cap-weighted / equal-weight / 50-day / first close under the 50-day）**在正文里一个都没出现**；
正文的两个判据（回调缩量、龙头有没有跟着回）图里**一个都没画**；图是几周的趋势，课讲的是**一天**。
- **判据**：图上的每个标签，正文里要么出现过，要么是正文那句话的直接图示。**「同一件事」是判断，不是核对。**
- **闸**（`visual.pick_edu`）：`figure != concept` 且没写 `figure_reuse_reason` → 判红。复用可以，**不许无声复用**。
- 新图 `leaders_vs_index_on_a_red_day`：左「指数在跌」、右「龙头窄幅守住」+ 下方缩量柱，
  builder 的 assert 把这三条claim 编进代码（指数跌够、龙头带宽 <1.5、龙头低点高于指数终点、量缩过半且单调）。
