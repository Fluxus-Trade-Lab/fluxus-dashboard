...
### [2026-09-13] 掉落线保持原样（Andy 原话「太粗了！！方案取消，换成原来的。」）
- 同一晚先后试过三种改法，全部否决：①落点按行情编码（大小/环/刻度/收尾变粗四方案，「落点这些方案不行」）；②落点裂纹（「大改动也暂停」）；③加粗 + 空心圆落点 + 删副标题（渲染出来「太粗了」）
- **产线维持**：sheet-1 `dropSvg(dl, 2.5, "thin", …)` 细线、不加任何落点标记、标题下 `byline` 副标题保留；宽度由 `.drop.thin{height:46px}` 自然收住，不撑满版心
- 改掉落线的粗细、落点或副标题之前先读这条；Andy 没主动重开就不再提。版式归 Visual Vera
