...
2. **哪个测试改动前红、改动后绿？** 指出文件与测试名。指不出 → FAIL。
   ⚠️ **`type: data_contract` 且改动只删/改名单或映射条目（不新增行为、不改分支逻辑）时，本问没有 red→green 载体，改用三件替代证据**（T-0921-70/71，2026-09-21 ops 裁）：①验收命令实跑的完整输出 ②全量回归前后都通过（不掉绿即可，不要求本条改动让它翻） ③**改动面与被验对象的相关性说明**——执行者必须写清楚被跑的验收命令读的是不是这次改动碰的文件/路径，读的是别处（比如改了 `frontend/` 却验 `data/output/`）就不算数，视同 FAIL。三件缺一不可；缺哪件按 FAIL 处理，不要放宽成 ASK。
