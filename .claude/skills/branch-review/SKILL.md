...
4. **验收逐条对得上吗？** 每条写「对上/没对上 + 证据」。
   ⚠️ **验收项写的是「合并后才为真」的状态、且分支上已能核到全部实物（commit sha / diff / 文件内容都看得到）、唯一剩余动作就是 merge 本身时**（如「确认已在/已推送到 origin/main」），复核发生在推 main 之前，这条天然还不成立（T-0924-67，2026-09-24 ops 裁；起因 T-0924-45 的复核把这条判成 FAIL，其实是流程时序，不是真失败）：判词写「Q4 该条：待合并后自动满足」，按对上处理，不要拿它当 FAIL 理由。**这条豁免只免「merge 这一下」——执行方合并后还要再做别的动作（上线、cron 跑通、发布、通知）才能让该项为真的，不适用，仍要证据；判词没写「待合并后自动满足」就不算用了这条。** PASS 之后如果真合并了却没做到，算那条的真 FAIL——同一类日常任务下一轮复核时顺带核一次上一轮是否兑现，不必专门开单。
