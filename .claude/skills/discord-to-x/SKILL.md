...
5. 提交：只 `git add` 你写的那一个 `draft.txt`，commit 信息 `content: thread draft <folder>`。**口径更正（T-0922-03，原文写反了）：写 `draft.txt` 是 `data/output/` 下的非删除改动，按 `fluxus-ops/tools/gate.py` 的 `NONE_PREFIXES`（`data/` 开头且非删除）判 `none`，本任务 `type=discord_to_x` 也不在 `TYPES_REQUIRE_REVIEW` 里——正常就是 none，直接 `git -C "$WT" push -q origin HEAD:main`，被拒则 fetch + rebase 后重推（最多三轮）；只有**删除** `data/` 下文件才会被 `gate.py` 判成 `andy`（status=D 的兜底）。仍以 `taskboard.py gate <任务id> --worktree "$WT"` 的实际输出为准，别凭这条记忆抢跑。
