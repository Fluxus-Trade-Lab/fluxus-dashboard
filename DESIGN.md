...
### 追记 2026-08-15 之三 —— 材质与光（Andy：不做平面的）

平面油墨路线正式放弃，抄参考渲染的**材质和光**——但只抄这一对，身份不抄：
配色仍是海报红蓝，内容仍是五段方法论。

- **摄影棚**：`RoomEnvironment` 过 PMREM 当反射环境（无 HDR 文件要拉），
  一盏主光 + 球与球互相投影（PCFSoft shadow），首屏地面换成 `--color-poster-paper`
  (#ececec)——参考图的浅灰纸。折叠线下回到海报黑，**这一刀就是设计**。
- **材质**：`MeshPhysicalMaterial`，clearcoat 1 / roughness 0.16 /
  envMapIntensity 0.55；红球带 0.14 的自发光（参考图的红有内光）。
- **色相保卫战**：ACESFilmic 会按设计去饱和，海报红被洗成珊瑚色——
  换 `NeutralToneMapping`（Khronos PBR 中性映射），色相保住，高光照样滚落。
- **太极点彩化**：玻璃球世界里贴平面矢量太极是照片里贴贴纸。改成球自己排——
  两条互咬的鱼（第一版描边读成「圆环」，改成**填充质量块**才成立），
  蓝鱼=红鱼绕圆心转半圈，逐坐标对称。同一批球排完市场、漏斗、网格后合成太极，
  这本来就是那句话：一笔资本走五段。
- 绿三角 → 绿圆锥；渐隐改为随权重**缩放**（物理材质的透明要花钱，缩比隐好看）。
- 相机仍正交（参考 demo 的 fov17.5@z30 本来就是伪正交）；深度只给光用，不给读数用。
