...
比它命名目的地的墨水还多**。句子删掉，簇改用**间距**保留：间距说"这几个是一伙的"，
不替它们编一个名字，而编名字正是那些句子唯一多做的事。

**两级，两种 highlight，且刻意不同类。**

| 层级 | 是什么 | highlight 手段 |
|---|---|---|
| 一级 MARKET / LIBRARY / COURSE / MY BOOK | 一个**区域** | **对比度**：当前页落在它里面时，组名从 muted (#9d968c) 升到 bold (#fafaf9)。没别的 |
| 二级 页面 | 一个**行** | **几何**：左侧 2px 竖条 + 整行底色 + 加粗 |

区域 vs 行——两种手段各占一条通道，谁也不会被认成谁，而且**都不靠色相**
（延续 v2 的"排序永不由色相独扛"）。收起态没有组名可升，所以那里退回发丝分隔线；
首组之上不画线，上面本来就没有东西要跟它分开。

节奏：组间 20px，MARKET 内簇间 12px，行间 0。收起态 12px vs 8+线+8。
