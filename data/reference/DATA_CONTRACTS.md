...
- [08-23] ⚠️(ops 重挂,原行在 acc58714 的 clerk sweep 里被**整行删除**——规矩是处理完打 ✅ 写 commit,不合也要追结论,不是删行) **6 条封存分支的真修复仍无人认领,一条都没合进 main**(08-23 复核:staleness.py / test_ohlc_store.py / test_stockbee_ratio.py / test_no_offsession_rows.py / hookOrder.test.jsx 全部不在 main)。归数据端五条:`fix/ohlc-staleness-guard`(ohlc 保鲜+159 行测试)、`claude/keen-germain-696b72` 与 `claude/optimistic-clarke-4c56d0`(同一 bug 二选一:stockbee 同日双计+归档截断)、`claude/sharp-boyd-7ee3fc`(ticker 冻结守卫+CI step)、`claude/silly-borg-68395f`(非交易日零行防护)。归前端一条:`claude/wonderful-shannon-b66cdf` 的 hookOrder 回归测试。要不要合归各线判断,**不合也请在本行后追一句结论再打 ✅**。
- [08-23] (ops→数据端,Andy 拍板) **audit_ledger 接 CI**:夜间组 08-23 晨报提出 audit_ledger 未挂进 nightly workflow;改 `.github/workflows/daily-data-update.yml` 属数据端边界,转交 DATA ALEX,夜间组已被告知不动。细节见 `data/research/night_reports/2026-08-23.md`(分支 auto/night-20260823-4b6905)。
