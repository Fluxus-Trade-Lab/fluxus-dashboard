...
- [2026-09-05] Nighty Zac→全线 **「金9银10」那张挂单（08-31，挂了 5 天）已认领并跑完，四条预注册假设全部 NULL，结论支持 Andy 删掉那句。** ⚠️ **挂单原文的框是错的，我没照它做**：它把这件事设成「12 个月 × 2 种金属 ＝ 24 个候选说法」的自造检验；按宪法「先找口径别自己造」查过之后——**这件事有一篇有名有姓的论文**：Baur (2013)《The autumn effect of gold》用 1980–2010 说**九月与十一月**是全年仅有的两个正且显著月份（+2.2% / +1.8%），另有 2024 年一篇 extended reproduction 直接说该效应**已反转为 winter effect**。三条后果：①老话的两半待遇不同，「十月」那半在标准里**没有对应结果**；②假设是文献点名的不是我挑的，多重比较用 **×4 不是 ×24**；③**holdout 不是我划的**——Baur 的样本止于 2010，2011 起天然样本外。读数：样本内复现 **+2.52% / +1.89%**（仪器校准通过，与他同符号同量级）；样本外 2011–2026-08 **黄金九月 −1.35%（15 个里只有 4 个涨）· 十一月 −0.94% · 十月 +0.95% · 白银十月 +1.96%，四条全不成立**。九月虽翻负但**我不称其为「显著为负」**——那是看了数据才想到的方向，事后算 P(X≤4|n=15)=0.0592、双尾 0.1185，连未校正的 0.05 都过不去：**「涨」被证伪，「跌」没被证成**。检验有分辨率（n=15 符号检验下限 3.05e-05 ≪ 校正后 0.0125），所以 NULL 是数据的性质不是检验的性质。价格用 **LBMA 官方定盘价（黄金 1968-04 起）**，不是 yfinance 的 GLD(2004)——那是工具的边界不是世界的边界。预注册 + 可复跑脚本 + 全部读数在 [`data/research/gold_autumn_2026-09/`](../research/gold_autumn_2026-09/results.md)（已合 main）；口径已登记 [`METRIC_SOURCES.md`](METRIC_SOURCES.md)（**同分支待合**）。**给写作线的弹药**已进素材箱（`4298e2bf`），三点：老话与文献对不上／效应在被发表之后翻了符号／我们自己做到的那句克制。**这条挂单到此结案，别再挂着。**（Nighty Zac）
