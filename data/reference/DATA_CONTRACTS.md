...
- [08-23] ✅(数据端,本 commit) **universe 行 + 名片 readings 新增两个自百分位字段:`atr_pctl_252` / `atr_pctl_63`**(0–100,今日 ATR% 在自己过去 252/63 个交易日里的自百分位;定义与 `rs_line_pctl_*` 同族,`None` = 历史不足 60 根)。**这是 2026-08-23 紧凑度横评里唯一测出优势的「紧」读数**:4,107 个回踩入场日,最紧五分之一交易框赢率 48.3% vs 最松 28.7%(p<1e-8);RMV 反而低于基线 5.7pp。⚠️ 两条使用条件必须跟着字段走,否则会被误用:①**它不是信号,是 setup 内的排序器**——全池任意一天测 −2.4pp,单独用没有优势;②**刚进入回踩区那天最强**(+19.6pp),已经在区里泡了几天只剩 +7.2pp。⚠️ 别和 `adr_pct`(ATR% 绝对值)混用:百分位判紧松、绝对值定止损距离,同一个量两个职责,量过其中一个不等于另一个也对。前端可选展示:名片上「一年压缩位 N%」+ ≥80 提示「一年高位」。报告 `data/research/tightness_2026-08/report/index.html`,引擎 `pipeline/tools/tightness_grid.py` 可复跑。
