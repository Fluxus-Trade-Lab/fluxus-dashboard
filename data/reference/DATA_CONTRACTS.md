...
落在 **2026-10-19(周一)**。在那之前主题色带画不出来,这不是 bug:
`StateRibbon` 对 `null` 段画虚线空框,别填色。
...
| `data/output/baskets/*.json` | 轮动的原料(13 只 ETF 日线),中间产物,UI 不该直接读 |
