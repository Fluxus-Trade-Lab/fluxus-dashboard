...
- **[2026-09-26] alex（T-0926-29 晨检，09-26 复核后改写）：`quality.json` 的 `groups_stocks` 连续两个交易日（09-23、09-24）报 degraded，是 09-23 `full-content baskets` 上线（Andy 已采纳）的预期副作用，不是取数缺口——本条首版诊断（「不在 universe.json 里」「来自更宽的行业成分源」）经复核证伪，已改写为下面这版。** 根因就是 commit `7077f78de`（2026-09-23，Andy 原话「主题篮子放开市值闸，采纳」）：`groups.json.stocks` 的成员条件从「过 $1B/$2M 市值闸」放宽到「只排除壳公司」，成员数当场 2,524 → 5,280（commit message 自己写明），新纳入的 ~2,756 支全是此前被市值闸挡在外的小微盘。抽样核对 `data/output/universe.json`（`ticker` 字段，非 `symbol`）：本次缺 `excess_1m`/`rs_accel`/`persistence` 的 174 支（AEHL/AIIO/AIXI/AMCI/AMPG 等）**全部**在 universe.json 里能查到，且 `perf_1m` 本来就是 `None`（如 AIXI 市值 24 万美元、AEHL 1.7 亿美元，均远低于原 $1B 闸），`perf_1m` 整体缺失率在 `data/history/universe_quality.csv` 里 09-22→09-24 稳定在 5.2–5.4%，没有跳变——**不是新出现的缺口，是分母从 2524 扩到 5281 后，这批本来就缺数据的小微盘第一次被这道 groups_stocks 质量闸看见**。**不阻断**：`audit_archives` 0 violation、`schema_snapshot --check` exit 0，`quality.json` 顶层 `status: degraded` 只由这一项拖累，其余 5 个 source 全 ok；`pipeline/quality.py` 的 baseline 是滚动对比近期历史，等 09-23 后的新分母积累够 `MIN_HISTORY` 根数会自己转回 ok，不需要改代码，也不建议归进 `is_sparse_by_design`（那会连真正的死列一起消音）。（alex）
