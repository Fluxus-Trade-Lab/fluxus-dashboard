...
- ✅ **[2026-09-25] OPS → 全线：波段大师课 **ver1 封版**，中英双语 × PDF 与交互网页两形态都已上线；`data/content/RELEASE_ver1.md` 是「ver1 到底发了什么」的唯一权威源，以后要答这个问题读那一份，别在别处另立说法（T-0925-56；Andy 原话「让整个系统都知道这部分已经完成」）。**
  ...
  **Whop**：`Swing Trade Masterclass` 档 **$1,499 一次性**、**库存 50**、Discover 开着、商品页文案与标题 2026-09-25 已更新。
  ⚠️ **三个已作废的说法，谁也别再引用**（同一个数作废过两次，最容易在角落留残）：**「中文全本 390 页是交付版」作废**（390 页那份是 `_内部留档_全本不对外/`，显式不对外）；**「中 199 页 / 英 213 页」也作废**（那是 09-25 12:30 的 ver1 首版，当日 b2→b5 四轮重出后的定版是上面的 194 / 210）。作废原文留在 `fluxus-ops` 的 `tasks/T-0925-49.md` 与 `agents/q/runs/2026-09-25T031509Z-T-0925-49.md` 里当审计痕迹，**那些是历史记录不是现行事实**；`SwingMasterclass/_pdf/README.md` 第 624、769 行的「EN 213 页 · ZH 198 页」是 2026-08-12 旧书（L1–L16 结构）的构建日志，同样不是交付声明。
