...
- [08-20] ✅ **Library 缺一个目录** —— 已建,今晚 cron 起自己写(`pipeline/content/library_index.py`,接在 run_all 的 site quality 之前,自己一个失败域;8 条单测)。**两点和你的原文不同,请核**:(1) 值是 **`.json` 文件名**不是 `.md` —— `useLibrary.js` 是 `fetch` 完 `r.json()`,`.md` 没人渲染,以跑着的代码为准;(2) 分组读的是**文章自己的 `page` 字段**,不是文件名前缀 —— `portfolio-management` 带连字符,按下划线拆前缀分不出它和一个叫 `portfolio` 的页;`page` 缺失或不认识的文件**只报不猜**,不会被塞进任何一桶。五个页 key **永远都在**,空的是 `[]`(「量过,这里没有」和「没有这个 key」不是一回事)。原文:`<页面>_<主题>.md` 是个约定,不是清单——浏览器没法从约定里知道有哪些文件。现在前端读的是**编译进来的文件名单**(`useLibrary.js` 的 `COMPILED_IN`),意味着你们每加一篇,前端就得发一版才看得见,而且那一版之前页面会说"1 篇"而实际有两篇。要 `data/output/library/index.json`,形如 `{"offense": ["offense_ep_mrna.md"], "defense": [...], ...}`。前端已经先 fetch 它、取不到才回落名单,并在页面上标明当前读的是哪一种(目录 vs 编译名单)——所以你们哪天放上去,不用通知我,自己就切过去了。
