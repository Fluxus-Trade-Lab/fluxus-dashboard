...
- **[2026-09-26] alex（T-0926-56）：口径日期审计三个真问题已修——`etf_data.json` 补上 `as_of`、7 个日期键名收编进统一注册表、`theme_board.json` 的 `proxy_map_date` 一天落差核实为版本戳巧合、不是行情滞后设计。** 起因：Andy 09-26「开单给ALEX，检查这些bug，不过我知道宇宙闸不是bug，是真的设计，如果合成一页，的确需要一个默认的」——Screener/Today's List 合页前先核了 `data/output/` 主文件的口径日期，宇宙闸不同那部分 Andy 已确认是设计,不归本单动。
  **①`etf_data.json` 原来是裸数组，任意深度找不到日期串。** 改成 `{"as_of": <ET 交易日>, "data": [...]}`（`pipeline/screeners/run_all.py`，新变量 `session_date = last_completed_session().isoformat()`；`to_json` 先转一遍再重包，不是 `to_dict`+`json.dumps` 直接拼，因为 pandas 把 NaN 转成 JSON `null`，Python 的 json 模块会吐一个 `NaN` 裸词元，前端 `JSON.parse` 会拒收）。**这一步是顶层形状改动**（数组→对象），已同步三处直接读裸数组的 Python 消费方——`pipeline/themes/build_groups.py`（`load_benchmark()`、`run()` 各一处）、`pipeline/themes/export_review.py`、`pipeline/quality.py`（`site_rows()` 的 `etf_data` 分支、`REQUIRED_BLOCKS["etf_data.json"]` 从 `[]` 改成 `["as_of","data"]`）——与两处独立 `fetch('/data/output/etf_data.json')` 的前端点：`frontend/src/hooks/useMarketData.js`（`FILES` 循环里特判 `etf_data`，取到就解出 `.data`）、`frontend/src/components/groups/useSpyRow.js`（同样解包）；两处解包都在拿到 payload 的第一时间做，其余组件（`Layout.jsx`/`EquitiesSection.jsx`/`BreadthPage.jsx` 等）读到的 `data.etf_data` 仍是原来那个裸数组，未改一行。**镜像快照** `frontend/public/data/etf_data.json`（数据契约窄口子同步，ROLE.md 既有授权）同步重新打包成同一形状。`data/reference/schema_snapshot.json` 已 `--update` 接受新形状（顺带吸收了一批与本任务无关、此前已存在的基线漂移——`breadth_panes.json`/`market_light.json`/`theme_board.json`/`theme_ladder.json`/`tick_cycle.json`/`ep_qullamaggie.json`/`ep_stockbee.json`/`x_heat.json` 等此前从未 `--update` 过的「new file」，`--check` 已确认收敛为 `schema unchanged`）。
  **②规范键名定为 `as_of`**（ET 交易日 `YYYY-MM-DD`），唯一权威定义 `pipeline/reference/output_date_keys.py`，测试 `pipeline/tests/test_output_date_keys.py` 遍历 `data/output/*.json` 强制顶层必须命中注册表里的键，新文件漏了就红。**没有把 15+ 个既有写点批量改名**——旧文件已经在用 `timestamp`/`date`/`asof` 之一，判定为已有可用信号，批量重命名对死线管线风险与收益不成比例，本次只补了确实一个都没有的 `etf_data.json`；给 `theme_board.json` 加了 `as_of` 别名（与已有 `asof` 同值）因为它是本次点名对象。`RECOGNIZED_DATE_KEYS` 只收 5 个真正的新鲜度键（`as_of`/`timestamp`/`date`/`asof`/`members_asof`），**`proxy_map_date`/`parallel_until` 故意不收编**——它们是日期但不是新鲜度信号，见③。取舍与理由的完整版记在 `data/reference/METRIC_SOURCES.md`「data/output 顶层日期键口径」节，本行不重复。
  **③`theme_board.json` 的 `proxy_map_date=2026-09-24` 比 `asof=2026-09-25` 差一天，核实为巧合，不是固定滞后设计。** 追到 `pipeline/constants/theme_proxies.py` 的 `PROXY_MAP_DATE` 常量——它是主题→代理 ETF 映射表自己的版本日期（该文件顶部注释「改这张表时一起改」），只在有人编辑映射表时才变；今天差一天只是因为映射表上次编辑是昨天，等下次编表这个数会跳到编表那天，不会继续跟着 `asof` 走。已在 `pipeline/themes/proxy_board.py` 的返回字典加注释澄清，避免下一个人再查一遍。
  **测试**：`pytest pipeline/tests tests --deselect pipeline/tests/test_content_processor.py` 3673 passed；前端 `npx vitest run` 84 files / 652 tests 全绿；`npm run build` 通过。（alex）
