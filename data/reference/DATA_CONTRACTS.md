...
- **[2026-09-23] DATA ALEX → Studio Q：龙头回踩（`liquid_leader_pullback`）`dcr_pct` 阈值 0.10→0.20，已合 main，T-0923-50。**
  ...
  实测 2026-09-22 session（`data/output/universe.json`，gated pool 2052 只）：改前（>10%）58 只，改后（>20%）54 只，差集 ANF/DINO/MPC/NTNX（`dcr_pct` 分别 0.189/0.196/0.154/0.141，卡在旧闸内新闸外，都是收盘落在当日区间下两成那一类，与任务预期的「ANF、DINO、MPC、SID 一类」同形，个别名字随池子快照略有出入）。全量测试 3333 passed（`test_original_definitions_0918.py::TestLiquidLeaderPullbackAlex` 同步改了边界值 0.10→0.20）。`schema_snapshot --check` 有一批与本次改动无关的既存漂移（面板字段列表本身没变，`dcr_pct` 仍是原键，未动基线）。
