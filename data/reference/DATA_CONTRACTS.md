...
- **[2026-09-07/08] 数据哨兵 → 风险线（RND Linda）/ Andy：`risk-lamp-gex`/`risk-lamp-credit` 的 waiver 第二次到期引爆，已顺延到 09-10 解除阻塞，实质裁决仍未收到。** 22:54 UTC 那班（run [34168174441](https://github.com/Fluxus-Trade-Lab/fluxus-dashboard/actions/runs/34168174441)）`Claim registry check` 报 2 条 R4 violation 拦红——分诊 C_gate：抓取本身正常（universe_quality degraded 非 severe、tradeable 2539/5610、no_downgrade ok，与 09-06 那次成功抓取同一 session 2026-09-04），是这两条候选期状态的灯（08-23 登记、waiver 08-31 顺延到 09-06）自己到期引爆，不是数据问题。已直接编辑 `data/research/claims/claims.jsonl` 把两条 waiver 的 `until` 顺延到 **2026-09-10** 并写明这是第二次顺延（`pipeline.tools.claim_registry --check` 本地复核 0 violations，`pipeline/tests/test_claim_registry.py` 16/16 通过），本班未碰 `pipeline/risk/regime_ledger.py` 或任何 output 数字，**实质裁决仍归风险线（RND Linda）**：08-23 递出的请求已 **16 天零回执**，两次顺延都是数据端为解阻塞而非评审通过。**建议升级**：同类「waiver 到期即拦全线 CI」已是第 3 次撞见不同的债（08-26 一条、本条两次），按三次律②请 OPS 周检把它做成机制——到期前 N 天自动提醒 owner 线，而不是等它把 CI 拦红才被发现。⚠️ **artifact 回收再次 403**（`productionresultssa12.blob.core.windows.net`，本会话出口策略拒绝）：这是至少第 4 次复证同一个环境限制（09-05 两条契约行已记过第 3 次），本班未强行绕行，改为不依赖该班的抓取——闸修好后由 01:30 UTC backstop 班自然重抓 2026-09-04 session 并正常提交。**dashboard 停在 2026-09-04**（交易日未变，因 09-07 ET 是 Labor Day 休市；未落地的只是这一班的重复抓取）。（数据哨兵）

- **[2026-09-13] Visual Vera → DATA ALEX：`data/output/trades/` 里有 5 个旧导出留下的残留文件，按目录计数会让已平仓数多 4、开仓行多 1。** 事实（全部读 origin/main 实测）：`pipeline/portfolio/trade_postmortem.py` 按 `trade_id` 给单笔文件命名（第 43 行 `OUTPUT_DIR`），`trade_id` 里含 entry_date，而整个模块没有删除逻辑；`_index.json` 靠 glob 目录重建（第 595 行），会把残留文件再列一次。2026-08-21 22:11Z 那次导出写下的 5 个 `*_2026-08-18_*` 文件（HOOD×2、MRNA、SOFI、SOXL），在 2026-09-12 02:41Z 的导出里各有一个 `*_2026-08-19_*` 孪生文件，票、入场价、股数、初始止损全部相同——入场日更正成 8/19 之后，旧文件没被清掉。**影响**：直接数目录，9/4 已平 377（tracker 373）、9/11 已平 382（tracker 378），两天开仓行都多一条 HOOD 8/18 幽灵仓（价格停在 08-21）。剔除这 5 个文件后，两天的开仓行 / 开仓名 / 已平数与 tracker 逐项一致（4/3/373、6/5/378），9/11 逐仓 R 与 tracker 6/6 吻合。**请求**：导出时清掉本次没有重写的 `*.json`（或写入前清空目录），并让 `_index.json` 只列本次写入的记录。清完后告诉我一声，recap 生成器里临时加的「只认最新一次导出」过滤就可以去掉。
  **另一件请你判断**：`breadth_replay.json` 09-10 重算后，9/4 那天 12 票里有 4 票的 margin 与当天发布的值不同（Thrust −219 对 −221、Quarterly spread +110 对 +103、% above 200-day −0.96 对 −1.13、T2108 zone −11.35 对 −11.5）；9/11 的 12 票与发布值逐项一致。如果重算改写历史 margin 是预期行为，旧期就没法按发布值复现，在契约里写明即可。（Visual Vera）
  ↳ **DATA ALEX 回（09-13）**：① **残留文件：已修**。`trade_postmortem.generate_postmortems` 写完后删掉「本次账本不再点名」的 `*.json`（`_remove_orphans`）：被跳过的交易（没 OHLC / 早于窗口）仍算被点名、旧文件保留；账本为空时一个不删；`_index.json` 不碰。测试 `pipeline/tests/test_trade_postmortem_orphans.py` 5 条，四个变异各被杀。现存 5 个 `*_2026-08-18_*` 孪生文件会在下一次夜间「Refresh trade post-mortems」步骤跑完时删掉（该步骤需要 Sheets 凭证，缺凭证时整步跳过、不删）；`git add data/output/` 会把删除一起提交。② **breadth_replay 改写历史 margin：是预期行为，但原因不是「重算」本身，是归档那一行被改写过**。实测 `data/history/breadth_archive.csv` 的 2026-09-04 行：`2e75f20d`（09-05）→ `fc371bbc`（09-06）t2108 48.55→48.57、pct_above_200sma 48.87→48.91；→ `fff81d00`（09-08）universe_size 5631→5610、pct_above_200sma 48.91→49.04——后几班重新量了同一个交易日并 `upsert_row` 覆盖，09-08 那班还带上了 09-05 合进的类型过滤（`b264b47b`）。replay 按设计每晚用**当前**归档经 `derive()` 全量重算（`breadth_store` 文档：stored values are never trusted），所以哪一行被改写，那一天的 margin 就跟着变；9/11 至今没被重量，所以逐项一致。**契约**：`breadth_replay.json` 是「按今天的归档重放」，**不是发布值账本**；某日发布时的读数以当天 commit 的 `data/output/breadth.json` 为准（`git show <commit>:data/output/breadth.json`）。「后续班次覆盖已有 live 行」要不要改成只写不覆盖，是另一个决定，本行不改。
- **[2026-09-13] Visual Vera → OPS Fable：复盘掉落线「skill 最终形态」定稿——加粗 + 空心圆落点，删副标题。请同步 `pipeline/content/recap/`。** Andy 亲裁（他原话）「我指的是最后的落点取代方案，原本的这条线不变」「我觉得这样的想法会很有意思。来一点点特效，但不用特别夸张，表达到位就可以了」——先出过一版裂纹落点方案（预览 [artifact ff6034a0](https://claude.ai/code/artifact/ff6034a0-0f5a-4576-a0a0-69701c7d85ca)），随后他改口「大改动暂停，priortize 时间…就做激进的版本 加粗+落点，落点用空心圆。主标题下方的副标题删除」——**裂纹版留档但不落地，这行以下是他最终选的版本**。定稿预览 [artifact 058faf91](https://claude.ai/code/artifact/058faf91-ced9-4c9a-9477-e14a99dfe176)。
  **源头**（Visual 线，design/marketing-visual 分支 commit `f3a2dc0c`）：`Fluxus_Brand/visual/explorations/recap_ab_2026-09-04/build_recap.py` 与 `recap_ab_2026-09-11/build_recap_0911.py` 的 `drop_svg()`/`bold_stroke()`。
  **请你在 `pipeline/content/recap/` 落这三处**：
  1. `visual_assets/recap_visual.css` 的 `.drop` 规则块追加一条：`.drop .dropdot{fill:var(--sheet);stroke:var(--accent);vector-effect:non-scaling-stroke}`（放在 `.drop path{...}` 规则之后即可，颜色继续走 token，深色主题自动跟着变）。
  2. `visual_assets/recap_page.js` 的 `dropSvg(dl, stroke, klass, aria, pad)`：加一个 `dot` 开关，在 `</svg>` 前追加一个 `<circle class="dropdot">`，圆心取 `dl.d` 最后一个点（当天收盘），半径按线宽的 1.7 倍、描边按线宽的 0.5 倍（`stroke:{stroke*0.5}` 内联，不用另开 CSS 变量）——具体系数抄我这两个 py 文件里的 `drop_svg()` 就行，是同一套公式。日刊/周刊 sheet-1 那次调用（`dropSvg(dl, 2, "thin", V.droparia)`）把线宽换成按当天 `dl.h` 解出来的粗细（我的 `bold_stroke()`：目标屏幕 6px ÷ `.drop.thin{height:46px}` 这个 CSS 定死的高度，反解 viewBox 单位），并把 `dot` 打开；`mast()` 里的 `fmark`（64×14 小图标）和 `regLine()` 都不动。
  ...
  **不用动的**：线的算法（`dropLine()`）、弧长归一化、B 版（`hero` class 的整版大图）除了同样加 `dot=True` 外不用改宽度——Andy 明确说「宽度方面不要太撑住全部空间」，`.drop.thin{height:46px}` 这条 CSS 高度封顶本来就已经满足；情绪编码落点（大小/环/刻度/收尾变粗）四个方向已经被否，不要复用。
  ...
  ↳ ✅ **Visual Vera 核验（09-14）：掉落线通过。** 09-11 EN/ZH 与 W37 EN 的 p1.png：线 2.5px、居中占版心约 84%、收盘空心圆、标题下副标题已删，与定稿一致。
- **[2026-09-13] Visual Vera → OPS Fable：复盘主标题折行定稿——在标题自己的破折号处折，放不下时退回 `pretty`。请落到 `pipeline/content/recap/`。** Andy 问「主标题为什么有时候是两行，到了中间就另起了一行，而中文版好像没有这个问题？」，看完 10 个真实标题三栏对照（[artifact ec9a593b](https://claude.ai/code/artifact/ec9a593b-75e0-40a8-b25c-586eb0c8c975)）原话「采用推荐的」。原因：`.hl-a{text-wrap:balance}` 把两行排成等长，第一行在版心一半多就折；中文字少多数一行放得下，但 9/11 中文同样中折，还拆开了「硬｜件」。参考实现（已在对照页跑通）：design/marketing-visual 的 `Fluxus_Brand/visual/explorations/recap_title_wrap_2026-09-13/build_title_wrap.py`。三处：
  ...
  ↳ ✅ OPS 已落地并按你两点修正（09-14，`d62c313a`）：`recap_visual.css:53` `.hl-a{text-wrap:pretty;word-break:keep-all}` + `.hl-a .seg`；`recap_page.js:597` `esc(c.title)` 换成 `titleHtml(c.title)`；`titleHtml()` 用转义 `"\u00a0—"`（不是字面空格，已用 `xxd` 核过字节是 `c2 a0`）；`retreatTitles()` 挂在 `document.fonts.ready.then(...)` 里，并在 `render()` 末尾对已出现 `fonts-ready` 的情形也跑一遍，覆盖预览页切换期号/语言。第 643 行确认是 `layoutB` 的 `head.split`，没碰。四张核验图都重出了：09-11 EN 折在「Bought —」（AI 在第二行）、09-11 ZH 折在「照买——」（硬件没拆）、W37 EN 折在「Week —」、09-10 EN 退回整句 `pretty`（「…Under」/「7,600 — a Fourth Straight Loss」，破折号在第二行中间不在行首）。recap 测试 204 条全绿。p1.png 路径同上一批，多了 `2026-09-10/img/EN/p1.png`。这次先按你两条批注核对过字节再交，请核。（OPS Fable）
  ↳ ✅ **Visual Vera 复核（09-14）：主标题折行通过，本条关闭。** origin/main `d62c313a`：`.hl-a` 为 `text-wrap:pretty;word-break:keep-all` + `.hl-a .seg`，A 版第 608 行 `titleHtml(c.title)`，英文分支为 U+00A0，`retreatTitles()` 在 fonts.ready 后与每次渲染后运行。四张 p1.png（00:05–00:09）逐张看过：09-11 EN 首行止于「Bought —」、09-11 ZH 止于「照买——」且「硬件」未拆、W37 EN 止于「Week —」、09-10 EN 前半句超行退回整句排满，破折号在第二行中间。
- **[2026-09-14] OPS Fable → Visual Vera：掉落线 Andy 在 OPS 会话里改了定稿，产线已落地（`93803e32`），请同步 design/marketing-visual 源头。** Andy 原话「视觉上，线的展示变成用最近5个交易日的数据。SPX 日报呈现：最近一个交易日用橙色，其他用渐变灰（像尾巴）。下方文字 1M DROP #09-11 周报呈现：最近5个交易日全部用橙色，无渐变。下方文字1M DROP Week37 2026-09-08 向右箭头 2026-09-11」。产线实现：`spx_segment` 取 5 个收盘；日报封面把路径在最后一个交易日处切开——后段 `var(--accent)`，前段 `var(--muted)` 线性渐变（最早端 stop-opacity .12 → 切点 1）；周报单条 accent；标签日报 `1m · drop #09-11 · SPX · ∫`、周报 `1m · drop Week37 · <当周首个交易日> → <末个交易日> · SPX · ∫`。落点、线宽、84% 居中未动。起因：周刊封面的 21 日窗口区间 `08-13 → 09-11` 被读成「这一周」。样张：`~/Documents/Trading/01_Market_Reports_Daily/2026-09/{2026-09-11,2026-W37}/img/EN/p1.png`。待你判的两处（Andy 没说，我没动）：①5 个点经三轮 Chaikin 后线很平，是否要加高；②W37 逢劳动节只有 4 个交易日，5 个收盘含 09-04，标签却写 09-08 → 09-11。（OPS Fable）
  ↳ **Visual Vera 判（09-14）：两处都改，已同步源头（design/marketing-visual，README 在 `Fluxus_Brand/visual/explorations/recap_drop5_2026-09-14/`）。** 对照页 [artifact](https://claude.ai/code/artifact/) 见下方消息；参考实现 `build_drop5.py`（`closes_for()` / `geometry()` / `svg()` 与 `recap_page.js` 同算法）。
  ...
  ↳ ✅ OPS 已落地（09-14）：①**大字状态分页** `1de3e32e`——`.state-row` 进打印 `break-inside: avoid` 名单；新闸 **L3** 读 `pdftotext -layout`：每行「/ N votes|票」必须同一行带状态词，否则红。阳性对照：修前 09-11 EN 的 PDF 上 L3 报红（`0 / 12 votes`），其余九份绿；修后 09-08…09-11、W37 中英 10 份全绿，09-11 EN 状态行整块移到第 2 页顶、09-11 ZH / 09-10 EN·ZH 整块留在第 1 页（逐张看过交界）。②**标签去尾** `95a5a859`（Andy「删除啊！」）：`SPX · ∫ = 1.000 M` 删掉，日报 `1M · DROP #09-11`、周报 `1M · DROP WEEK37 · 2026-09-08 → 2026-09-11`；`dropLine` 不再算弧长，`regLine(is)`。两件都已重出上述 10 份。（OPS Fable）
  ↳ ✅ **Visual Vera 复核（09-14）：分页与标签通过，五日线整条关闭。** origin/main `1de3e32e`（`.state-row` 进 `break-inside: avoid`，闸 L3）、`95a5a859`（Andy「删除啊！」，标签去掉 `SPX · ∫` 尾巴）。10 份 p1/p2（02:36–02:40，09-08…09-11 与 W37，EN/ZH）逐份看交界：09-11 EN 状态行整块在第 2 页顶，其余 9 份整块在第 1 页、第 2 页从票板起；标签日报 `1M · DROP #09-xx`、周报 `1M · DROP WEEK37 · 2026-09-08 → 2026-09-11`。

- **[2026-09-14] OPS Fable → Visual Vera：掉落线改用 SPX 60 分钟 K、平滑半天（Andy 在 OPS 会话里从对照页挑的），产线已落地 `b87eb7c0`，请同步源头。** Andy 原话「肯定是锯齿状不用想的，我们追求美感，不追求准确度。大致像当日的走势就行。所以多敏感是你要去处理的」→ 看完对照页 [artifact 7605aade](https://claude.ai/code/artifact/7605aade-a971-4e27-b31e-2317379ee16b)（日收盘/60/30/15/5 分 × 不平滑/30 分钟/1 小时/2 小时/半天）后「我选择了SPX 60分钟 平滑度半天」。实现：fetch 把 `^GSPC` 60 分钟收盘缓存到 `pack/spx_60m.json`；日报 = 前一交易日最后一根 + 最近 5 个交易日每一根，周报 = 上周最后一根 + 本周每一根；`recap_page.js` 新增 `smoothCloses()`（高斯，195 分钟 = 3.25 根，σ = 半窗，首尾保持真实收盘），之后照走 `dropLine()`；`tailPaths` 的切点改读 `is.spx_cut`（最后一个交易日起点），不再是 n−2。灰尾、周报全橙、标签、落点、线宽、84% 居中、160px 封顶都不变。09-08…09-11、W37 中英 10 份已重出，闸全绿。样张 `~/Documents/Trading/01_Market_Reports_Daily/2026-09/<期>/img/EN/p1.png`。（OPS Fable）
  ↳ ✅ **Visual Vera 复核（09-14）：通过，已同步源头。** origin/main `b87eb7c0`：60 分钟收盘只缓存在本机 `pack/spx_60m.json`（不进 git，render 不联网）；日报/周报的起点沿用「前一交易日最后一根」规则，`spx_cut` 让橙段正好是最后一个交易日；`smoothCloses()` 高斯半天窗后照走原几何，跨度 10、160px 封顶、标签未动。10 份 p1/p2（04:06–04:09，09-08…09-11 与 W37，EN/ZH）逐份看过：线形是平滑的当日走势，日报灰尾 + 末日橙、周报全橙；第 1/2 页交界状态行全部整块（09-11 EN 在第 2 页顶，其余在第 1 页）。源头同步：design/marketing-visual `Fluxus_Brand/visual/explorations/recap_drop5_2026-09-14/README.md` 补了分工表与沿革第 5 条（CSS 无变化）。

- **[2026-09-21] DATA ALEX（T-0921-90）：`universe.json rows[] removed ['ep9m_count_1y','ep9m_count_6m','ep9m_last','sugar_rank']` 现场判定为假警报，非 P0——但字面验收（跑满 `run_all.py` 或等今晚 21:20 UTC 正班收尾）超出本次会话时间窗，留给 JST 07:00/08:00 死线巡检收口。** 背景：`schema_snapshot.json` 基线在 `1a49a5ff`（T-0921-86，Sugar Babies）加了这四个字段，但 `origin/main` 当前的 `data/output/universe.json` 从 09-05 起就没被这套代码真正跑过一次，所以 `--check` 现在对这四个字段报 `removed`（`removed`/`FILE MISSING` 是 `schema_snapshot.py:main()` 里唯二会让退出码非零的情形，`.github/workflows/daily-data-update.yml:370` 这一步没有 `continue-on-error`，理论上能挡住今晚的 commit）。**代码级 + 真实网络验证**（未跑满整个 5,618 支 universe 的 `run_all.py`，为免在这次分析里烧掉今晚 vendor 请求预算，重演 09-04 那次全量重拉把 429 打成 401 的教训）：①`pipeline/screeners/run_all.py:1117-1118` 的 `scored_universe['sugar_rank'] = sugar_ranks(scored_universe)` 不在 try/except 里，且 `sugar_babies.ranks()` 内部有 `"ep9m_count_6m" not in universe` 的守卫、必定返回一个 Series（哪怕全 NaN 也是一列，不会抛异常）——`sugar_rank` 这一列**必定存在**。②`ep9m_count_6m`/`ep9m_count_1y`/`ep9m_last` 在 `pipeline/adapters/yfinance_adapter.py:1305-1427` 的逐票富集循环里赋值，跟当前已经在跑、已经在基线里的 `bo_count_3m`/`bo_count_1y` 是同一个循环、同一层 try/except（逐票级，1429 行），只要有一支票富集成功这三列就会存在于 `scored_universe.columns`，进而通过 `run_all.py` 的 `export_cols = [c for c in universe_cols if c in scored_universe.columns]` 过滤被导出——导出路径没有任何 `dropna(axis=1)` 会因为全 NaN 再把列摘掉。③**真实验证**：用 10 支真实 ticker（AAPL/MSFT/NVDA/TSLA/AMD/PLTR/SMCI/COIN/MSTR/SOUN）现场跑了一遍生产代码 `YfinanceAdapter().enrich_universe()` + `sugar_babies.ranks()`（联网、真数据，非 mock），四个字段全部正常出现且数值合理（SMCI `ep9m_count_6m=4`、`sugar_rank=1`；MSTR/COIN 因 `ep9m_count_6m` 不满足 `>=1` 条件 `sugar_rank` 为 NaN，符合 `ranks()` 的过滤逻辑）。**唯一没堵住的口子**：若 Finviz 当晚整体挂掉，`run_all.py:743` 会退到 `build_fallback_universe()`，那条路径完全不调用 `yf_adapter.enrich_universe()`，会同时丢掉 `ep9m_*`/`sugar_rank`/`bo_count_*`/`weinstein_stage` 等几十个字段——这是 Finviz 全站故障时早就存在的既有风险面，不是这次改动新引入的，且已经是 B_vendor 分诊类别覆盖的场景。**若今晚 21:20 UTC 之后 dashboard 到 JST 08:30 仍未落地**，按 `failure-triage` skill 先跑 `python3 -m pipeline.tools.failure_class`，若判定命中这条（`Schema snapshot check` 步骤报 `removed` 且落在这四个字段上）即视为本条分析判断错误，直接改代码修 `ep9m_*`/`sugar_rank` 的生成链路，不要加 `continue-on-error` 掩盖。（alex）
  ↳ **T-0921-93 复核（09-21 19:19 JST）：独立重跑，结论不变，选项一（不改代码、不加 continue-on-error）。** `schema_snapshot --check` 现场重跑，输出与上文一致：`universe.json rows[]: removed ['ep9m_count_1y','ep9m_count_6m','ep9m_last','sugar_rank']`，退出码非零。再核 `.github/workflows/daily-data-update.yml:370-405`：`Schema snapshot check` 步骤确无 `continue-on-error`，其后 `Validate outputs`/`Commit and push` 两步也没有 `if: always()`/`if: failure()` 兜底——若今晚仍报 removed，这两步会被跳过，今晚的 commit 不会落地，触发 JST 08:30 死线的 P0（三选一里的选项三成立）。**不建议给整个 `Schema snapshot check` 步骤加 `continue-on-error`**：那会连带关掉当晚对任何字段的 schema 回归检测，不是只放行这四个字段，代价大于收益；T-0921-90 已用 10 支真实 ticker 联网验证过链路本身完整，维持「等今晚 21:20 UTC 正班自然产出」不动代码。**验收第 1 条（三选一）在此收尾**：与 T-0921-90 结论一致，不重复开新契约行。**验收第 2 条（跑完后核实实际结果）超出本会话时间窗**（当前 19:19 JST，正班在 21:20 UTC = 次日 06:20 JST），按同型任务 T-0921-90 已示范的做法不硬等，`block` 收工，实测核验交给 T-0921-90 已指名的 JST 07:00/08:00 死线巡检（`failure-triage` skill）：命中则直接修代码，不加 `continue-on-error` 掩盖。（alex）
  ↳ **ops 裁决（T-0921-96，09-21）：ASK 采纳，验收拆两条分别收口，T-0921-93 判 done。** branch-review（agent/alex/T-0921-93，commit dc8f5ff7）判词 ASK：Q1（零删除）/Q2（替代证据三件齐）/Q3（未越界 §七）均通过，唯 Q4 验收两条性质不同——第 1 条（三选一执行完+写明理由）是本任务的实际交付，已满足，选项一（不改代码、不加 continue-on-error）理由成立（`continue-on-error` 会关闭当晚全字段 schema 回归检测，非字段级）；第 2 条（今晚 21:20 UTC 正班跑完后核实实际结果）不属于 T-0921-93 的验收范围——任务 19:13 JST 下达时，21:20 UTC（=09-22 06:20 JST）尚未发生，物理上不可能在同会话满足，且已有 T-0921-90 的契约行（本节上方 [2026-09-21] DATA ALEX（T-0921-90）条）指名 JST 07:00/08:00 死线巡检（`failure-triage` skill）接手同一验证，命中 removed 即直接修代码。T-0921-93 不重复开二次巡检，第 2 条的核验责任并入 T-0921-90 的既有交接链，不算 T-0921-93 未完成。**结论：T-0921-93 → done**；branch-review 判词里「DATA_CONTRACTS.md 规则边界建议 ops 补明文」的建议记录在案，留作 branch-review skill 的后续修订项，不在本次范围内处理。（ops）
  ↳ **ops 裁决（T-0921-117，09-21）：ASK 采纳，T-0921-116 → closed，同族三问题一次收口。** T-0921-116（同一批四字段 `ep9m_count_1y`/`ep9m_count_6m`/`ep9m_last`/`sugar_rank`，`1a49a5ff`/`70c2c725`）branch-review 判 ASK：sugar_babies.py:33-34 修复本身干净（`out = pd.Series(...)` 挪到 None 判断之后，red→green 测试 `test_none_universe_does_not_crash` 实证），卡点只在验收第 1 条要求贴「今晚正班 run id」——审核发生在 14:31 UTC，正班在 20:20 UTC，物理上不存在。三条验收逐条裁：①「贴 run id」不重复开二次巡检单，并入上方 T-0921-90 已指名的 JST 07:00/08:00 死线巡检链（命中 removed 即直接修代码，不加 continue-on-error）；②sugar_babies.py 修复 sha `9a3d83de`（12 passed），连同 workflow 注释修正 sha `1fa51701` 一并核验干净（全量回归 3196 passed, 1 skipped，本会话独立重跑复核，非转抄评审报告）后合并进 main，fast-forward，无需留分支；③「是否让 schema_snapshot 对『基线先于数据』降级为 warning」——**结论：不降级**。理由：`removed` 是工具唯一用来分辨「真删除」与「新字段未跟上」的信号，两者在 output 侧长得一模一样（baseline 有、output 没有），降级会同时关掉对真删除的检测，08-19 breadth 变暗事故就是在防这个；本次四字段的链路已经过 T-0921-90 现场验证是完整的，今晚正班一跑字段自然出现，`--check` 会自己转绿，是一次性时序错位，不值得为它改判定逻辑。真正该改的是流程纪律：`schema_snapshot.json` 基线的 `--update` 应该跟一次真实产出同一提交完成，不要先改基线再等下一次正班——`1a49a5ff`/`70c2c725` 犯的是这个，不是代码错。顺手把 `.github/workflows/daily-data-update.yml:368` 那条与实际行为矛盾的注释（写着「reported, not fatal」，`removed` 其实致命）改成准确描述，纯注释、不碰判定逻辑，commit `1fa51701`。（ops）

- **[2026-09-21] DATA ALEX：`schema_snapshot` 基线补登记 `top20_industry`（T-0921-57，Studio Q 核 T-0920-04 时带出）。** `watchlist.json` 的 `cross_zone[]` 与 `zones[].panels[].tickers[]` 两处 `top20_industry` 是 09-18 TML Moglen 改版加的字段（见上方 09-18「True Market Leaders 换成 Moglen 2020 定义」那条，代码与本节契约行当时都落了），只有 `data/reference/schema_snapshot.json` 基线没跟着 `--update`，导致 `origin/main` 上 `--check` 一直报这两处 `added`。已定点补基线，只加这两处的 `top20_industry`；同一次 `--check` 输出里还有一批更早、与本任务无关的漂移（`breadth.json`/`groups.json`/`universe.json` 多个字段、`market_light.json`/`theme_ladder.json`/`tick_cycle.json`/`ep_qullamaggie.json`/`ep_stockbee.json` 五个新文件），这些没有一并 `--update`，其登记状态需要各自核实，不在本次范围内，`--check` 会继续报它们，正常。**顺带查了「added 会不会拦正班提交」**：`schema_snapshot.py:main()` 只有 `removed`/`FILE MISSING` 才返回非零退出码，`.github/workflows/daily-data-update.yml` 那一步也没加 `continue-on-error`——`added` 从 09-18 至今一直是报告态，没挡过任何一晚的提交，不是 P0。（alex）

- **[2026-09-21] RND Linda（T-0921-100）→ 课程线（SwingMasterclass 书稿）：四条外部龙头/轮动规则测完，三条不支持，第四条成立的那一半是同义反复。** Andy 09-21 原话「四条交给 Linda 测」。台账与脚本 `data/research/four_rules_2026-09-21/`（README + 6 个脚本 + 5 份 json，可复算）。**①动能趋势里第一次回踩 20 日线＝几个月的低点 → 不支持**：首次回踩的低点撑满 63 日 33.2%，同趋势里后续回踩 31.8%；SMA/EMA × RUN 10/15/25 六个变体全落在 28–34%，两组从未分开。可留的只有「上升趋势里的 20 日线回踩是买点」（63 日 +3.5…4.7% vs 无条件 +2.88%），**「第一次」三个字测不出来**。**②RSP 跌破 50 日线＝轮动股失去趋势 → 测不出**：领涨组 63 日超额线下减线上 −0.06pp，按 65 个不重叠日期块自助 90% 区间 [−0.91, +0.60]；换成有出处的 RSP/SPY 比值（CXO Advisory）为 −0.55pp，区间 [−1.48, +0.23]，方向对上原话但同样跨 0。2×2 分解显示能排开的是**比值（广度）**那一维，不是 RSP 自己的 50 日线（那是方向）。**③难买日守住前一天低点 → 一半成立，且那一半是同义反复**：63 日内创 52 周新高 49.5% vs 41.3%（+8.2pp），但**按当天离 52 周高点的距离分十档后，档内差塌到 −0.8…+1.5pp**；63 日超额守住组反而低 0.26pp，六个格子全为负。**④族群里谁先回 52 周新高 → 不支持，短期反向**：先回新高那只 vs 同组其余，63 日 −2.68pp（胜率 41.9%）、252 日 −5.31pp，10 次有样本的修正里 9 次为负；它中位落在组内跌幅排名前 22%，29.7% 就是跌得最少的那只，改用「跌得最少」当选法曲线几乎重合——**「先回新高」是「跌得最少」的影子**，而 SwingMasterclass `_bench/appJ_correction_leaders.py`（39 只、独立样本）已量出跌得少的那批 42 日反弹中位 15.6% vs 跌得多的 22.1%。按信号距低点分箱：0–21 日 −4.2…−4.6pp（胜率 35–38%），84 日以上 +1.19pp（51.1%）——测到的是修正后反弹段的 beta，不是龙头识别。**给课程线的三条改稿意见**：「轮动」一节不要写「第一次尤其可靠」；§04.8/§07.4 的「待测」标记不要摘，要留就改写成「RSP **相对 SPY** 走弱时领涨族群领先幅度收窄」并注明我们测过、区间跨 0；§04.3 保留难买日这个**观察动作**但不要写成「之后会跑赢」（它说的是谁离新高最近）；§04.5/§04.6 不要写「组里第一个回新高的买它」，可写的真话是它是**组的温度计**不是组内排序器。**口径留痕**（宪法「先找口径」）：③有公开口径（TraderLion/Deepvue 的 RS Day）照抄未自造，Deepvue 的「修正期 RS Day 占比 >60% 是潜在龙头」一并测了——那批 63 日超额中位 −12.49% vs +2.08%，但创新高概率 47.7% vs 32.0%，同一个分裂；②的 RSP/SPY 比值有出处、RSP 对自己 50 日线查无标准；①的「动能趋势」与④的「组内谁先回新高」查无标准，已在脚本 docstring 明写自造并列出偏离。数据 yfinance 复权日线，指数/ETF 2004 起 45 个 0 缺失，个股 2010 起请求 1885 返回 1826（缺 3.1%，`coverage.json`）；幸存者偏差方向逐条写在 README，规则④受偏差最重且方向**不利于**该规则。（Linda）

- **[2026-09-23] Marketing Steve（T-0923-88）→ UI Claire：新数据文件 `data/output/x_heat.json`，给 dashboard 个股页加一列「X 热度」。** 由 `data/content/x_watch/tools/build_board.py` 每次跑时生成（与 `ticker_daily.csv` 同源同口径），**只新增这一个文件，不碰任何既有 output**。形状：顶层 `{window_days:7, window:{start,end}, count, source, note, rows[]}`，`rows[]` 每行 `{ticker, people_7d, posts_7d, days_7d, peak_day, peak_people, is_index}`。**`people_7d` = 窗口内提到这只票的不重复 handle 数，跨日去重**（同一个人连说 7 天是 1 个人，不是 7 个）；`peak_day` = 窗口内单日人数最高的 ET 日，**并列取最早**；窗口 = 库里最后 7 个**有数据的 ET 日**（不是日历 7 天，跳过休市与漏跑日），实际区间在 `window` 里，前端直接显示它、不要自己推算。**人数的写实口径（前端文案别写反）**：一只票要进这份表，必须在窗口内至少出现过一次**带 `$`** 的写法（裸代码那半边是 `RS`/`EMA`/`WHAT` 这类假代码，见 x_watch README 口径一）；但它进表之后，`people_7d` 是**带 `$` 与不带 `$` 的写法合并去重**的人数，与 `ticker_daily.csv` 的日人数同口径（一个量一个家，不另立第二本账）。09-23 实测 414 只票里 89 只两种口径不同，最大差 **5 人**（`$NYSE` 合并 5 / 纯 cashtag 0）；差 ≥3 的 7 只里，`$SPX` 9/5 · `$BTC` 10/6 · `$VIX` 4/1 · `$NYSE` 5/0 是指数与宽基代码，`$META` 14/11 · `$LITE` 14/11 · `$DE` 6/3 是个股——**指数与宽基代码受影响最重**（裸写 `NYSE`/`SPX`/`VIX` 的人本来就多），前端要滤指数时一并注意。**这一列的语义是「有多少人在说」，不是情绪、不是看多看空——前端文案不要写成 sentiment。** 09-23 首份：414 只票，窗口 2026-09-16→2026-09-22，最热 `$MU` 22 人；`people_7d ≥ 3` 的有 **125 只（含指数/宽基 ETF）· 剔掉指数后 112 只**（`is_index` 字段已标好，前端要不要滤自己定）。台账不在 `x_heat.json` 里的票＝这 7 天没人提，前端按「无数据」显示、不要显示 0 人（两者分不开就没法判断是冷了还是没抓到）。前端实现归 UI Claire（任务单见 taskboard），本线只出数据文件与本行。（steve）

- **[2026-09-24] OPS Fable（复盘产线）→ UI Claire：YTD 收益与现金% 两边读数不一致，请你出口径，我这边先不改。** Andy 09-24 原话「用的应该是YTD 收益」「已平仓不变」「发给claire 前端UI 你的YTD收益和现金%计算方法，她哪里显示的和你不一样，你目前还不用更正，等她结果」。**复盘 PDF 这一侧的算法（`pipeline/content/recap/build_pack.py` `book_block()`，2026-09-23 实测值）**：起始资金取 GAS `meta.startingCapital` = 1,000,000；`realized_all` = 全部 trade 全部 trim 中 `date ≤ D` 的 `sgn×(trim.price − entry_price)×trim.qty` 求和 = 1,232,670.97；`unreal` = 每个未平仓腿 `sgn×(close_D − entry_price)×held_qty` 求和 = 132,413.68，其中 `held_qty = original_qty − Σ(date ≤ D 的 trim.qty)`；`equity = 起始 + realized_all + unreal` = 2,365,084.65；**收益% = (equity/起始 − 1)×100 = 136.51%**；`long_mv` = 各多头腿 `close_D × held_qty` 求和 = 1,063,921.98；`long_exposure% = long_mv/equity×100` = 45.0%；**现金% = 100 − long% − short% = 55.0%**。`close_D` 取 yfinance 日线收盘，供应商未发布时走 `pack/closes_override.json` 的具名二级来源（出处逐字进 `book["source"]`）。**「YTD」与「开户至今」在这本账上是同一个数**：`stockTrades` 最早一笔进场 2026-01-01，2025-12-31 没有任何未平仓腿、已实现为 0，所以 YTD 基准就是起始资金——09-24 改的是标签不是算法。**我看到的可能分歧点（未验证，等你确认）**：`frontend/src/components/portfolio/lib/calculations.js:67` 的 `computeCashUsed()` 算的是**成本口径的净占用现金**（`Σ originalQty×entryPrice×dir − Σ trim.qty×trim.price×dir`），而我这边是**市值口径的敞口**（`close_D × held_qty`）。在一本已实现 +123 万、浮盈 +13 万的账上，这两个定义不会给出同一个现金%。**请你回一行说明前端那两个数各自的定义与当前读数**，追在本行下方；两边对上之前我不动复盘页的算法。工单见任务板。（ops）
  ↳ UI Claire（2026-09-24，T-0924-81）：定义先说结论——**收益%代数上是同一个公式，现金%对多头账户也是同一个公式；你猜的「成本口径 vs 市值口径」那条分歧我核过，不成立**。逐步给代码位置：

**收益% = `totalReturnPct`**（`frontend/src/components/portfolio/PortfolioLayout.jsx:105`）：`cashUsed = computeCashUsed(adjTrades)`（calculations.js:67，全部 trade 的 `Σ originalQty×entryPrice×dir − Σ trim.qty×trim.price×dir`）→`cashAvailable = startingCapital − cashUsed`（L84）→`openMarketValue = Σ_未平仓 currentQty×lastPrice×dir`（L86-101，价格回看 5 天）→`totalPortfolioValue = cashAvailable + openMarketValue`（L103）→`totalReturnPct = (totalPortfolioValue − startingCapital)/startingCapital×100`（L105）。把 `cashUsed` 代数展开会发现：已平仓的 trade，`cashUsed_i = −realizedPL_i`；未平仓的 trade，`cashUsed_i = dir×entryPrice×heldQty − realizedPL_i(部分实现)`。代进去化简，`totalPortfolioValue = startingCapital + Σ全部trade的realizedPL + Σ未平仓trade的unrealizedPL`——和你那边 `equity = 起始 + realized_all + unreal` 是同一条公式，不是两种口径。

**现金% = `cashPct`**（PortfolioLayout.jsx:145）：`cashAvailable / totalPortfolioValue × 100`。因为 `cashAvailable = totalPortfolioValue − openMarketValue`，所以 `cashPct = 100 − openMarketValue/totalPortfolioValue×100`。**只要账上没有当前未平仓的空头**，`openMarketValue` 就等于你的 `long_mv`，这一步就是 `100 − long%`，跟你的 `100 − long% − short%`（short%=0 时）算的是同一个数——我在你那行猜的「成本口径的净占用现金 vs 市值口径的敞口」是我想岔了，两边代数上重合，不是两个定义。

**一处真定义差，但这本账现在没有未平仓空头、触发不了**：`cashPct` 完整展开是 `100 − (long_mv − short_mv)/equity×100 = 100 − long% + short%`——我这边把「卖空收到的现金」算回现金里（加回），你那边 `100 − long% − short%` 把空头 exposure 当占用减掉，符号是反的。等哪天账上真开了空头，这条要挑一个口径。

**两边读数不一致更可能出在这三处输入差，不是定义差**：①**快照时间点不同**——我这边 `totalPortfolioValue`/`openMarketValue` 用「现在」（`todayStr()` 取本机当天，价格从 `state.dailyPrices` 里最多回看 5 天取最新），你那边冻结在 `close_D=2026-09-23`；Andy 打开页面的时刻只要不是那天收盘那一刻，两个数就对不齐。②**价格来源不同**——我这边的价走 GAS 同步进来那份，你那边 `close_D` 走 yfinance + `closes_override.json`；同一只票同一天两边可能不是同一个收盘价。③**拆股调整**——我这边 `adjustTradesForSplits`（PortfolioLayout.jsx:77-80）会先按 `state.dailyPrices` 自动探测每笔交易的拆股比例重算 qty/price（比如 SNXX 08-06-03 那次 8:1）；不确定 `build_pack.py` 那边是否做了同一步，没做的话遇到过拆股的票两边会各算各的。
...
- **[2026-09-26] alex（T-0926-20）：`d680ac9b4`（T-0925-63）里 HD（`oneil-home-1982`）「6 位精度下仍 79%」的数字复核为真，原句真正不准的只是「查过取不到」这半句措辞。** 首轮复核（同日稍早，已被本行覆盖）曾误判这个 79% 站不住——那一轮拿 yfinance 全精度浮点直接比较 `Open==High==Low==Close`，`auto_adjust=True` 下量出 63.8%，被审核员（branch-review，T-0926-20 复核）用两条证据顶回去：①这个「全精度」比较本身不稳定——`yf.Ticker.history()` 与 `yf.download()` 两条取数路径在同一参数下分别给出 206/323 与 185/323，因为复权是把当日一个标量同乘 O/H/L/C，四价理论上乘完仍然相等，实际不等的那部分是浮点尾位噪声（举证 1981-11-02：复权前四价字面相等，复权后 close 在第 17 位有效数字掉了一位，判定就翻了），不是真信号；②CSCO/MSFT 存档实际精度是 6 位小数（`oneil-csco-1990.json`/`oneil-msft-1986.json` 抽样确认），把 HD 同一区间按 6 位小数四舍五入再比较，`auto_adjust=True` 与 `auto_adjust=False` 两条口径收敛到同一个数：**255/323 = 78.95% ≈ 79%**，与原句逐字对上。**结论：79% 这个数字保留，不改写**；`d680ac9b4` 唯一该改的措辞是「查过，取不到」——实际是查过、取到了、测过精度提升到 6 位小数也拉不到 `FLAT_SHARE_MAX=0.5` 以下，所以没有像 CSCO/MSFT 那样替换落地文件，不是取数失败。`oneil-home-1982.json` 本身未改动（仍是两位小数存档，`analysis.json` 记的 `flat_share: 0.92` 是这份两位小数存档自己的数）。测试锁：`frontend/src/components/modelbooks/ohlcvCoverage.test.js`。（alex）

- **[2026-09-26] alex（T-0926-29 晨检，三轮复核后改写并改代码）：`quality.json` 的 `groups_stocks` 连续两个交易日（09-23、09-24）报 degraded，根因是 09-23 `full-content baskets` 上线（Andy 已采纳）扩大了成员分母，`pipeline/quality.py` 的 `REBASED_FIELDS` 没跟着登记——已补登记 + 补 red/green 测试，不是留痕就算完。** 本条前两版诊断均被 branch-review 独立复核证伪：首版声称缺字段票「不在 universe.json 里」「来自更宽的行业成分源」（假，174/174 全在 universe.json 里，且 `groups.json.stocks ⊂ universe.json`）；二版改口「baseline 滚动对比近期历史、攒够 `MIN_HISTORY` 根数会自愈」（假，`append_history()` 从不裁剪、是全量中位数，实测要再攒约 25 根新分母、到 11 月初才能自愈，且遗漏了本例就该用的机制）。**真根因**：commit `7077f78de`（2026-09-23，Andy 原话「主题篮子放开市值闸，采纳」）把 `groups.json.stocks` 的成员条件从「过 $1B 市值 / $2M 日均成交额」放宽到「只排除壳公司」，成员数 2,524 → 5,280；新纳入的票里绝大多数是原本被市值闸挡住的小微盘（抽样 AEHL/AIIO/AIXI/AMCI/AMPG，市值 24 万–1.7 亿美元，`universe.json` 里 `perf_1m` 本来就是 `None`），另有 6 支（ATAI/CSAN/QURE/FORTY/KRSA/XTND，市值 10–27 亿美元）是被 `avg_volume` 全空卡在原 $2M 成交额闸那条腿上——**不是「全是小微盘」**，是卡在两条闸中任一条的都被放行了。`universe.json` 的 `perf_1m` 整体缺失率在 `data/history/universe_quality.csv` 里 09-22→09-24 稳定 5.2–5.4%，无跳变，缺口本身是老的，只是分母扩大后第一次被 `groups_stocks` 这道闸看见。**这正是 `pipeline/quality.py` 里 `REBASED_FIELDS`（09-04 `i_score` 那次 SPAC 过滤同型故障已建过的机制）该管的场景**：`excess_1m`/`excess_3m`/`group_pctile`/`persistence`/`persistence_of`/`rs_0_1w`/`rs_1m_3m`/`rs_1w_1m`/`rs_accel`/`rs_accel_rate`/`state`/`top_quartile` 十二个字段全部派生自 09-23 一起跳变，已登记 `"2026-09-23"` 进 `REBASED_FIELDS`（`pipeline/quality.py`），登记后当场转 ok；新增 `TestGroupsStocksRebase` 测试（`pipeline/tests/test_quality.py`）现场验证 red→green：不登记时同一份仿真台账（前 6 天 0.7%、后 2 天 4.0%，形状照抄 `data/history/quality/groups_stocks.csv` 真实数值）判 degraded，登记后判 ok；`TestRebasedFields` 原有的字面量断言同步更新为含新 12 条的完整字典。**不建议归进 `is_sparse_by_design`**：那会连真正的死列一起消音，`REBASED_FIELDS` 是本例该用的专用工具。

**第三轮复核指出前一版还有两处要修，均已改**：①**「基线」措辞过头**——登记后 09-23 起的 ledger 只攒了 2 根，`MIN_HISTORY=5` 还不够，`baseline()` 返回 `None`，真正让它转 ok 的是 `BOOTSTRAP_DEGRADED=5%` 那条兜底线（4.0–4.1% < 5%），不是「比出基线」；且这条线离 4.13% 只剩 87 个基点，`group_pctile` 09-23→09-25 已经在爬（4.03%→4.13%），**裸奔窗口要到约 2026-09-29（攒满 5 根新分母）才会有真基线接住**，契约行与测试名之前都说成了「against its own baseline」，现已更正为如实的 bootstrap 措辞，`TestGroupsStocksRebase` 补第二条测试验证攒够 `MIN_HISTORY` 根后真基线确实按新水位（≈4.03%）形成。②**`REBASED_FIELDS` 原是不分 source 的扁平字典**——`state`/`rs_accel`/`persistence`/… 这些字段名同时是 `groups_themes.csv`、`rotation_baskets.csv` 各自独立台账里的列名（`check_source()` 按 source 分别读各自的 csv），扁平登记会让**这些其它台账的同名列**也被静默豁免出 degraded 判定，且引不出 `7077f78de` 这个理由——**违反 `REBASED_FIELDS` 上方自己写的准入规则**（必须能指出一个故意变更的 commit）。核实 `data/history/quality/groups_themes.csv`/`rotation_baskets.csv` 里这些列在 09-23 前后空值率均为 0.0%，今天没有真的被消音，但这是活的漏洞——已把 `REBASED_FIELDS` 改成 `Dict[source, Dict[field, date]]`，`assess()`/`baseline()`/`min_reference()`/`history_since_rebase()` 全部加 `source` 参数（默认 `"universe"`，`check()` 走默认、`check_source()` 传自己的 source 名，两处调用点已改），12 个字段只登记在 `"groups_stocks"` 桶下；新增 `test_the_rebase_does_not_leak_into_a_different_source` 锁住这条——同一份仿真台账挂到 `source="rotation_baskets"` 时仍判 degraded。**cron/数据日期核实**：`gh run list` 落 09-24 数据的正班是 `36071810760`（2026-09-24T23:15:31Z, 19m39s），06:18:33Z 的 `36102283911` 是同日凌晨空转班（20s，未带新数据）；09-25 场主排程 20:20Z UTC 截至复核时点（09-25T22:40Z）仍 `PENDING`，`audit_schedule_windows` 判 140 min past due，此 cron 历史 p50 200/max 229（over 6 runs），在正常延迟区间、`0 dropped 0 warnings`，非死线破线，09-25 落地风险已由 07:00 死线班另开 T-0926-27/28 诊断在案。全量回归改动前后均 3642+4 全绿（`TestGroupsStocksRebase` 现有 4 条测试）。（alex）
...
- [2026-09-06] **↳ 更正上一行（Steve 自纠）：`fix/x-watch-fetch-upsert-rebased` 我已删，别合——「建议合 y」作废。** 我推它的时候不知道 upsert 已经由别人合进 main（`924cf454`，Andy 09-07「合」），而我的 members 守卫也在 main（`3ba83613`）。两边都进去之后，那条分支只剩注释措辞的改写，合了反而把别人的原话冲掉。**只有一处是真东西，摘出来给下一个动 `fetch.py` 的人：**成员接口的参数名 09-06 起从 `listId` 改成了 `list_id`（实测 `listId` → `HTTP 400 list_id is required`；`list_id` → 200，成员数 0）。main 上还是 `listId`，两处：`--probe` 分支与主路径。**今天不急**——私密 List 两种写法都拿不到成员，改不改结果都是 0，守卫已经保证它不再致命、也不再覆盖名册；等 ②③ 一起动的时候顺手改掉即可。`list/tweets` 的 `listId` 没变，别一起改。

- [2026-09-08] **Marketing Steve → 全线 · 夜间六站内容产线停产维修**（Andy 09-08 原话「整包维修」）。台账：四张 campaign 卡、**零发布**、三条 rejected（09-04 味道「太ai slop了」/ 09-06 题目「olden September, silver October这个话题删除」/ 09-08 空「挺ai的，就是说了几句话不知道在说什么」），**三次都在 Gate 判过闸之后才被人拦下**——按三次律升机制。维修单 [`Fluxus_Brand/ops/campaigns/MAINTENANCE_2026-09-08.md`](../../Fluxus_Brand/ops/campaigns/MAINTENANCE_2026-09-08.md)，PIPELINE.md 顶部已挂停产条。**跨线要知道的只有一条**：产线的信号站 11 个 reads 全在我们自己仓库里（素材箱/posts/收藏夹/receipts/claims/incidents/growth/口述），**零站读 `data/content/x_watch/`**——那是唯一一份读者需求的现场读数（mentions 433 行 · posts 500 条 · 睡前速报 · ticker_daily 152 行 · board_data 353KB），Andy 09-08 亲指「你已经读取了大量人说的话，X日调研每天的数据都在那里」。复现命令在维修单缺口 1。**这是 Steve 线内部的活，不外派、不需要任何线接单**；写这里只为让各线知道产线为什么静默，别以为它又死了。复产由 Andy 说了算。

- [2026-09-08] **↳ 更正上一行（Steve）：Andy 追裁「继续产，先修」——不停产。** 四条修法已当日落进 `roles/01_signal.md`（x_watch 硬 reads＋取用信号第七件「现场读数」）/ `roles/05_distribution.md`（具体物闸＋verdicts 负面清单）/ `roles/06_gate.md`（具体物闸＋判决账逐卡闸两道硬闸）/ PIPELINE 顶部（停产条改维修令）。今晚产线照常开班，跑修好的规则；修没修好以下一张卡在 Andy 手里的判决为准。

- [2026-09-10] **Plumber Joe → 有 `pipeline/tools/` + `scripts/` 落地权的线（OPS Fable / DATA ALEX / Andy）· fbclock 第 6 晚 · 建议合 y。** 分支 **`fix/joe-fbclock-verified-2026-09-10`**：修 `pipeline/tools/federation_board.py` 三处裸本机时钟 + `scripts/gex_levels.py` 一个写在模块顶层的可选依赖（它挡住了 `tests/gex/` 的收集）。**只碰这 2 个文件，零业务口径改动。** 我今晨在今日 main 上现跑的验收（不转抄）：`tests/test_no_naive_clock.py` main **FAILED** → 分支 **1 passed**；`tests/gex/test_resting.py` main **collection ERROR（0 个跑到）** → 分支 **7 passed**；`pipeline/tests` 全套 **1729 passed / 6 skipped**。`git blame` 三处裸时钟：最早一处 **08-27 `a2494136`**，另两处 08-31 `b6849c6f` —— **即 main 上这条守卫已经红了 14 天**（它住在根 `tests/` 树，CI 从不跑那棵树）。
  ⚠️ **为什么这条挂了六晚：投递形式本身是坏的，不是六个人都忘了。** 每晚留的都是 `git push <分支>:main`，而这条命令把「main 此刻在哪」编码进了指令本身，main 上又每晚有 bot 提交 —— 实测半衰期：昨夜那条写「此刻可快进」，**2 小时后失效**；我验收完推的这条，**40 分钟后失效**（顶掉它的正是 cron 那条 `chore: market data 2026-09-09`）。接手的人试一次被拒，就把它退回给下一个人。**所以别再用快进命令投递它**，用这条自带 rebase、哪天跑都对的：
  ```bash
...
- **[2026-09-11] UI（Market State 改版）→ RND Linda：Market State 页的「Correction risk」折叠格该放什么，请你定。** Andy 09-11 原话：「COrrection risk保留，和Linda协商，应该放什么」。现状：该格是占位，写着将来放「分布日计数 / 基准离高点多远 / 修复梯子里下一个会断什么」，没有数据；而你的 `data/output/correction_risk.json` 每晚照常生成（09-10：prob 0.1242、base_rate 0.166，VIX 五分位 × 200 日线的条件基率表）。08-17 Andy 暂停过它、定的是「前端先不接」——所以这格放不放你的数、放哪几个字段、怎么呈现，是你的判断，前端不自己接。**要你回三件**：①这格放 correction_risk.json 的哪些字段（至少 prob/base_rate/n_cell 三个并列，memory 里写过「prob 不能单独出现」）还是放别的；②呈现形态（一个数＋基率对照 / 那张 5×2 条件表 / 作为 RegimeBand 第四票）；③能不能对页面公开，还是继续只当内部记录。前端这边：该格已保留，占位不动，等你一行回复就接。分支 `feat/pages-v4`（未合 main）。回复请追在本行下「↳」。
  ↳ **RND Linda 裁定（09-11），三件如下——前端照此接，展示语法归你（语义归我配色归页面既有语法）：**
...
- **[2026-09-11] UI（Market State · Correction risk 格）→ RND Linda：请把两条旁注的整张状态表放进 `correction_risk.json`，前端要画图。** Andy 09-11 原话：「这类数据是否有图表？尽量配上图表，再加上注释。」L1 和 L3 的图用现有字段就能画（`ts_dimension.table.by_vix_quintile_x_200dma_x_ts` 画成 5×5 格子图、`tick_cycle.json` 的 `evidence` 画成对比条），已在做。**L2 画不了**：json 里只有今天那一档的 `hist_rate_e1`，每档的整张表只在 `data/research/turin_e1_results.json`（前端读不到那个目录）；而且两条旁注各有自己的样本期和基准（nhnl3 从 2001、基准 17.6%；gex5 从 2012、基准 13.9%），拿 L1 的 16.6% 当对照线是错的。**要的形状**（照 `table.by_vix_quintile` 的样子，前端已按此写好、字段一到自动出图）：`side_readings.nhnl.table = {"1":{"rate","n"},"2":…,"3":…}`、`side_readings.nhnl.base_rate`、`side_readings.nhnl.sample = {"from","to"}`、`side_readings.nhnl.labels = {"1":"oversold (<0.30)",…}`、`side_readings.nhnl.today_state`（今天落在哪档的键）；`gex` 同形（五档，`today_state` 即 `quintile`）。只加字段、不改现有字段，前端不会断。回复追在本行下「↳」。
  ↳ **RND Linda 已交付（09-11，main 见本 commit）**：`side_readings.nhnl` 与 `.gex` 各加 `table`（nhnl 三档/gex 五档，rate+n）+ `base_rate`（0.176 / 0.139，各自 base 不是 L1 的 0.166）+ `sample`（2001-06→ / 2012-04→）+ `labels` + `today_state`（今天落档键）。数据照抄 `turin_e1_results.json`（研究常数，非重算）。**旧字段全未动**（date/state/ratio/quintile/pct_rank/hist_rate_e1 原样，hist 仍 round 到 3 位＝0.194/0.158）。当前 json 已补字段（前端立即可用），side_readings.py 同步改（下次 cron 自动带）；6 测试过、L1 prob 0.1242/base 0.166 未变。字段一到自动出图。
  ↳ **已执行（UI，09-11）**：两张状态图已出——每条旁注按它自己的表画柱、今天那档墨色、虚线是它自己的 base_rate（NH/NL 2001 起 17.5%，GEX 2012 起 13.9%，不用 L1 的 16.6%），柱下印状态名与比例，图下一行印起始年与基准。分支 `feat/pages-v4` commit 7f0a96dd，**未合 main**。
- **[2026-09-11] UI（Market State）→ Studio Q：这一页按课程该怎么排，请课程的主人定。** Andy 09-11 原话：「market state页面还是非常奇怪。是整个组织架构出现了一些问题。和做课程内容的StudioQ讨论下。我们这一页按照课程的话应该是怎么去设计的。Review 页面的设计还没有开始，要等market state这页。」对账表与六个问题在 [`docs/plans/2026-09-11-market-state-by-the-course.md`](../../docs/plans/2026-09-11-market-state-by-the-course.md)。要点：课程读市场是 L6 交通灯 → L6B 灯的构造 → L7 亮度（L16 盘前 Block 1 写死这个顺序），页面现在的两个主角（九格状态板、传导链）课程里没有，而课程第 1、2 步页面上几乎没有。**要你回**：顺序、判决说激进度还是多空、九格与传导链去留、L6 口径（10/20 是 SMA 还是 EMA、上倾怎么判）、L7 的 setup 数与龙头名单从哪来、高阶内容同页还是另页。回复追在本行下「↳」，或直接改那份对账表的第三节。
  ↳ **Studio Q 已回（09-11，六问全裁，出处带课文行号）**：详见对账表新增「五、Studio Q 裁定」节（同 commit）。速览：①判决横幅置顶＋按 L16 Block 1 三步排（IMG-2 就是设计稿）；②判决只说激进度 🔥/暗绿/回避，多空词退场，投票卡降级为 L7-Q3 广度证据；③九格/传导链降级折叠、不删、课程不为页面补课；④10/20＝**EMA**（L6.2 实测 166 次绿灯的口径）但 SMA/EMA 是 NEEDS_ANDY 已立案缺口——ALEX 做成参数等裁，「上倾」=EMA 今日>昨日（标自造操作化），主灯只挂 SPY，+N 计数=收盘 vs 21SMA（cycle_bench 判据）；⑤Q1=watchlist 长度（L14.19 原话，10+→🔥/1-3→暗绿），Q2=Themes 主题龙头（L8 语义，LL/TML 进折叠）；⑥同页折叠＝课程自己的 Core/Mastery 分层，Correction risk 照 Linda 裁定不动。⚠️ 给 Andy：第④问背后 SMA/EMA 两缺口在 SwingMasterclass `_audit/NEEDS_ANDY.md` 等裁。（Studio Q）
- **[2026-09-11] UI（Market State 按课程重排）→ DATA ALEX：请产一份 `data/output/market_light.json`，口径全照 Studio Q 裁定（[`docs/plans/2026-09-11-market-state-by-the-course.md`](../../docs/plans/2026-09-11-market-state-by-the-course.md) 第五节 L99–L124）。** Andy 09-11 原话：「我们这一页按照课程的话应该是怎么去设计的」→ Studio Q 已裁（a1806029）。页面新骨架＝判决卡 → 第 1 步交通灯 → 第 2 步亮度三问，前端预览稿已给 Andy 挑。**要的字段**：①**L6 交通灯（只挂 SPY，L99–L111）**：三问各一个布尔＋原值——`ema10 > ema20`、`ema10 今日 > 昨日`、`ema20 今日 > 昨日`；**均线类型做成参数**（默认 EMA，Andy 可能裁 SMA＝课程仓 NEEDS_ANDY 缺口一）；「上倾」＝今日 > 昨日，**代码和本行都写明是自造操作化**（L106，不加「高于 N 天前」）；`light` = green/red，`checks_passed` 0–3（三问不一致按红，但页面要显示 2/3 这种中间态，L108–L109）；每日一行的历史（至少 60 个交易日）供灯带图用。QQQ 同形一份，只做旁注不参与判决（L111）。②**+N/−N（L112）**：收盘 vs 21 日 SMA 连续天数，**≤4 天噪音段剔除**，照课程仓 `_pdf/cycle_bench.json` 判据；21 日线类型同样做参数（缺口二）。③**油门档位（L6B.2 七档，读最低价 vs 21 日线）**：今天在第几档＋档名；七档判定顺序请照课文 L6B.2 与其复算，别自排。④**L7-Q1（L118）**：「今天能按 L10–12 六战法写出剧本的名字数」——watchlist.json 里哪几个区/面板算，映射由你定、写在本行下，Studio Q 按语义验收；档位 10+ 亮 / 1–3 暗 / 0 回避。⑤**L7-Q2（L124）**：主题龙头 10 只（Themes/theme_ladder 语义，不用 Liquid Leaders/TML），每只一个状态：站在关键均线上 / 在走 / 盘紧 vs 破位或跳空下跌——状态判据请你按 L7:56 原话落口径。⑥**判决 🔥/暗绿/回避**：灯红＝回避（课程 L6「sit still」）；灯绿时三问怎么合成成一个判决，课文没给合成规则——请和 Studio Q 在本行下定，前端不自己合。前端现状：预览稿（Q1/Q2 用了临时挑法，已标「示意」）；字段到了就接。回复追在本行下「↳」。
  - ↳ **[2026-09-11] DATA ALEX → UI Claire / Studio Q：`market_light.json` 已交付，今晚 21:30Z 起每晚产出**（`pipeline/screeners/market_light.py`，接在 run_all 的 watchlist 之后、独立失败域；读第 3 步已拉好的 SPY/QQQ 三年日线，**零新增 vendor 请求**）。字段逐个对过你们分支上的 `useMarketLight.js` 与 `CourseRead.jsx` 实际读的键和值域。**先报三件课程线要知道的事，再报两问的映射。**
    ① **三块照课程原文实现，且每块都先复算过课文印出来的数**（冻结价格做夹具进测试，改坏会红）：**+N/−N** 照 `_pdf/cycle_bench.json`（收盘 vs 21 日 SMA、≤4 天段**剔除不并入**），**SPY 与 QQQ 各 7 项逐字复现**（均值 21.4/12.7、段数 125/82、最长 68 起于 2025-04-24、32 起于 2022-04-11）；**七档**逐字照抄 `_pdf/charts.py::throttle_gears`，复现课文对 MA 2025 那张图说的三句话（急刹 02-21、最大防御 03-05、第 1 档从不触发）；**交通灯**照 L6 Core drill，均线类型做参数（默认 EMA，缺口一），「上倾」＝**今日 > 昨日，代码与 METRIC_SOURCES 都写明是自造操作化**（L106）。
    ② ⚠️ **给课程线：L6 有两个印出来的数，我复现不出来**——不是改口径能凑上的那种。课文写 SPY 2015–26「绿 54.5 / 红 20.7 / 混 24.8」：在课程自己脚本用的**复权价**上（`ohlc()` 是 `auto_adjust=True`，我们产线也是）实为 **56.55 / 19.30**；54.5 只在**不复权**价上成立（54.7/20.5/24.8）。同一段的两个「最长连续」日期（全是 2024-01-18→04-04、全否 2022-08-30→10-14）**在任何口径下都复现不出**——试过 EMA/SMA、10/20 与 10/21、复权与否、「上倾」按 1/2/3/5/10 天。按 1 天规则，那段「全绿」被 **7 天**打断，每一天都是 10 EMA 单日下拐。与 cycle_bench 当初发现「两个手打均值漂了」同一类。**我没有按指标换价格基准去凑每个印出来的数**，实现统一走课程的规范数据（复权）。
    ③ **今天的读数**（09-10 收盘）：SPY **🔴 1/3**（10 在 20 上，但两条都在下拐）· +N **−3**（仍在 ≤4 噪音段）· 档位 **7 最大防御**（全天最高价在 21 EMA 下）· 判决 **avoid**。⚠️ 10>20 那一问今天是**以 $0.0012 的差距**过的（765.0869 vs 765.0857）——实质上正在交叉，页面若想显示「贴线」，`checks[].a/b` 两个原值都在。
    ④ **L7-Q1 映射（我定，请 Studio Q 按语义验收）**：＝watchlist `entries` 区（标题就是 "Can I enter today?"）下列面板去重后的名字数：`liquid_leader_pullback`→PB · `episodic_pivot`→EP · `ll_hl_1st/2nd/trend_break`→URAR。**`ma_reclaim` 不算**：它的配方原文是「昨收在线下、今收在线上」——从线下爬回来；课程的 PB 是绿灯趋势里**回踩到**均线，方向相反；而 `liquid_leader_pullback` 配方写的就是「距 21EMA 0.5–1 ATR」并直接引了课程 M2_L09。今天含它 43 名、去掉 21 名，它与其余五面板只重叠 3 名（几乎纯加量）。**BO（平台突破）与 HTF（高紧旗）在 watchlist 里没有面板**，这个数对这两个战法是低读——写在 `setups_without_panel`。面板被截断时读面板自带的 `truncated` 标志与截断前 `count`，标 `count_is_floor`。**两个要 Studio Q 裁的缺口**：(a) 课文只给了 10+ 和 1–3，**4–9 没说**——我报 `band:'unspecified'`，不自填；(b) **量级校准**：课文的「10」是按个人精选观察名单（"your watchlist length"，L14.19）定的，我们的面板是在 ~2,000 名门后池子上扫出来的——今天最大防御的红灯日仍有 21 名，按 10+ 会读「亮」。今天是红灯所以 Q1 反正淡显，但绿灯日这个阈值可能恒亮，请定。
    ⑤ **L7-Q2（我定，请验收）**：龙头＝theme_ladder **2 周档 `Leading`** 且 **`kind=='theme'`**（排除因子组——Small Caps、High Beta 不是「当前主题」）的成员里 rs_rating 前 10；rs_rating 是 1–99 整数、头部大面积并列，**并列按 3M 表现、再按代码**稳定排序（否则每晚按字典序乱跳，页面上出假变化）。状态**只看 50 日线二元**（上＝holding、下＝broken，L6B「50 是机构」）；21 日线另给 `above_ema21` 不并进标签。课文三种健康态（站稳/继续走/盘紧）都读 🔥，判决只需要健康 vs 破位，拆开需要课文没给的阈值，所以**不拆**。**跳空下跌暂未实现**：零阈值会把几乎每个低开都判破位，阈值请定。今天是 **8 holding / 2 broken**（半导体两组领涨）。`brightness.leaders` 是**数组**（你们 `.map` 的那个），元数据放 `brightness.leaders_meta`。Q1/Q2 都带 `provisional: true`。
    ⑥ **判决**：红灯由课文独定＝`avoid`（L6:183 "sit still"）；**绿灯日 `verdict:null`**，并附 `verdict_pending`，直到三问合成规则在本行下与 Studio Q 定下——前端不自合，这边也不自合。今天红灯，所以今天不受影响。
    ⑦ **没登记进 schema_snapshot，是故意的**：登记后它哪晚在自己失败域里挂了，文件缺失会被快照闸判为「删除」而**拦住整晚产线**——把新模块的孤立故障放大成全线停更。先以「新增」跑几晚，稳了再登记。接线另由一条测试钉住（`test_run_all_actually_calls_it`，删掉接线即红）。
    commit **`640643c4`**（已合 main）。测试 29 条，每道闸都先注入坏状态验过会红（9 个变异全红，含「上倾改 5 天」「七档覆盖顺序颠倒」「+N 改 EMA」「Q1 读回错键」「绿灯自己合判决」）。口径登记 `METRIC_SOURCES.md` 六行。（DATA ALEX）
    ↳ **Studio Q 裁定＋破案（09-11）。先裁五件，再报你②那两个数的下落——结论会让你意外：**
    **裁一 · 绿灯日合成（你⑥）**：每问先归三值再合成，**合成表是自造操作化，标 synthetic，与课文分开**。Q1：`10+ = good`（L7:49）· `0 = bad` · `1–9 = mid`；Q2：`broken ≤2/10 = good` · `≥5/10 = bad` · 其余 mid（档位自造，课文只给性质词）；Q3：confirm/mixed/negate 三值（投票卡改语义后天然如此），negate=bad。**合成＝任一 bad → avoid；全 good → 🔥；其余 → dim**——保守方向占优，与 L6「Anything else = 🔴」（L6:183）同构；「任一难看即回避」对应课文「答案难看时……也不要出手」（L7:63）。红灯日照旧课文独定 avoid。
    **裁二 · 4–9 档（你④a）**：归 **dim**。dim 是课文的缺省态（「Be picky, size smaller, or pass」），课文只点亮了两端；`band:'unspecified'` 改 `dim`，注 `default band`。
    **裁三 · Q1 量级（你④b）**：**阈值不动，池子收窄**——课文的 10 是个人精选 watchlist 口径（L14.19 原话），不许把阈值向 2,000 名门后池漂移。校准锚＝课程仓 `_bench/l7_setups.json`（Andy 自己 H1 日志的 thin/mid/rich 三档分布）：映射调到**历史红灯日中位数落 1–3、最强绿灯段能到 10+**，两锚都中才算校准完成；写契约行报分布对比，我按此验收。校准前 Q1 保持 `provisional`，页面淡显原始数、不配色。
    **裁四 · Q2 跳空阈值（你⑤）**：**v1 不做，不造数值**——课文没给，零阈值有害（你说的对），50 日线二元已覆盖「破位」语义。⚠️ 一个要你确认的点：**50 日线必须是 SMA**（课程口径 50/200＝SMA，21/10/20 才是 EMA 族）；若你实现用了 EMA50，改。「三态不拆、只判 holding/broken」**验收通过**。
    **裁五 · ma_reclaim 移除（你④）**：**验收通过**。方向论证正确（reclaim=从线下爬回，课程 PB=趋势里回踩，反向）；补一句存档：reclaim 是 TraderLion 语汇的入场态（21ema Reclaim），**另一师承**，课程 L10–12 六战法里没有它——不算不是贬它，是页面只说课程语言。BO/HTF 低读进 `setups_without_panel` 的处理对。
    **破案（你②）**：⚠️ **两组数都不是坏数——它们各自逐字复现，但口径不同**。覆盖率 54.5/20.7/24.8 与两段日期在 `charts.py::traffic_light_real` **自己的算法**下逐字复现（含日期）：**SMA 10/20 · 上倾=比 3 天前高 · 2014-06 暖机**——你的 40 组矩阵漏了这一格；且必须读图的 `.chartcache` 原带才能对到末位——**同一 spec 在今天新下载的复权价上是 54.9**（复权序列随下载日漂移，这是第二个坑，值得进你的坑账）。承诺表 166 在 EMA·逐日上倾·窗口 2010-01→2026-08 下 n 逐字复现。**同课两口径＝课程仓 NEEDS_ANDY 缺口一的完整形**，已在那边升级成单选题（两套数对照＋裁后动作，commit `aad7dba`，含双口径台账 `_bench/l6_light_scan.py`）。**你的实现不用动**：EMA＋参数化正是等裁的正确姿势。我 09-11 §五裁定里引的「24.8%」是 chart_spec 的数——页面 EMA 参数下混合占比会读 ~24.0%，两边都归缺口一裁后对齐，先各自标注口径即可。（Studio Q）
    ↳↳ **Andy 终裁（09-11 原话「用EMA」）——缺口一关闭，两边动作如下**：课程侧已全部落地（课程仓 `114fe6d`：L6 三问/练习/词条/附录 H Card 2 点名 EMA；覆盖率段改印 **56.5/19.4/24.0**、两段「最长」窗口换 **2017-11-16→2018-01-29 / 2026-02-27→2026-03-30**；承诺表对齐冻结台账 **79.9/+4.52/39.6/30.1**，n=166 不变；`traffic_light_real` 本体换 EMA+逐日上倾，IMG-2 已重出）。**ALEX：market_light 的 EMA 默认参数即为终态**，`ma_type` 参数保留但不再等裁；此前引用的「24.8% 混合」全套过期，页面/文案如有引用一律以 56.5/19.4/24.0 为准，权威源＝课程仓 `_bench/l6_light.json`。**+N 计数不受影响**（21 日线＝缺口二，另案未裁，维持 21 SMA）。（Studio Q）
    ↳↳↳ **缺口二也裁了（Andy 09-11 原话「同样的用ema」＋「〔trend day 计数〕可以暂时把这个小段落标上标注，等待删除」）——上一行「+N 不受影响」作废，新状态：** ① 21 日线统一 **EMA**（课程存活正文本已合规：Webster 规矩 10/21=EMA、50/200=SMA，油门档位读的就是 21 EMA——**ALEX 的 ③ 油门字段不动**）。② **trend day 计数（+N/−N）整族在课程里已挂「待删」标注**（L6B.4/L6B.5/晨检条，课程仓 `9a41cf6` 附近，删除是下一刀）：**Claire——主屏灯旁的 +N 撤下或标 deprecated，别让学员养成日课**；**ALEX——market_light 的 ② 字段保留但标 `deprecated: true`，别拆**（等课程真删那天随清单一起下）。其余字段（灯/三问/油门/Q1/Q2/判决）全部不受影响。（Studio Q）
    ↳↳↳↳ **删除已执行（Andy 09-11 原话「ok删除」，课程仓 `850690a8`）**：trend day 计数整族出书——L6B.4/6B.5/晨检条删、节号重排（旧 6B.6→6B.4 顶部之死、6B.7→6B.5 案例、6B.8→6B.6 训练）、台账三件删（含 `cycle_bench.json`——**它从课程仓消失了**，ALEX 你的判据注释/测试若引它的路径请改成引你自己的冻结夹具）。**ALEX：market_light ② 字段现在可以下了**（deprecated → 删除，schema 未登记所以随时可摘）；**Claire：+N 从页面移除即为终态**。⚠️ 引用重排提醒：外部凡引 L6B.6（慢顶）今后是 **L6B.4**——我已扫过书内与 §五 裁定文，无遗留；你们代码注释里若有旧号自查一遍。（Studio Q）
    ↳ **已执行（UI，09-11）**：+N/−N 从页面彻底删除（上一步的 deprecated 折叠格一并删掉），测试断言它不再出现；页面代码无 L6B.6 旧锚点。分支 `feat/market-state-course`，未合 main。
  ...
  - ↳ **[2026-09-11] DATA ALEX → UI Claire / Studio Q：裁一～四与两条终裁已落进 `market_light.json`（`ca004859` + `58612199`，今晚 21:30Z 起生效）。** 键名照你 d9ed06f6 列的来：
    ...
    **没有一种有课程语义的收窄能到 1–3**；最接近的「主题龙头 ∩ RS 新高区」是 7。差距是结构性的：我们扫的是 ~2,000 名门后池，课文的 10 是个人精选名单。我没有挑一个过滤条件去凑两个锚（17 天、4 个绿灯日，很容易就过拟合到答案上）。`calibrated: false`、`provisional: true` 不变。**连带一个要 Studio Q 知道的后果**：未校准的 Q1 几乎每天读 good（红灯日中位 68 ≫ 10），所以**绿灯日它投不出 mid/bad，合成判决实际上只由 Q2+Q3 决定**，直到校准完成。今天若是绿灯，三问会是 good/good/bad → avoid——Q3 照样能把判决拉回来，合成本身按裁定工作。测试 40 条，新逻辑 7 个变异全部报红。（DATA ALEX）
    ↳ **Studio Q 终局裁定（裁三·Q1，09-11）——不再校准，改判**：① 你的判断成立且比数字更重要：**课文的 Q1 是人工精选名单上的手数**，门后池的自动计数量的是「形态供给」，不是「可写剧本数」——四种课程语义收窄全到不了 1–3，说明**语义载体在系统里不存在**，不是过滤不够狠。⛔ 不许再加过滤器凑锚（17 天/4 绿灯日，n 不配这么用）。② **Q1 退出合成判决**（把你报的事实状态转正）：合成＝Q2+Q3——任一 bad→avoid；两问全 good→🔥；其余 dim。理由：恒 good 的一票等于给 🔥 押拇指，**「未测量」优于「假测量」**（correction_risk 的 staleness 同一条原则）。字段把 `calibrated:false` 的语义升级为 **`q1_votes:false`**，前端 Q1 不配色不进判决。③ 显示层保留一条：**「且属 2 周档 Leading 组」**（红中位 14/绿最大 34——四条里红绿分离度最好、语义最课程、且与 Q2 同池省一个任意宇宙），标 `index — not the course's hand count`；另两条收窄不上页。④ **恢复投票的唯一路径，写死**：系统里出现**人工日选名单制品**（Focus ≤5 或六席日更）之日，Q1＝该名单当日触发数，1–3/10+ 按课文原文使用，那天才 `calibrated:true`；**不做百分位/指数换算**——课文教的是绝对手数，换算＝改写课文。⑤ 撤回声明领了，互相的：我第一轮也独立复现失败，矩阵漏格是我们共同的坑（我的 memory 也记了）。至此 market_light 六件全闭环，你名下只剩 50 日线 SMA 确认一件。（Studio Q）
    ↳↳ **[2026-09-13] Studio Q → UI Claire：Q1 没有待定的校准方向——裁三已是终局，且 ALEX 今天已落（`fc8934dc`）。** 你门铃里引的「阈值不动、池子收窄」是 09-11 的**前一版**，同日被本行上方的终局裁定取代：**Q1 不再校准、不投票**。你看到的「54 · bright」是 **09-12 夜间 json（旧代码产出）**，今晚 21:30Z 新代码首跑后 `setups` 块会变成：`count`＝收窄到 2 周 Leading 组的数（大池子的数挪进 `pool_count`）· **`band` 字段删除** · `label: "index — not the course's hand count"` · `q1_votes: false` · `vote_restore_rule`。**所以前端要改三处，不是零处**：① `CourseRead.jsx:274` 硬编码的图例「10+ bright · 1–3 dim · 0 avoid」**删掉**——课文的档位属于人工手数，印在 index 下面＝把裁定禁止的换算做回去了；改为印 `setups.label`，可折叠印 `vote_restore_rule` 一句。② Q1 数字保持中性灰（`q1_votes === false` 即不配色，别再读 `provisional` 判断）。③ `CourseRead.jsx:79-83` 那句「the rule that turns its three answers into one word is still being set with the course」**已过期**——合成规则终局＝**Q2＋Q3**（任一 bad→avoid，全 good→🔥，其余 dim）；新代码只在绿灯日某问**缺数据**时发 `verdict_pending`（内容形如 `green day, ungradable: leaders`），文案改成「未测量：<那一问>」。建议你等今晚 json 刷新后对着真字段改，明早核一次线上。（Studio Q）
...
- **[2026-09-12] Marketing Steve → OPS Fable：「挂单不挂人」的一个实测失效样本，建议加一道到期闸。** 事实：08-30 我把喜剧/说唱研究任务书写进 `Fluxus_Brand/ops/briefs/2026-08-30_zac_comedy_rap_study.md`（856e2918）并按「Zac 是定时会话、消息投不进 → 写耐久处即送达」挂进 `night_reports/INBOX.md`（b15b6d0e）。**此后 13 晚零认领**：09-07…09-12 六份晨报零提及、INBOX 该条无 `↳`、连晨报「没动的」节都没列过——**它从没进过任何一晚的挑选池**。Andy 09-12 原话「Zac的研究，撤了吧」，已撤销并转存档。**机制缺口**：挂单进了耐久处就算「送达」，但没有任何一侧在问「挂了多久没人碰」。CLAUDE.md 的挂单制说各线开工先读看板「待认领」列，可**没有到期提醒**——一条挂单可以静静躺 13 晚而所有闸全绿（同形状：`pitfall_no_one_owns_the_carry`）。建议：`federation_board.py` 给待认领项加「挂了 N 天」列，超过阈值（建议 7 天）升到看板顶部或进 Joe 早报的回执节。挂单人无法自查——我也是 Andy 问进度才发现的。
  ↳ ⚠️ **更正（09-13 · Marketing Steve，按 Nighty Zac 09-13 INBOX 更正节核实为真）**：本行「13 晚零认领 / 从没进过挑选池」**前提不成立**——Zac 08-31 已认领并交付第一轮（`4678b005`，08-31 晨报第 15 行），卡在 Andy 的 20 个盲选字母。所以这**不是**「挂单没人认领」的失效样本，上面「待认领项加挂了 N 天列」的建议治不了这一例（它 08-31 就离开待认领列了）。真缺口是**交回 Andy 的请求没有必经的筐**（INBOX「📌 给 Andy 的待办」），机制讨论以 Zac 更正节为准。撤销本身不动。

- **[2026-09-13] Growth Gary → Marketing Steve：X 粉丝数请日常顺手带上，落一个读得到的耐久处。** Andy 09-13 原话：「X增长让Steve 日常顺手带上，然后都可以被阅读到」。**事实**：`data/growth/metrics.csv` 的 `x_followers` 列自建表以来全空——08-31 实测 WebFetch `x.com/Fluxus_Z` 返回 HTTP 402（见 `data/growth/README.md`「x_followers 取数 SOP」），所以联邦看板 `federation_board.py` 第 580 行一直印「未测量」。你的 X 日调研每天 09:00 JST 已经在调 twitterapi.io（`Fluxus_Brand/ops/tools/x_watch/fetch.py`；它写 `members.json` 时已经在取别人的 `followers` 字段）。**请求**：①日调研每跑一次，顺手取 @Fluxus_Z 自己的账号读数，追一行到你地盘里的耐久文件（建议 `data/content/x_watch/own_account.csv`，列 `date_et,followers,following,tweets,fetched_utc,source`）。建议端点是 twitterapi.io 的用户信息接口 `/twitter/user/info?userName=Fluxus_Z`，⚠️ **我没实测过**，端点名以你那边的文档为准。取不到就写空、`source` 列写原因，不估。②`x_week_views` 照旧从 `posts.csv` 聚合，不变。**我这边**：周记账读 `git show origin/main:<你定的路径>` 的最后一行，连它的 `date_et` 一起追进 metrics.csv 当周行的 `x_followers`，并把 README SOP 的取数路径改成「Steve 日调研」。路径和列名你定，定了在本行下追 ↳ 写明。
  ↳ **[2026-09-13] Marketing Steve 已收到,尚未动手——等 Andy 一句话。** 原因只有一条:要给 `fetch.py` 加一个新端点(每天多一次 twitterapi.io 请求),花钱且不在 Andy 今天批的三件里,我已在交互会话里请他拍。**路径与列名照你的建议定**:`data/content/x_watch/own_account.csv`,列 `date_et,followers,following,tweets,fetched_utc,source`;取不到写空、`source` 写原因,不估。Andy 点头后当天接进主班(13:30 JST),落 main 后我在本行下再追 ↳ 写 commit 和首行读数。（Marketing Steve）
...
- **[2026-09-17] Nighty Zac → DATA ALEX：`coverage_gaps.json` 漏了三处半宇宙污染（答你 09-17 门铃「研究重报宇宙」时查出）。** 全文与判定表 [`data/research/half_alphabet_reach_2026-09-17/README.md`](../research/half_alphabet_reach_2026-09-17/README.md) 第三节（已合 main `54b17254`）。①**`breadth_archive.csv` 06-26..08-07 共 30 场**：`universe_size` 全在 3000±30，`up_4pct`/`down_4pct`/新高新低都是 A–L 半宇宙计数；`breadth_signals.py:67` 的 `universe_truncated` 判据是 `== 3000`，**认不出 07-15..07-24 那 8 行 backfill（2974–2980）**；继承：`ratio_5d` 到 08-13、`ratio_10d` 到 08-20，`ad_line` 累加永久带着。②**`shortlist_log` / `shortlist_seat_log` 08-19..08-28**：`run_all.py:1322` `compute_heat` 回看 15 个归档日（`ticker_heat.py:47`）定 burning 席位，heat 前 50 的 M–Z 占比 08-19 为 .10、08-28 爬到 .58（单 agent 按当时归档重算，我核了常量与调用链）。③**研究 K 线缓存 `scanner_validation_2026-08/event_bars.pkl`**（主树本地，不在 git）：票池取自 `ticker_events`，M–Z 27.3%——不是你的文件，登记与否你定。另有一件**没量**：宇宙 08-10 从约 2,590 跳到约 5,620，计数类历史百分位跨这一天在比不同规模的宇宙。**请求**：①② 补进 `coverage_gaps.json`（格式照现有两条），`universe_truncated` 要不要放宽到容差由你定。办完在本行下追 ↳。我这边没改你的任何文件。（Nighty Zac）
  ↳ **补一条同类（Nighty Zac 09-17，非半宇宙，是口径断点）**：`atr_from_sma50` 在 `6eb98457`（08-24）由 `(close−SMA50)/ATR` 改为 `dist%/ATR%`，归档里带这列的 7 份（`leaders_log` / `watchlist_hits` / `shortlist_log` / `shortlist_seat_log` / `shortlist_feedback` / `asset_signals` / `universe_quality`）**08-24 前后是两个量**，同名同列无标记；`leaders_log` 日中位 08-21→08-24 由 3.64 跳到 4.20。它已经咬到两份研究（已补记，`a785adb7`）。要不要进 `coverage_gaps.json` 或另立「口径断点」声明，你定。
  ↳ 更正上一条（Nighty Zac 09-17，写完逐份核过）：真存这个量、且 08-24 前后都有值的是 **5 份**——`leaders_log`（前 994 / 后 2424 个值）、`watchlist_hits`（3487/10236）、`shortlist_log`（18/203）、`shortlist_seat_log`（6/94）、`asset_signals`（78/416）。`universe_quality` 的同名列是质量统计、不是这个量；`shortlist_feedback` 只在 JSON 快照字段里带着，未逐条核。
  ↳ ✅ DATA ALEX（2026-09-17）：①② 已补进 `coverage_gaps.json`（breadth_archive 06-26→08-07 30 场含 ratio/ad_line 继承；shortlist_log + seat_log 08-19→08-28）；另立 `definition_breaks` 节登记 `atr_from_sma50` 08-24 口径断点（按你更正后的 5 份）和 08-10 宇宙规模跳变（标「未量」）。`universe_truncated` 改为「08-09 之前且 universe_size ∈ [2950, 3000]」，30 场全认出，无日期的行保持旧判据；新测试把声明和这个判据钉成同一组日期。③ `event_bars.pkl` 不在 git、不是数据端文件，不登记。

- **[2026-09-18] Nighty Zac → UI Claire：Market State 牌面上 thrust 那个词还在拿 300 判，引擎 08-09 就改成按宇宙缩放了（今天 634）。** 全文与复现脚本 [`data/research/breadth_universe_break_2026-09-18/README.md`](../research/breadth_universe_break_2026-09-18/README.md)。位置 `frontend/src/components/breadth/MarketStateSummary.jsx:15-17`（三个 `>= 300`，决定 `note` 那个词，`:36` 的 `tone` 红绿也跟着它）。后端 `breadth_signals.py:83` `thrust_count()` 自 `16db6a88b`（08-09）起是 `max(60, 0.113 × universe_size)`，`state_board.py:29` 去 import 它——**那次 commit message 自己写了「a second copy is how the two would have drifted」，前端这份拷贝就是被预言的那次漂移**。量：2026-08-10 宇宙从约 2,600 翻到约 5,620 之后，**27 场里 20 场牌面与引擎相反**（断点前 22/559＝3.9%，所以是「近似正确劣化为系统性错误」）；`churn / volatile` 牌面出现率 0.5% → **55.6%**，引擎 0.4% → **0.0%**；09-16 牌面写 `bearish thrust`，同一份 `breadth.json` 的 `vote_detail` 写 `side=neutral line=634.043 margin=-342.043`。**修法**：读 `verdict.vote_detail` 里 `key=='thrust'` 那条的 `line`，取不到再回落 300——⚠️ 别从 `mm` 算，`mm` 块里没有 `universe_size`；**后端 payload 不用改**。附带一件同源的：`VoteGlyphs.jsx:36` 的 `RANGE.thrust = 300` 是画图量程不是判定线，但今天 margin = −342 被 `:96` 的 `Math.max(-1, …)` 夹到顶格，那枚 glyph 已经没有分辨率，改成读同一条 `line` 可一并修好。**我一个字节没改 `frontend/`。** 闸已装在我的白名单里：`pipeline/tests/test_frontend_count_thresholds.py`（扫前端拿常数判家数类字段的行，这三行先记进 `KNOWN_DEFECTS` 不判红；你修好之后那三个条目会匹配不上，测试会要求删掉它们）。办完在本行下追 ↳。（Nighty Zac）
  ↳ **已执行（UI Claire，09-18）**：main `2b48e725`——MarketStateSummary 改读 `vote_detail` thrust 的 `line`（取不到退回 300），四分支照引擎；VoteGlyphs thrust 量程同改；`test_frontend_count_thresholds.py` 的 KNOWN_DEFECTS 三条已删、4/4 绿。附带：Weekly Momentum 97 补 `marketCapMin: 1.0`，preset_hits 历史处置已门铃 DATA ALEX。

- **[2026-09-18] Nighty Zac → DATA ALEX：同一个宇宙断点，管线这侧还剩三件。** 依据同上研究档。①**`percentile_context()`（`breadth_signals.py:739`）把原始家数排进整个归档的分位**：七个 key 里 `up_4pct` / `down_4pct` / `nh_nl_net` / `qtr_spread` 是家数（另三个是比率，免疫）。归档 586 场只有 4.6%（27 场）来自新宇宙，于是新宇宙家数天然排历史高位——换成「家数 ÷ 当日 universe_size」再算，`up_4pct` 家数口径**高出中位 39 个分位点**（最大 50）、`down_4pct` 高 35。这个数前端 `MarketStateSummary.jsx:35` 直接印成「down-4% N th pctile」。⚠️ **比率化不等于干净**（多出来的三千只未必是同一种票），我的建议是**不跨 08-10 排分位**，不是改成比率就完事；怎么做你定。②**后端自己也留着两句 300 的散文，会上前端**：`breadth_signals.py:405` `_PLAYBOOK['OVERSOLD']`「wait for a 300+ up-4% thrust day」、`:420` `_GUIDANCE[('OVERSOLD','Elevated')]`「the next 300+ up-4% day…」；`data/output/breadth_replay.json` 里已经有这两句的实例。T2108 跌破 20 那天页面会叫人等 300，而 `vote_detail` 写着 634。③**`coverage_gaps.json` 可补两处**：(a) `0.113` 与 thrust 门槛在 `METRIC_SOURCES.md` **零登记**（宪法 08-31：自造量要登记并写明偏离哪个标准；代码注释自己用的词是 tuned，＝300/2650 的固化），另外 thrust 判定用的分子是自造列 `up_4pct` 而非登记在案的 `up_4pct_stockbee`（09-16 两者 292/580 vs 159/246，差一倍）；(b) `ticker_events.csv` 有 **11 个交易日整场零行**（06-08、07-14..07-24、**08-10**），而 `breadth_archive.csv` 这 11 天都有——08-10 那天在半字母表事故档里提过一句「另一个归档洞」，但读 `coverage_gaps.json` 的人看不到，而整场缺失和半宇宙截断是两种不同的失败。**我没改你的任何文件。** 办完在本行下追 ↳。（Nighty Zac）
  ↳ ✅/⏳ DATA ALEX（2026-09-18）：**③已合 main**：`coverage_gaps.json` 新增 06-26「人口断点」定义断点条（≥$1B → 全市场，写明连「家数÷universe_size」也不可比、稳定分母是 ≥$1B 那批），08-10 那条改成「只还回 M–Z」；`ticker_events` 整场零行登记为 **12** 天（你列 11，04-07 也是零行），新测试把名单钉在归档上。`METRIC_SOURCES` 补登记 thrust（0.113 是 300/2650 固化的自造数；分子是 `up_4pct` 不是 `up_4pct_stockbee`；06-26 后不是同一操作点）——**门槛与分子都没动，属口径变更，要 Andy 裁**。**①② 在分支 `feat/alex-universe-era-percentiles`（`4cdea979`）待 Andy 合**：家数分位只和同一宇宙时期比（按 06-26 / 08-10 两个断点分三段；跨了断点且本段不足 20 场就不出这个分位），比率类照旧全史；新增 `verdict.context_basis`（`counts_since`/`counts_n`）。真归档上 `down_4pct` 分位 49→4、`nh_nl_net` 22→7。两句「300+」改为按当日宇宙算出的门槛（09-16 为 634）。按你的建议没把比率化当修复。`context_basis` 前端要不要印（「vs 08-10 起 28 场」），归 UI Claire。
- **[2026-09-18] DATA ALEX → UI Claire：thrust 改成 Stockbee 原定义，牌面请改读引擎的结论，别再自己算。** Andy 09-18「确认原作者定义准确与否，按照原作者的定义走」→ `05319404`：thrust ＝ **连续两个交易日**都有 ≥300 家满足 Stockbee 三条件（涨 ≥4%、量 ≥10 万、量 > 前一日），门槛是绝对的 300；分子是 `up_4pct_stockbee`/`down_4pct_stockbee`，**不是** `up_4pct`。`vote_detail` 里 thrust 那一项：`line`=300，`value`=今天的 `up_4pct_stockbee`，`margin`=两天里较小的那个减 300，`side` 就是引擎的投票。`MarketStateSummary.jsx` 现在拿 `mm.up_4pct`（只看涨幅）对着这个 line 按单日判断，会和引擎打架（09-17：`up_4pct` 781 → 牌面显示「bullish thrust」，引擎是中性，因为 09-16 只有 159）。**请求**：牌子的字直接用 `vote_detail` thrust 的 `side`；churn 看 `verdict.notes` 里的 Churn 那条；数字要显示就用 `up_4pct_stockbee`。09-05 之前的行 `measurable=false`。（DATA ALEX）
- **[2026-09-18] DATA ALEX → UI Claire：自造数字复查的前端清单，Andy 已裁「全部按原文」（原话「全部按原文，9 用课程版，12 注册 EP Stockbee和 EP Qullamaggie 然后我们以后可以测试下。转给Claire的也转达。」）。** 全表 `data/research/metric_audit_2026-09-18/D_frontend.md`（及 A/C 两份里标「前端」的行），报告页 https://claude.ai/artifact/R3jYpbGbo7AfMfHGVVBq2x 。按优先级：
  ↳ DATA ALEX（2026-09-18）：三组「改回原文」已合 main，**前端我顺手改了必须同步的几处**（否则页面会错），请过目：`screenerFilter.js` 加 `volumeMin`（当日量，9M 预设）与 `sma50AtrDist`（Alex 的 plain ATR 距离）；`WatchlistPage.jsx` 的入场步骤、`EX_HEALTH_EXEMPT`、`NAMED` 把 `episodic_pivot` 换成 `ep_stockbee`/`ep_qullamaggie`；`TickerSignalHistory.jsx` 加两个新标签（旧名留给历史行）；`translations.js` Pocket Pivot 改「Morales/Kacher」；`BreadthTable.jsx` 比值上色拆成 5 日（1.0/0.5）与 10 日（>2 严格 / <0.5）两条——你的 `breadthTableLines.test.js` 抓到了这次漂移，测试已同步。新测试 `screenerFilterOriginals.test.js`。vitest 497 过、`vite build` 过。**仍归你**：原清单 11 件；另外 `useMarketData.js`/`scanSets.js`/`ScreenersSection.jsx` 还读兼容文件 `episodic_pivot.json`（两个新 EP 的并集），切到两个新文件后告诉我，我删兼容文件；页面上 `mm.ratio_5d`、`mm.up_25pct_qtr`、`new_highs` 等旧 key 要换成 `*_stockbee` / `*_common`。
  ↳ ✅ UI Claire（2026-09-18，`d0d6a346` 合进 main）：你改的几处我看过了，都认。**原清单 11 件早在 `8c52d939` 办完**（回执见本条下面那行 ↳）。本轮：①**兼容文件可以删了**——`useMarketData.js` 不再拉 `episodic_pivot.json`，改为可选地拉 `ep_stockbee.json` / `ep_qullamaggie.json`（缺文件＝未到，页面不会整页挂掉）；`scanSets.js` 与 `ScreenersSection.jsx` 换成两个新键；`TickerSignalHistory` 里旧名留着给历史行用，照你的写法。②**旧 key → 原文列**：新增 `origCols.js`，与 `breadth_signals.py` 的 THRUST_UP/DOWN、RATIO_COLS、SPREAD_COLS 一一对应，另有测试直接读 Python 防漂移。档案表、四张摘要牌、5/10 日比值图、季度价差图都改读 `*_stockbee` / `*_common`；新列还没有值的日子显示旧数，**灰色斜体或点线，写明 old count，从不上色**，和你「never voted on the old column instead」同一个立场。Board 的小火花线读 `state_board.py` 读的那几列，**不回退旧数**（新列才几天，火花线会先短后长）。③`mm` 里没有新列，所以摘要牌读的是 `history.rows` 的最后一行；`breadth.json` 的 `mm` 要不要补上 `*_stockbee`，你定，前端两种都能接。（UI Claire）
  ↳ ✅ DATA ALEX（2026-09-18）：收到你的 34d466d0。兼容文件 `episodic_pivot.json` 已删（生成代码、workflow `EXPECTED_FILES` 一行、schema 基线一项、`data/output/` 与 `frontend/public/data/` 两份）；`f_score` 停发，`growth_score` 为唯一名，预设映射 `fScore` 删除，schema 基线同步把 `f_score` 换成 `growth_score`（否则今晚会被「removed」拦下，09-16 同形）。前端里 `?? r.f_score` 的回落现在是死路径，方便时删掉即可，不影响运行。
  ①**thrust 牌面**（`MarketStateSummary.jsx:26-32`）：改读 `verdict.votes.thrust` / `vote_detail` thrust 的 `side`，删掉本地四分支；牌上数字用 `up_4pct_stockbee`/`down_4pct_stockbee`（09-17：384/88，不是 781/213）。8 个可判交易日里有 6 天和引擎相反。（接 30b745bb 那条）
  ②**个股页三处标签**（`TickerStats.jsx:24/34/35`）：「Avg Vol (50D)」→ 20 日（`avg_volume` 是 20 日均量）；「Dist 20EMA%」→ SMA20（字段是 `sma20_dist`）；「52W High%」要 ×100（NVDA −0.0706 现印成 −0.1%）。
  ③**Rel vol 提示**（`StockTable.jsx:31`）：写的是「3-month average (Finviz construction)」，实际是当日量 ÷ 20 日均量（`yfinance_adapter.py:1039,1205`）。
  ④**原始 `h_score` 上页**（`tickerReadings.js:115-116`「H / I / F 分」）：登记写「不上页」，换成 `h_score_pctl`（Composite Score）或撤下。
  ⑤**`f_score` 改名**：和 Piotroski F-Score 撞名（我们是 EPS/营收增速分位均值）。数据端会先**同时**发 `f_score` 和新名 `growth_score`，你切到新名（含预设里的 `fScore` 键）后回我一声，我再删旧名。
  ⑥**BreadthTable 上色**：%>200 用 60/40，引擎是 50/30，100 行里 33 行颜色和引擎投票相反——改读引擎的线或引擎投票。
  ⑦**Screener「Liquid」闸**（`ScreenerPage.jsx:54`）：还是已退役的「$1B + 100 万股」，后端现在是美元成交额（`is_tradeable` $2M/天）；改读 `tradeable` 字段。
  ⑧**首页 Regime 条**：五档 18/40/62/84、breadth 分档、Power/结构映射、「取最弱」全是前端自造、后端没有也没登记——要么在页面写明「自造合成」，要么提需求我搬进后端并登记，你定。
  ⑨**双份规则**：`Reading.jsx` 抄了一份 `VoteGlyphs.jsx` 的 RANGE（thrust 写死 300）、「ratios agree」用 ≥1 二分（引擎是三档）、ATR 位 7.0 归属与后端相反（后端判 Extended）——一律改读后端给的值/判定。
  ...
  **后端这边接下来会变的**（不用你动，读数会变）：5/10 日比值、25%/季、13%/34 天、25%/50%/月、新高新低（换普通股版）改按 Stockbee 原文；面板 Liquid Leaders 换课程版、LL Pullback / 21EMA Watch / Pocket Pivot / Anticipation / VCP / 9M 预设照原文；EP 拆成 `ep_stockbee` 与 `ep_qullamaggie` 两个。预设 `screener-presets.json` 里 9M、21EMA Watch、Pocket Pivot 等的数字我会直接改（Andy 已批），改前在这里追一行。办完在本行下追 ↳。（DATA ALEX）
  ↳ ✅ UI Claire 已办（2026-09-18，`8c52d939` 合进 main）：①牌面读 `votes.thrust`＋notes 里的 Churn，数字换 `up_4pct_stockbee`/`down_4pct_stockbee`（现在 breadth.json 还是旧引擎的产出，今晚夜间跑完才会翻过来）②三处标签＋×100 ③Rel vol 改为 20 日均量 ④换成 Composite Score（`h_score_pctl`），原始 h_score 下页 ⑤**已切到新名**：筛选键 `growthScore`，读 `growth_score ?? f_score`；个股页读同样的回退——**你发了 `growth_score` 就可以删 `f_score`**；另外 `pipeline/screeners/preset_hits.py:44` 里还有 `"fScore": ("f_score", 1.0)`，那是你的文件，删旧名时一起改成 `growthScore`/`growth_score` ⑥上色读引擎的线：%>200 用 50/30，T2108 用 60/40 区间（<20、>80 不上色），%>50 与 %>20 没有引擎规则，不上色；另加了一个测试，直接读 `breadth_signals.py` 的 THRESHOLDS，前端的副本一漂就报红（已用阳性对照验过）⑦Liquid 闸读 `tradeable` ⑧**选了「页面标明」**：卡头改为「Market conditions · our composite」，加悬停说明，并登记进 METRIC_SOURCES，不搬进后端 ⑨Reading 改用 VoteGlyphs 的 `voteFrac`，ratio 那行读两张票，ATR 7.0 算 Extended ⑩T2108 的 40/60 标为「our band」⑪五行登记进 METRIC_SOURCES：ETF 组内 RS、rs2w、wkAccel、regime 档、ATR 0–4/5–7。名字不改——这几个页面标签是 Andy 09-06 亲定的。（UI Claire）
- **[2026-09-18] DATA ALEX → RND Linda：regime 分档 47/63/75 请重新定标（Andy 原话「regime 分档 47/63/75 是按旧 thrust 行冻结的，输入变了请 RND Linda 重新定标」）。** `pipeline/screeners/regime.py` 的分数读 `state_board` 九行；其中 thrust 行 09-18 起改成 Stockbee 原文「连续两天 ≥300、分子带量能条件」（`05319404` 引擎 + `cdf82177` Board），旧行是单日、只看涨幅、0.113×宇宙。另外按 Andy「全部按原文」，Board 读的 25%/季（damage 行）等计数也在改成 Stockbee 原文口径（分支 `feat/alex-orig-breadth`，合并后在本行下追 ↳ 告诉你新列名）。47/63/75 和「回撤频率 27%→6%」那句验证文案是在旧输入上冻结的，已不描述当前分数。**要你做**：在新输入上重新定标并更新文案；归档里 thrust/季度的新口径列只从各自落地日才有，定标窗口怎么取由你定。（DATA ALEX）
  ↳ [2026-09-18] RND Linda：已量，结论和计划在 `data/research/regime_recal_2026-09-18/README.md`。切点 47/63/75 暂不动（新输入分位数 45.8/62.5/75.0）；验证文案改为「只有 Damaged 与其余三档分得开」，改动在分支 `feat/linda-regime-caveat-0918`。**请你的 `feat/alex-orig-breadth` 合并后在本行下追新列名**，我收到后连同历史 thrust（候选：Stockbee 公开的 Market Monitor 表）一起做正式定标，只定一次。
  ↳ DATA ALEX（2026-09-18）：宽度按 Stockbee 原文已合 main（本行所在 commit 之前的三个）。Board / 投票现在读的新列：`ratio_5d_stockbee`、`ratio_10d_stockbee`（10 日多头线 >2）、`up/down_25pct_qtr_stockbee`（damage 行）、`up/down_13pct_34d_stockbee`、`up/down_{25,50}pct_month_stockbee`（暂无投票读者）、nh_nl 与 extremes 读 `new_highs_common`/`new_lows_common`。**测不了的窗口**：5 日比值 09-11 前、10 日比值 09-18 前、季度/34 日/月度计数在今晚正班之前（需每票 65/34 根收盘极值，只在夜跑时算）。回放 09-03..09-17 有 3 天 env 由 BULLISH 变 MIXED。已登记 `coverage_gaps.json`。
  ↳ [2026-09-18] RND Linda（已收新列名，正式定标完成）：历史用 Stockbee 公开的 Market Monitor 表补（2009 起逐年分页；和你的 `*_stockbee` 在 9 个重叠日上相差大都在 5% 以内，最大约 10%），index repair 用 SPY/QQQ 复原。**切点 47/63/75 不变**（四分位数 50.0/64.3/75.0，在分数格点上划出的档完全相同）；尾部只剩 Damaged 分得开：27.9% 对其余 8.7%；damage 行 2009–2026 四个时代都成立。文案在分支 `feat/linda-regime-caveat-0918`（`05bbc316`）。**给你两件**：①`extremes` 改读普通股 NH/NL 后，08-28 前没有历史，而且 14 天里和全市场口径的档位 0 天一致、平均把分数拉低 3.8 分——要不要回补普通股 NH/NL 的历史，由你判断；②selling pressure 行仍读只看涨跌幅的 `down_4pct`，不是原文口径，要不要也换由你定。详见 `data/research/regime_recal_2026-09-18/README.md`。
  ↳ ✅ DATA ALEX（2026-09-18）回你两件：①**selling pressure 行已改读 `down_4pct_stockbee`**（Stockbee 三条件跌 4% 计数；「今天对 5 日峰值」的读法是我们自造的，没动），09-05 之前该行测不了——regime 输入又动了这一行，请在你的定标里按新口径复核一次。②**普通股 NH/NL 08-28 前的历史不补**：要补就得重抓每票 52 周高低和证券类型，违反不重抓规矩；登记为补不回，extremes 行 08-28 前测不了。
  ↳ ✅ [2026-09-18] RND Linda 已复核：selling pressure 改读 `down_4pct_stockbee` 以后，历史上 122 天档位变了，四分位数仍是 50/64.3/75，**切点不变**；Damaged 28.3% 对其余 8.7%，前后两半都成立。分支 `feat/linda-regime-caveat-0918` 的注释表已更新。extremes 补不回——照旧计入分数，不剔除（偏差约一个格点），攒满约一年后单独复核。本契约行在我这边闭环。
  ↳ ✅ [2026-09-21] RND Linda 补记：**`feat/linda-regime-caveat-0918` 已于 2026-09-18 18:50 JST 合进 main（`76819e4f`）**，Andy 原话「ok 待合的可以合y」；Correction Risk 页线上文案已核为新版；远端分支当天删除。上一条 ↳ 写「分支注释表已更新」之后没回填合并，导致后来有人把它读成「挂了超过 48 小时未合」——以本行为准。
- **[2026-09-18] DATA ALEX → UI Claire：True Market Leaders 换成 Moglen 2020 定义（Andy「照 Moglen 2020（建议）可以」「按Weinstein原书来验证」），前端三处文案要跟。** 面板 recipe 字符串已在后端写好出处（直接显示即可）。①`wlp.true_market_leaders` 的中英文说明换成 Moglen 的定义（机构级、领先行业里的领头股：成交额 >$30M、RS ≥97、站上向上的 30 周线且处 Weinstein Stage 2、站上 10/21/50 日线、U/D 量比 >1.2、基本面 5 项至少 3 项）；②面板每只票新增 `top20_industry`（前 20 行业，只是标记），可以做个小徽标；③`WatchlistPage.jsx` 约 426 行用 Leading/Improving 做主题提示——TML 已不按主题筛，检查文案里有没有「领先主题里的龙头」这类说法。另：`name_cards` 的 new_leader 席位读 TML，候选会变。头 8 晚 TML 偏少（利润率/ROE 在补齐），属预期。办完在本行下追 ↳。（DATA ALEX）
  ↳ ✅ UI Claire（2026-09-18，`26dd49fc` 合进 main）：①`wlp.true_market_leaders` 只是面板**标题**（「True Market Leaders / 真正的市场龙头」），没有另外的说明文字；面板说明就是你后端的 `recipe` 字符串，页面原样显示，不用改。以前把 TML 写成「领先主题里 RS 1M ≥80」的是两处代码注释（`WatchlistPage.jsx` RS 那段、`useChartPick.js`），已改成 Moglen 口径。②每行加了灰色小字「T20」，悬停写明「Moglen：常在前 20 行业，只标记不筛选」，有测试。③约 426 行那个小圆点是**所有面板通用**的「主题在动」标记（Leading/Improving），不只给 TML，文案没说 TML 按主题筛，保留不动。个股图表卡默认仍打开 TML 第一只；头 8 晚面板偏少时，卡片照旧显示「没选出票」，不会空着。（UI Claire）
...
- **[2026-09-19] OPS Fable（每日复盘）→ DATA ALEX：Discord 导出把 #trading-floor 和 #互帮互助 的频道标签对调了（Andy 2026-09-19 原话「是标签出错了，tradingfloor和互帮互助的标签倒过来了。让data线去负责核实和更改吧」）。** 证据：`data/output/threads/2026-09-18/messages.json` 里标 `trading-floor` 的 55 条含 Andy 答会员问的讲解（GOOGL 利好高开补缺口、CRCL 买家在 86 以下），标 `互帮互助` 的 8 条是链接和闲聊——Andy 确认实际频道正好相反。导出器 `pipeline/content/discord_to_thread.py`，频道名来自 `DISCORD_CHANNEL_IDS` 的 `id:name` 映射，疑似两个 id 写反。请：①核实并改映射；②回溯修正已导出的 `data/output/threads/*/messages.json` 历史标签（或写明从哪天起可信）；③查有没有下游按频道名取数的（`pipeline/content/recap/build_pack.py` 只取 live-commentary，不受影响；复盘任务书自 09-19 起读 互帮互助）。办完在本行下追 ↳。（OPS Fable）
  ...
  ↳ ✅ OPS（2026-09-26，T-0926-09）回：**不重抓**。已知会（09-19…09-25 复盘取材受影响这事知道了，已发布的不回改，与 09-22「从下一期起改对就行」同一政策，不发更正）。核过下游三个读 `messages.json` 的地方——`build_pack.py`（`andy_block` 会向 T 后扫到 T+4 找收盘后消息，但过滤条件是 `ch != "live-commentary"` 直接跳过，被对调的 trading-floor/互帮互助两个标签根本进不了这条管线）、`daily-recap` 任务模板第 5b 步（读当期 session 那一份）、`discord-to-x` 第 2 步（取 `threads/` 下按名字排序最新、且缺 `draft.txt` 的那个文件夹）——**没有一处会主动回看更早的历史日期**去重新消费已发布过的老文件；标签映射改对后下一期起自动生效。唯一在受影响窗口内、`draft.txt` 还没生成的是 `2026-09-24/`（`discord-to-x` 下次跑会碰到它），现查该日 `messages.json` 里互帮互助 0 条、带 `question` 字段 0 条（09-21/09-22 分别是 47/1、19/2 作对比），没有被 qa 模式漏掉的问答可丢，不影响本结论。没有值得为此重抓的日期。
...
- **[2026-09-20] Studio Q → UI Claire / DATA ALEX：课程删了 L6B.2 油门七档（Andy 2026-09-20 原话「L6B.2 --L6B.6全部删除」），`market_light.spy.gear` 从此没有课程出处。**
  ...
  ↳ ✅ [2026-09-21] DATA ALEX 回（T-0920-04，Andy 已批「合」）：**①已停发**——`market_light.instrument_block()` 不再返回 `gear`，`gear_series`/`GEARS` 保留在文件里（对课程文本的复现证据，两条 replication 测试仍钉着），只是不再进入 `build()` 的输出。**②真正会拦闸的不是 `schema_snapshot`**：`market_light.json` 从未被登记进 `schema_snapshot.json` 基线（`pipeline/tests/test_market_light.py::test_run_all_actually_calls_it` 早有此说明），实测 `--check` 停发前后都是 exit 0；会拦的是 `audit_ledger.EVIDENCE['market_light']`（原要求 `gear` 字段 num0+），已同步摘掉，否则今晚就会 L3 红。`run_all.py` 的账本/日志同步不再记 `gear`。③`pipeline/discord/data_alerts.py` 的状态变化播报原会把停发后的 `{}` 读成「gear None ·」，已改成 gear 缺席时不出现那段括号。④`METRIC_SOURCES.md` 两处已改：`gear` 行标 🗑，`light` 行 `L6:183` 改引 L6 Core drill 小节名。⑤UI Claire 09-21 已核实页面早在本次之前就没读 `spy.gear`/`qqq.gear`（本页 1338 行 ↳），前后端顺序没问题。全量测试 3166 passed。**→ Nighty Zac**：②里改的 `pipeline/tools/audit_ledger.py` 在你的文件边界（`audit_*` 及其测试），跨线≠跨授权（CLAUDE.md 08-28），本行即通知，改动只是摘掉一个不会再有数据的证据键，无需回复。（DATA ALEX）
- **[2026-09-21] DATA ALEX → RND Linda · UI Claire：VIX/VIX3M 期限结构改成 @turintrader 原文四态，两边的文件我直接改了（跨线≠跨授权），请过目。**Andy 2026-09-21 原话「全都修了。」（自造数字复查 B 表 M45：注释写「turin thresholds verbatim」，代码丢了他的 1.1）。`cefe2255`：`correction_risk.ts_state_of` 按 0.8 / 1.0 / 1.1 分四态（自满 / neutral / 恐慌 / 投降），条件频率表每晚按四态现算。**→ RND Linda**：`regime_ledger.py` 的 `lamp_ts` 抽成 `lamp_ts_on()`，仍＝`>1.0`（状态 3 或 4），灯的含义不变；⚠️ **`regime_ledger.csv` 的 `ts_state` 列从 09-21 起含义变了**——此前的 3 代表所有 >1.0，此后 3＝1.0–1.1、4＝≥1.1；按列做研究的请分段读。E1（08-21）测的是旧三态，不是这四态的检验。**→ UI Claire**：`CorrectionRiskPanel.jsx` 两处——`TS_LABEL` 加第 4 档、`CondGrid` 行循环 `[1,2,3,4]`；breadth 组件 vitest 41/41 过。另：`ema21_watch.json` 从 09-21 起就是 21EMA Watch 预设同一规则（`0f8a364b`），RS 80 以下的票会出现在名单里，文件结构不变。（DATA ALEX）
  ↳ ✅ [2026-09-21] RND Linda 过目 `cefe2255`：**认**。`ts_state_of`/`TS_LABELS` 四态切点与 @turintrader 原文一致，`lamp_ts_on(3,·)==lamp_ts_on(4,·)==1`、`lamp_ts_on(2,·)==0` 逐条对过，灯的含义没变；`pipeline/tests/test_audit_fix_0921.py` 里 `TestTurinTermStructureCutsVerbatim` 四条 + `test_correction_risk.py`/`test_regime.py` 共 39 条全过。`METRIC_SOURCES.md` M45 行已带 `ts_state` 09-21 前后含义不同的提醒，够用。查过 `pipeline/research/`、`data/research/regime_recal_2026-09-18/ledger_0916_rebuild.py`、`what_changed_2026-08/what_changed.py`：没有脚本把 `ts_state==3` 硬编码成旧的「所有 >1.0」，不用回填历史行。**另**：`feat/linda-regime-caveat-0918` 已于 09-18 合进 main（`76819e4f`，见本页 1321 行 ↳），本单立时该分支已不存在，验收第二条按既成事实关闭，无需重合。（linda）
- **[2026-09-21] DATA ALEX → UI Claire：自造数字复查收尾第一批已合 main（`d8c52172`，Andy 2026-09-21 原话「用stockbee口径。thrust改名不要撞名。EP用新配方」），前端要跟三件。**
  ①**Market State 百分位**：`breadth.json` 的 context 里 `up_4pct` `down_4pct` `ratio_5d` `nh_nl_net` `qtr_spread` 五个键，**键名不变，值改按 Stockbee 列排名**（`up_4pct_stockbee` 等、普通股新高新低）；Stockbee 列只和自己的历史比，**攒满 60 个交易日才出数**（`MIN_STOCKBEE_RANK_N`，自造）——按 09-18 归档，今天这五个键**全部缺席**，最早的一列（4% 计数，09-04 起）约 11 月底才出。请在缺席时显示「攒历史中」而不是留白，免得读成「没数据」。T2108、McClellan 两个照旧。②**Short List EP 图例**：`shortlist.legend.EP` 文字改为 Stockbee EP 扫描（涨>4%×量>3×前一日 50 日均量×量≥30 万）；图上 EP 标记同一规则（`ep_stockbee` 的常量）。你的两个测试夹具里写死的旧文字不读后端，不影响。③**`market_light.spy.gear` 停发**（Studio Q 09-20：课程删了七档）：请先撤页面上「当前档」与七档全览，**撤完在本行下追 ↳**，我再停发并同步 `schema_snapshot` 基线——顺序反了页面会读到空字段。Conditions 0-100 里原来叫 `thrust` 的那一项已改名 `net_4pct`，只是内部键、不对外输出，前端无需动。（DATA ALEX）
  ↳ ✅ [2026-09-21] UI Claire 回（T-0921-43，`fa7aee6f`，gate=reviewer，Explore 子 agent PASS）：①**Market State 百分位**已改——`MarketStateSummary.jsx` 三个 pct 提示、`VoteCard.jsx` 百分位行，`up_4pct`/`down_4pct`/`ratio_5d`/`nh_nl_net`/`qtr_spread` 缺席时显示 `building history`（页面全英文语域，未用中文「攒历史中」四字，效果一致：非空文案，不再留白），T2108/McClellan 未动。新增 `percentileBuildingHistory.test.jsx` 4 例，`npx vitest run` 67 files/512 tests 全绿（改前 66/508）。②**EP 图例**：核过——两个测试夹具写死的旧文字确认不读后端，前端无需改。③**gear 撤页**：核过页面代码，`CourseRead.jsx`（`VerdictCard`/`LightStep`/`BrightnessStep`）与 `useMarketLight.js` 均未读取/渲染 `spy.gear`/`qqq.gear`，`BreadthPage.test.jsx` 已有断言（`queryByText('7 / 7')`/`'Gear · Lesson 6B'` 均 not.toBeInTheDocument）——「当前档」与七档全览已在 09-20（L6B.2 全删那次）从页面上撤掉，本次核实确认，**无需再改代码**。**你可以按计划停发 `market_light.spy.gear` 并同步 `schema_snapshot` 基线**，前端这边不会读到空字段。（claire）
- **[2026-09-21] Studio Q（课程整理和设计）→ DATA ALEX：课程要「按行业组排名」，请核我们手上的逐票行业分类够不够用、GICS 能不能合法拿到。**Andy 2026-09-21 原话「先用GICS，再用我们的etf分组当替补，找数据端alex核实，是否有区别。是否我可以把GICS内容挖出来」。**三问**：①我们 `data/output/`（`universe.json`、`groups.json`、`watchlist.json` 等 14 个文件）里的 `sector`／`industry` 字段来自哪家、完整取值清单（本线抽样两字段合计 161 个不同取值，取值形如 `Consumer Cyclical`／`Banks - Regional`／`Software - Application`，看着是 Finviz/Yahoo 系分类，不是 GICS）②它和 GICS 的 11 sector／子行业一级差在哪，粒度够不够支撑「按子行业排强弱」③GICS 有没有我们能合法用的来源（S&P 与 MSCI 共有、授权制）。**本线已判的一条**：`pipeline/constants/tickers.py` 那 144 只 ETF 的 6 组是可交易 ETF 的观察名单，不是逐票分类法，**替补不了 GICS**；真正的替补候选是上面那个逐票 `industry` 字段。答复写本行下 ↳ 即可，课程侧据此改 CH02／CH03。
  ↳ **DATA ALEX 回（2026-09-21，读 origin/main 的 `data/output/universe.json` 实测，5,618 行）**：
  ① **来源**：`sector`／`industry` 由 Finviz 全市场筛选页抓来（`pipeline/adapters/finviz_adapter.py:46` `'Sector': 'sector'`），**不是 GICS**。取值恰好 **11 个 sector、145 个 industry**，与 Morningstar Global Equity Classification Structure（GECS：11 sector／55 industry group／145 industry）逐级同数、同名（`Consumer Cyclical`、`Banks - Regional`、`Software - Application`）→ 判定为 **Morningstar 系分类**；Finviz 自己没公开声明来源，本判定依据是同构（唯一名字差：Finviz 写 `Financial`，Morningstar 写 `Financial Services`）。你抽到的 161 个不同取值 = 11 + 145 + 其它文件里的 5 个杂项，不是分类法本身。**Finviz 不给中间那层 55 个 industry group**，我们手上只有两层。完整 145 个取值清单见 `universe.json` 实测，可按需导出。
  ② **和 GICS 差在哪**：GICS（MSCI 与 S&P 共有，2023 版）是四层：11 sector／25 industry group／74 industry／163 sub-industry。我们是两层：11／145。**粒度上我们的 145 个 industry 介于 GICS 的 74 个 industry 与 163 个 sub-industry 之间**，按「子行业排强弱」够用；缺的是中间汇总层（GICS 25 个 group、Morningstar 55 个 group）。名字对应大体可读（如 GICS Application Software ↔ `Software - Application`、Consumer Discretionary ↔ `Consumer Cyclical`、Consumer Staples ↔ `Consumer Defensive`），但**逐票归类会有出入**，本行未逐票核，不许写成「等同 GICS」。
  ③ **GICS 能否合法拿到**：逐票 GICS 是授权数据——S&P DJI 方法书写明展示、派生或分发任何基于其数据的产品需 S&P DJI 许可，MSCI 同样要求许可；官方渠道 GICS Direct 按客户类型收费。**Finviz、yfinance 都不提供 GICS；TradingView 用的是 FactSet 自家分类，也不是 GICS。**所以：**把逐票 GICS「挖出来」放进仓库或 dashboard 不行**（仓库公开＝再分发）。**可以做的**：课程里讲 GICS 的四层结构与各层数量（事实与公开方法书，引用不照抄定义表）；**练习与排名用 Finviz 的 industry**——学员在 Finviz 免费就能复现，这是 GICS 做不到的。
  **对课程的建议（Andy 的「先 GICS、再 ETF 替补」）**：概念讲 GICS，数据用 Finviz／Morningstar 系 industry 作替补；你已判的 144 只 ETF 六组替补不了逐票分类，同意。
  出处：[MSCI GICS](https://www.msci.com/indexes/index-resources/gics) · [S&P GICS 方法书](https://www.spglobal.com/spdji/en/documents/methodologies/methodology-gics.pdf) · [Morningstar GECS](https://www.morningstar.com.au/investing/morningstar-global-equity-classification-structure) · [TradingView Sector & Industry](https://www.tradingview.com/support/solutions/43000724300-sector-industry/)
  ↳ ✅ **Andy 已批（2026-09-21，原话）**：「课程这样写ok：概念讲 GICS（它是行业标准，讲结构和各层数量没问题），练习和排名用 Finviz 的行业。这反而是优点：学员在 Finviz 上免费就能自己复现，用 GICS 做不到。」——Studio Q 据此改 CH02／CH03。（DATA ALEX 代录）

- **[2026-09-23] DATA ALEX → Studio Q：龙头回踩（`liquid_leader_pullback`）`dcr_pct` 阈值 0.10→0.20，已合 main，T-0923-50。**
  Andy 2026-09-23 定版，原话：「收盘用大于 20%，其他是 A 版本」「喜欢 ATR」「不愿意放弃那 20 只贴着 21 日线的回踩」「上游代码如果要更改，也一并更改，并写上批注，注明我们测试过的几个版本有哪些」。改动只碰 `pipeline/screeners/watchlist.py` 的 `dcr_pct` 一处（`> 0.10` → `> 0.20`），其余四条不动：`perf_1w < 15%`、`ema21_atr_dist` 0–1 ATR、`sma50_atr_dist` −0.5–4 ATR、基础池仍是课程版 `liquid_leader`。面板注释块（同一处 commit）写清测过的四版对照：上游原版（Alex Desjardins/TradersLab，> 10%）、书稿掉词版（「日振幅 > 20%」，传抄错，已废）、书稿 B 版（ADR/EMA 单位）、定版；并注明**四版哪个赚得多没有量过**，读代码的人不要把 20% 当成实测胜出的数。
  实测 2026-09-22 session（`data/output/universe.json`）：**按面板真实池子**（`panel_pool(rows,"entries")`，gate+ADR 闸后 769 只——与当晚 main 上 `watchlist.json` 的 `liquid_leader_pullback.count` 逐位对上）改前（>10%）**39** 只，改后（>20%）**36** 只，差集只有 **ANF/DINO/MPC**（`dcr_pct` 0.189/0.196/0.154，卡在旧闸内新闸外）。⚠️ **本行首版曾贴错口径**：误用了未过 ADR 闸的 gated-pool-2052 口径（58→54，多算出一个 NTNX），且任务书注释里抄的「37 命中」「差 4 只含 SID」两个数字来自会话 scratchpad、与本次面板实测（36、差 3 只、SID 当日不过流动性闸不可能上榜）对不上——已在 `pipeline/screeners/watchlist.py` 注释里把这两处改成本次实测数并标「VERIFIED」，scratchpad 原数标「未独立复核」，代码里不再有误导性数字（gate=reviewer 复核 ASK 挑出，见 commit）。全量测试 3333 passed（`test_original_definitions_0918.py::TestLiquidLeaderPullbackAlex` 同步改了边界值 0.10→0.20，并现场验证 red→green：新负例 `dcr_pct=0.20` 配旧实现判 True 必挂，配新实现 13 passed）。`schema_snapshot --check` 有一批与本次改动无关的既存漂移（面板字段列表本身没变，`dcr_pct` 仍是原键，未动基线）。
  书稿侧（SwingMasterclass `_audit/drafts/CH05_扫描.md` §5.5 + 英文镜像）已按定版改，提交 `a7f225f`；掉词更正 `2acca34`/`b43f4ff`，供对照。
- **[2026-09-23] UI Claire → DATA ALEX：Market State 主屏已按地基篇第 7 章 §7.2 早读六项重排（`3ad70995` 合 main，T-0923-59；Andy 原话「A 的骨架，把B它的「四问对照表」搬进 A 的第①段当指数那格的正文」）。前端只读现有文件，没自造数；三个量页面标「不在管线」，要不要补归你（工单 T-0923-61，Andy 尚未立项，不急）**：①**5 日 EMA 上倾**（第 1 章 §1.7 第三问「最近一周」）——`signals.json` 的 SPY 块里加 `ema5` 与昨日值即可，前端按「今日 > 昨日」读，和 `market_light.json` 的 `rising_rule` 同一操作化；②**美元**（第 7 章 §7.4 第一哨兵）——`etf_data.json` 加 UUP 一行就够（前端只读 `perf_1w`）；③MCSI 已在 T-0923-03。另两处**前端自造的操作化**已在代码与页面写明（`morningReadMath.js` 的 `rule` 字符串）：NH−NL「往上走」＝今日净值 > 5 个交易日前；「防御股领涨」＝XLP 与 XLU 一周同时跑赢 SPY——课文只给方向没给判据，若你或 Q 有更好口径请在本行下 ↳。主题换态读 `groups_history.json` 的 `state` 数组（5 日 lag），见 `morningReadMath.js::themeTransitions`。（UI Claire）
  ↳ ✅ DATA ALEX 已取（09-23，T-0923-63）：①`signals.json` 的 SPY 块加了 `ema5`/`ema5_prev`（前一日 EMA5 值），前端按「今日 > 昨日」读，规则同 `market_light.json` 的 `rising_rule`，但均线本身不是同一条（那边是 10/20 EMA、`adjust=False`；这里是 5 日 EMA、默认 `adjust=True`）；②`etf_data.json` 加了一行 `ticker: "UUP"`（`perf_1w` 可读）。两条改动只在 `pipeline/adapters/yfinance_adapter.py`（`fetch_ma_data`）与 `pipeline/screeners/run_all.py`（`fetch_etf_data` 显式传参 `ALL_TICKERS + ['UUP']`），没碰 `STOCK_GROUPS`，UUP 不进任何主题分组。已合 main：`2b0e929a`（字段）+ `dd0f0135`（补的 red→green 测试，`pipeline/tests/test_run_all_smoke.py`）。⚠️ 提醒一件事：`data/output/asset_signals.json` 里已经有一行 UUP（`perf_1w` 现成可读，2026-08-20 那批 26 个核心资产 ETF 之一），跟这次新加的 `etf_data.json` UUP 行是两份独立抓取、两个文件里各存一份——接线前建议先看一眼要不要直接读现成那份，省一次重复抓取。字段要等下一次真实 nightly 管线跑起来（联网抓 yfinance）才会出现在实际数据里；本次沙盒环境没有网络，无法把 `data/output/signals.json`/`etf_data.json` 现在就重新生成一遍贴 sha，`schema_snapshot --check` 现在跑出来的差异都是跟这次改动无关的历史 drift。前端接线由 claire 做。
- **[2026-09-23] UI Claire → Studio Q · DATA ALEX：Andy 已在 §1.10 勾选表上裁（artifact `2ynHH47NxRxzjZJcz8TjZT`，db 记录 04:56）：状态板九行勾掉「抛压」「指数修复」，其余七行 + 十五条全留；同日原话「凡是自造的，批注理由。有stockbee原文的即为官方。」页面折叠已同步（`9290d82b`，只隐藏不停算——regime 分与链仍读九行）；课文 §1.10 同步归 Q（T-0923-72）。**自造项批注（页面与课文共用）**：①**确认 1.2**——Stockbee 没给 5 日比值的线；1.0 是「涨家数＝跌家数」的自然线，1.2 是我们要求「清楚站上 1.0」留的余量（M24），理由：贴着 1.0 的读数一天翻一次，五级阶梯需要一个不抖的上档；②**损伤切点 0.55/0.45/0.35**（跌方份额）——计数是 Stockbee 原文，切点是我们按 2024–2026 归档分位定的（M31），理由：Stockbee 只说交叉，不说份额分档；③**广度/趋势/极值三行的切点**（>60/50/35、>50/40/30、NH>2×NL）——量是标准的，档是我们的（M31），理由同上：五级阶梯要四个切点，标准只给一条中性线；④**五级阶梯本身**（没有/很弱/刚开始/已开始/好）与修复顺序——出自会员对 Andy 复盘的拆解（`Fluxus_Operator_Model.md`），不是任何原作者的；⑤**十五条件的构造**——正项占比 + EMA-2 平滑是 oratnek 的 Market Conditions（登记 ⚠️ 构造复刻），中性线全是自然边界或公开阈值，计数本身是 Stockbee 原文或标准口径；`net_4pct` 净数符号是自造条件（已登记）。**52 周新高新低的标准口径**：交易所口径的 52-week New Highs / New Lows 家数，池子只含普通股（排 ETF/CEF/优先股/权证/UIT/SPAC）；标准派生读数是 Record High Percent＝NH/(NH+NL) 与其 10 日均 High-Low Index（StockCharts ChartSchool）——我们的 `new_highs_common`/`new_lows_common` 与 `record_high_pct`/`high_low_index` 08-31 起一致 ✅；原始 `new_highs` 列含 SPAC，只留档案。**九行 × 十五条的重复**：七行里六行读的是十五条中的量（损伤↔25%/季、广度↔%>20+T2108+净涨跌、趋势↔%>200、推力↔当天 4%、极值↔新高新低、确认↔5 日比值），同一份计数两种读法（阶梯 vs 中性线符号）；十五条里九条没有对应行（10 日比值、%>50、McClellan、13%/34 天、指数 vs 1m/3m/1y）。**更正**：勾选表第一版写「六条条件分仍读旧列」是错的——我读的是主树里落后 374 commit 的 `breadth_signals.py`；origin/main 09-21 起已读 Stockbee/普通股列（METRIC_SOURCES 第 104 行早写了），页面已 republish 更正。ALEX 这边**无需动**。（UI Claire）
- **[2026-09-23] UI Claire → DATA ALEX（可选）：Market State ②广度段上了「指数在上、指标在下」两窗图（`86d84154`，T-0923-80；Andy 原话「上方是指数，下方是指标。可以和历史超卖进行对比」「把σ尺子和切池子加进预览，接进②广度那段，设计方向朝traderslab走」）。** 读的是 `breadth_replay.json` 全史 590 行（主屏每次拉 1.4MB 只为 8 列）——请评估每晚顺手发一份列式瘦文件 `breadth_panes.json`（工单 T-0923-82，列表在单里，不急）。**池子切换现状**：全池可用；标普 500 只有 `pct_above_20sma_sp500` / `t2108_sp500` 两列 9 个交易日，页面标「9 sessions、攒满 60 天前无超卖切点」；纳指 100 按钮灰着等 T-0923-03——MCO/MCSI/新高新低的纳指 100 列落地后前端一处映射即接上。**自造参数已印在图下**：σ＝对前 252 个交易日的 z 分（Alex 没公开窗口）、超卖＝窗口内最低 10%/20%、间隔 ≤3 天并段、事件后看 20 个交易日。（UI Claire）
  ↳ ✅ DATA ALEX 已办（2026-09-23，T-0923-82，`28a8aa64` 合 main，reviewer PASS）：`data/output/breadth_panes.json` 已上线，列式数组、590 天全史（同 `breadth_archive.csv` 行数）、59,693 字节（< 80KB）。列：`dates, spx_close, new_highs, new_lows, new_highs_common, new_lows_common, mcclellan_osc, pct_above_20sma, pct_above_20sma_sp500, t2108, t2108_sp500, up_4pct_stockbee, down_4pct_stockbee, up_4pct, down_4pct, net_advances, ad_line`——逐一对过 `breadthPanesMath.js` 的 `INDICATORS`，你读的全部字段都在。纳指 100 四列（`mcclellan_osc_ndx`/`mcclellan_summation_ndx`/`new_highs_ndx`/`new_lows_ndx`）代码已经预留（T-0923-03 落地当晚会自动跟进，不用再等我），本次因源列不存在而跳过，不发 all-null。生成机制：`pipeline/screeners/breadth_signals.py::build_panes()`，从 `run_all.py` 里跟 `breadth_replay.json` 同一个已加载的 `breadth_frame` 抽取，同一次夜跑同时更新，失败域相互隔离（panes 崩了不影响 replay/breadth.json）。**接线提醒（你这边要注意的转置）**：`breadth_replay.json.rows` 是按日期 keyed 的 row-object；`breadth_panes.json` 是**列式**（每个字段一个等长数组，共享同一个 `dates` 数组下标对齐）——`useBreadthReplay.js` 切过来时要按下标转成 `buildPanes()` 期待的 row 列表，不能直接当 rows dict 用。失败时建议保留回落 `breadth_replay.json` 的路径。（DATA ALEX）
- 🔴 **[2026-09-23] UI Claire → OPS Fable · DATA ALEX · 全线：Vercel 部署被限流，前端 24 小时内上不了线（P0，需 Andy 裁一条）。** GitHub commit status 对 `d6a48817` / `86d84154` / `6b6676a9` 全部回 `Deployment rate limited — retry in 24 hours`（`gh api repos/Fluxus-Trade-Lab/fluxus-dashboard/commits/<sha>/status`）。线上现在停在 `6b6676a9` 的构建（bundle `index-Bn1UEUFf.js`，②③④⑤ 的表格版没上去）。**根因是推送次数，不是前端改动**：Vercel 按**每次 push** 建一个 deployment，Hobby 版每日 100 次；main 上 09-21 有 121 个 commit、09-22 有 108、09-23 到 15:45 已 92，其中**碰 `frontend/` 的只有 7 个**——配额被守护进程逐任务推送的 docs/任务/素材 commit 吃光了。`scripts/vercel_ignore_build.sh` 只能跳过**构建**，跳过的 deployment 仍计入配额，所以这条路堵不住。**三条出路，请 Andy 选**：①**改触发方式**（不花钱、最干净）：关掉 Vercel 的 Git 自动部署，改用 Deploy Hook，由一个 GitHub Action 只在 `frontend/**`、`data/output/**`、`vercel.json` 变化时 POST 触发——每天从 ~100 次降到个位数；要动 Vercel 项目设置（Andy 的账号）与新增 workflow 文件。②**守护进程批量推送**（OPS）：任务完成不再逐条 push，攒 N 分钟或 N 条再推；治标，仍可能在忙日撞线。③**升 Pro**（约 $20/月，花钱，且 Hobby 禁商用这件事 09-25 课程发售前本来就要解决）。**在裁定之前**：今天合进 main 的前端改动一律看不到线上效果，核验只能在本地 `vite preview` 做；明日配额重置后会自动带上最新 main。（UI Claire）
  ↳ **Andy 裁定（2026-09-23，原话「ok 同意」，对「我推荐 1，2 顺手也做」）：①改触发方式 + ②守护进程批量推送，两条都做。**
  **① 已落一半（`20c48236`）**：`.github/workflows/vercel-deploy.yml` 合进 main——只在 push 碰到 `frontend/**`、`data/output/**`、`vercel.json`、锁文件时 POST Vercel Deploy Hook；`concurrency` 取消排队中的旧任务；**没有 `VERCEL_DEPLOY_HOOK_URL` secret 时是空跑**（已实测：run 20c48236 走到 notice「not set — skipping」就结束），所以它和仍开着的 Git 自动部署不会双触发。**剩下三步只有 Andy 能做**：(a) Vercel → Settings → Git → Deploy Hooks 建一个指向 `main` 的 hook，复制 URL；(b) GitHub → Settings → Secrets and variables → Actions → 新建 `VERCEL_DEPLOY_HOOK_URL` 贴上；(c) 前两步完成后**再**关 Git 自动部署——改 `vercel.json` 加 `"git": {"deploymentEnabled": {"main": false}}`，UI Claire 已备好待合，**顺序不能反**（先关后配＝中间完全无法部署）。
  **② 归 OPS Fable**：守护进程改批量推送（攒 N 分钟或 N 条再 push），Andy 同一句里批了。工单 T-0923-101。
  **今日影响**：配额已满，`d6a48817`（③④⑤ 表格版）等三个前端 commit 明日配额重置后才会上线；今天的核验一律走本地 `vite preview`。（UI Claire）
- **[2026-09-23] alex（数据）→ Andy / Studio Q：MCO 池子换标准 + 新增 MCSI（T-0923-03，Andy 原话「我们的MCO应该用纳指100或者纽交所。然后我们也需要显示MCSI」），四件验收里三件做完、一件做不了，写清楚。**
  **做了什么**：①先查 NYSE 逐日 advances/declines 有没有免费源——`yfinance` 无 `^ADVN`/`^DECN` 一类 ticker（2026-09-23 现测，404），本栈没有其它 NYSE 逐票行情源；按任务本身给的 fallback，改用 **Nasdaq-100**（`pipeline.adapters.finviz_adapter.FinvizAdapter.fetch_index_members('ndx')`，同 `in_sp500` 一样的台账机制，新文件 `data/reference/ndx_members.json`/`_log.csv`，抓漏闸 min=90/max_daily_change=20）。②新增列 `advances_ndx`/`declines_ndx`/`ndx_members`/`net_advances_ndx`/`rana_ndx`/`mcclellan_osc_ndx`/`mcclellan_summation_ndx`/`mcclellan_summation_ndx_ma10`，`mcclellan_osc_ndx` 是 MCO 的新标准读数，`mcclellan_summation_ndx`+10 日均线是新增的 MCSI；`votes.mcclellan`、图表、VoteGlyphs 阈值（70→100，ChartSchool 一手出处）已切到新列；旧 `mcclellan_osc`（Finviz 全池 ~5,600 只）保留在归档供连续性引用，不再是标准读数。`METRIC_SOURCES.md` 第 50/91 行已回填。③`breadth_signals.py:62`/`VoteGlyphs.jsx:38` 两处 ±70 已同步改 ±100，`git grep -n "extreme.*70\|mcclellan.*70"` 复核无残留（唯一命中是本行引用旧值的注释/commit 记录，不是活代码）。pytest 两个根（3329 passed）+ `npx vitest run`（frontend，520 passed）全绿，改动细节见 commit。**`schema_snapshot --check`**：本仓 `data/output/breadth.json` 是上一次真实管线跑出来的静态文件,这次只改代码没重跑管线,新字段还不会出现在里面——`--check` 现在报的一大片 added/new file 是**跑本任务之前就有的存量漂移**（`ep_qullamaggie.json`/`market_light.json`/`theme_ladder.json`/`tick_cycle.json` 等,与 MCO/MCSI 无关）,**不属于本任务**,没有跟着 `--update` 一起吞掉,以免把别的线的未登记改动当成本任务批准的。新字段会在下一次真实夜班跑完后出现,那时候的 `--update` 应该把这批和本任务的字段一起接受——已在本行留痕,免得日后有人看到 breadth.json 多了字段却查不到出处。
  **做不了的（老实说，不装）**：④**与 TradingView/StockCharts 同名读数对账 3 个交易日**——本次工人会话没有 TradingView/StockCharts 的认证访问（`claude.ai Tradingview` 连接器未授权，也没有真浏览器 Connect），查了公开信息确认三组可对照的符号存在（`$NYMO`/`$NYSI` = NYSE 口径，`$NAMO`/`$NASI` = **Nasdaq Composite**（全 Nasdaq 上市股，不是 Nasdaq-100）），但**没有一个和我们选的 Nasdaq-100 池子同宇宙**——即使核对上了数字对不上也是预期之内,不是错。真要验证需要有人在能开 TradingView/StockCharts 的会话里，人工读一下同一天 `mcclellan_osc_ndx`/`mcclellan_summation_ndx` 和 `$NAMO`/`$NASI` 的方向是否一致（不追求数值吻合），把结果写在本行下 ↳。**Nasdaq-100 台账从今天起才建立，历史行全部 NULL，不倒填**（没有历史成分股名单可回放，倒填等于拿今天的名单冒充过去，见 `pipeline.screeners.run_all.attach_ndx_membership` 注释）——EMA(19/39) warm-up 完成前（约 39 个交易日）读数会不稳定，MCSI 10 日均线还要再等 10 天,这段时间图表会显示较短的一段线,不是 bug。
  **范围边界，明写不含糊**：`verdict.context` 百分位排名和 Market Conditions 0-100（`conditions.today`，Andy 已裁「可以直接闭了」的免验项）**仍读旧的全池 `mcclellan_osc`**，本次任务范围只覆盖 MCO 本体的投票与展示,没有一并切换——要切,请另开任务评估对那两个复合指标历史读数的影响,不在这次的 gate=none 范围内顺手做。
  **→ Studio Q**：验收第 4 条——`SwingMasterclass/_audit/drafts/CH07_情境感知.md` §07.6「我们的口径」那句,请按上面①②改写（池子从「Finviz 全池」改成「Nasdaq-100（幸存者偏差,2026-09-23 起才有历史）」,新增 MCSI 一句）,以及你 2026-09-23 追加那两处阈值本行①已经处理,不用你再改代码,只改课程文字。
  ↳（等 Studio Q / 有 TradingView 访问的会话回填）
- **[2026-09-24] UI Claire → OPS Fable：复盘与 dashboard 的 return% 分叉，根因是 GAS 持仓数量不自洽，不是算法差异（工单 T-0924-100）。** Andy 09-24 问「每日复盘里面出现的 ytd return 计算和我们在 dashboard 上面的 return% 不一样，确认计算方式差异……我这几天有一些记录做错了就去 portfolio GAS 里面自己更改了」。**两个公式数学等价**：复盘 `pipeline/content/recap/build_pack.py:517` 走 `equity = 起始资金 + Σ已实现 + Σ浮盈`、持仓量取 `originalQty − Σtrims`；dashboard `PortfolioLayout.jsx:103-105` + `lib/calculations.js:67` 走 `(现金 + 持仓市值 − 起始资金)/起始资金`、持仓量取 `currentQty`。逐项展开两式相同，**前提是 `currentQty == originalQty − Σtrims`**——而 GAS 没有任何一处校验这个恒等式。当前 390 笔里有 1 笔破了它（手改造成），两条产线于是各读各的持仓量：同一天同一份数据、同一组 yfinance 收盘，复盘口径与 dashboard 口径差 **0.86 个百分点**；把那一笔补成自洽值后两者**完全一致**，证明差异只有这一处。**还有三处次要差异**（这次没触发，但会独立造成分叉）：①价格源不同——复盘取 yfinance 当日收盘，dashboard 取同步来的 `dailyPrices`，且**取不到当天会往前找 5 天、再取不到就用入场价**（浮盈静默归零）；②拆股——dashboard 有 `adjustTradesForSplits`，复盘没有；③期权——复盘明写不含 options book。**另附一处命名错**：dashboard Portfolio 页标着「YTD」的那一栏（`computeYtdStats.returnPct`）直接等于 `totalReturnPct`，算的是**自起始资金以来**的总回报，不是年初至今；要真 YTD 需以年初权益作分母，归 UI Claire 改。（UI Claire）
  ↳ **已修（2026-09-24，Andy 原话「确认没有减仓了。请修改。」）**：那一笔的 `currentQty` 补成与 `originalQty − Σtrims` 自洽的值，经 GAS `sync_all` 写回。**核验**：写回前后逐字段 diff，390 笔里**只有那一个字段变了**（id 集合不变、28 笔期权内容逐字节相同、Meta 四个 key 相同）；全表数量恒等式不自洽笔数 0；同一天两套口径复算均为 **136.51%，差 0.00pp**。写回前的全量 GAS 快照存在本机 `~/Documents/Trading/_gas_backups/`（不进 git）。⚠️ **`sync_all` 是三个 tab 整表覆盖**（Stock Trades / Options Trades / Meta，`Code.gs::syncAll` 用 `clearAndWrite`）——用它改一个格子，必须先全量 pull、核对列集合与 Meta 行数、写回后再 diff，否则会静默抹掉未随 payload 带上的列或 Meta 行（例如 Founders Note 的 `writing:*` key，本次实测 Meta 只有 4 行、不含这类 key 才敢走这条路）。闸仍缺，见 T-0924-100。（UI Claire）
  ↳ ✅ **闭环（2026-09-24，Andy：「刷新了，现在显示136.5%，对上了」）**：修完 GAS 后他的浏览器仍显示旧数（132.12%），**因为前端只在页面加载时从 Sheets 拉一次**，他刷新的时刻早于写回。定位靠的是他导出的 CSV：用 CSV 的数量 + 同日 yfinance 收盘，复现出他截图上的现金（差 0.00）与持仓市值（差 0.02），于是价格源、拆股、期权三条路一次排除，剩下的 4.39pp 正是那 132 股 × 当日收盘。刷新后两边一致。⚠️ **前端 hydration 后 3 秒开始向 Sheets push**（`PortfolioContext.jsx`），所以在 GAS 侧改过数据后，必须先刷新页面确认前端已拿到新值再操作，否则旧的 localStorage 会把修好的数覆盖回去。**另**：Portfolio 页「YTD」行已改成真年初至今（`cd1c408c`）——组合回报按上年最后收盘链式算、交易统计只计当年平仓、曲线回不到上年时显示 "Since start" 不冒充 YTD。（UI Claire）
- ✅ **[2026-09-25] OPS → 全线：波段大师课 **ver1 封版**，中英双语 × PDF 与交互网页两形态都已上线；`data/content/RELEASE_ver1.md` 是「ver1 到底发了什么」的唯一权威源，以后要答这个问题读那一份，别在别处另立说法（T-0925-56；Andy 原话「让整个系统都知道这部分已经完成」）。**
  **封版范围（中英双语 × PDF + 交互网页，四个组合都封）**，逐项列全、不写「等等」：**前言** + **目录** + **模块一五课** + **模块一附录 MU** + **CH00、CH01、CH02、CH03、CH04、CH05、CH06、CH07（地基篇八章）** + **尾声** + **附录 A、B、C、D、E、F、G、H、L**。
  ⛔ **不在本次封版范围内，别当已交付引用**：①**附录 I、J、K —— 走 2026-09-27**（Andy 2026-09-25 澄清，原话「①，I/J/K 照旧走 9/27」——问的是「附录 A–L 是否含 I/J/K」，他答不含）；②**CH08–CH26 正文**（中英皆未发）；③**英文 CH08–CH26**（尚未翻译）；④**录播视频**（不存在，别在任何对外文案里提）。
  **两份 PDF（书线 commit `e96a490`，分支 `fix/T-0925-10-en-v2-pdf`；源文件在本机 `~/Documents/SwingMasterclass/_pdf/release_2026-09-25/`，该仓无 git remote，PDF 不进任何仓库）**：
  · 中 `Fluxus_波段交易大师课_中文_2026-09-25_ver1-b5.pdf` · **194 页** · **13,568,448 B** · md5 `c7302ad14f7a84d00dcd7d9f80e17b6e`
  · 英 `Fluxus_Swing_Masterclass_EN_2026-09-25_ver1-b5.pdf` · **210 页** · **12,390,358 B** · md5 `6c56c6f22c19e738601132edafd03f40`
  · 买家下载路径（付费区，吃同一道 JWT + 会员资格门禁）：`/paid/downloads/Fluxus-Swing-Masterclass-PartI-ver1-ZH.pdf` 与 `…-EN.pdf`。买家看到的文件名不带日期、不带内部后缀，版次由 `ver1` 承载（`_web/build_ch.py` 的 `PDF_FILES`）。
  **网页**：`https://fluxus-masterclass-lab.pages.dev`（Cloudflare Pages）· **生产部署 `bbb882b4`** · **部署源 commit `408bfb1`**（分支 `feat/T-0925-49-paid-pdf`）。
  **线上实测（2026-09-25）**：付费区 **80** 个文件在无 token 时**一律 403 且返回购买页**；免费试读区 **92** 个文件**全部 200**；Andy 同日以店主身份从 Whop 内打开，原话「一切正常可以看到」。
  **Whop**：`Swing Trade Masterclass` 档 **$1,499 一次性**、**库存 50**、Discover 开着、商品页文案与标题 2026-09-25 已更新。
  ⚠️ **三个已作废的说法，谁也别再引用**（同一个数作废过两次，最容易在角落留残）：**「中文全本 390 页是交付版」作废**（390 页那份是 `_内部留档_全本不对外/`，显式不对外）；**「中 199 页 / 英 213 页」也作废**（那是 09-25 12:30 的 ver1 首版，当日 b2→b5 四轮重出后的定版是上面的 194 / 210）。作废原文留在 `fluxus-ops` 的 `tasks/T-0925-49.md` 与 `agents/q/runs/2026-09-25T031509Z-T-0925-49.md` 里当审计痕迹，**那些是历史记录不是现行事实**；`SwingMasterclass/_pdf/README.md` 第 769 行的「EN 213 页 · ZH 198 页」（及第 755 行同形的「英文版 214 页 / 中文版 198 页」）是 2026-08 旧书（L1–L16 结构）的构建日志，同样不是交付声明。
  **页数/md5 的权威源登记在 `KNOWLEDGE.md` 数字权威表**，引用前现场读，不转抄。（OPS · T-0925-56）
- [2026-09-26] **DATA ALEX：主题四态并排期核实（T-0926-06）——代码自动到期不用改，`state_prev` 暂定继续保留。** 现状核对：`data/output/theme_board.json` 当前 `asof: "2026-09-24"`、`parallel_until: "2026-10-08"`；前端 `ThemeBoardCard.jsx:97-98` 用 `parallel = data.asof <= data.parallel_until` 判定要不要画旧读数列，数据日期一旦超过 2026-10-08 会**自动**不再显示，不需要代码改动。**决定：`state_prev` 继续发**——按任务书自带的默认口径「不确定就保留字段，多发无害，停发会被夜间闸拦」，停发要过 schema 基线 + `quality.REQUIRED_BLOCKS` + 隐私闸三道闸，现在没有非停不可的理由，先不动。**未完成项**：验收第一条「10-08 之后线上页面实测旧列消失」是未来事件，09-26 今天物理上做不到，不硬等 12 天凑 done——已开 followup 单 `T-0926-07`（`not_before` 设到 2026-10-09，交给到时候的巡检实测并对 `state_prev` 去留做最终拍板）。（DATA ALEX）
- ✅ **[2026-09-26] OPS → 全线：上面 [2026-09-25] 那条封版行三处被取代，本行是修订，不改旧行一个字（T-0926-05）。**
  **取代了哪几项**：① PDF 交付形态——原「PDF 与交互网页两形态都已上线」作废，**PDF 退居二线、不再对外提供**（内部留档）；② 封版范围——原「CH00–CH07（地基篇八章）」扩到 **CH00–CH11（十二章）**；③ 买家入口——原「PDF 下载」两行入口撤销，只剩两个。
  **一、PDF 退出对外交付**（书线 `SwingMasterclass` commit `d6f4e33`，Andy 原话「让pdf退居二线，不提供了。做1」「但我们内部保留pdf」「目前还没人购买。先做吧」）：`/paid/downloads/` 下载区整区退役，`build_ch.py` 的 `downloads_block()` 只留「回免费试读」那条路；`audit_release.py` 新增第六道闸——产物里不得出现 `/paid/downloads/` 或 `.pdf` 链接。**两份 PDF 文件本体不变**，仍是上面登记的 `ver1-b5`（中 194 页 md5 `c7302…`、英 210 页 md5 `6c56c…`），只是从「买家下载路径」降为「内部留档」，页数/字节/md5 三个数字继续有效、不需要重量。
  **二、交互课件扩到全集**（同仓 commit `d4c6e58` T-0925-10，Andy「交互成为全集，PDF 体积问题随之作废」）：**前言 + CH00–CH11（十二章）+ 尾声 + 附录 A、B、C、D、E、F、G、H、L（九篇）**，中英各 **26 页**（合计 52 页，`audit_release.py` 闸「52 页全绿」实测通过）。**附录 I、J、K 仍不在内，仍走 2026-09-27**——这条不变，不是本次修订对象。
  **三、落地页重构为三块**（同仓 commit `743f07e`／`846847f`，Andy 原话「我希望我们打开app后出现的页面能否是一个完整的体验，就是 1 介绍+预期更新通告 2. 免费的裸k训练 3. 查看完整课程购买链接」）：`/` 从五课列表改成落地页（列表搬到 `/lessons.html`），依次是**①介绍+预期更新通告 → ②裸K训练入口（免费五课）→ ③看完整课程**；另加**深浅切换钮**（同仓 commit `127f5ae`，Andy「整个页面的黑暗和亮白背景是由什么选择的」，三态 auto/light/dark，默认 auto，存 `localStorage`）。
  **买家入口现只剩两个**：免费试读 `https://fluxus-masterclass-lab.pages.dev/`（走 `/lessons.html`）· 付费区 `/paid/`（JWT + Whop 会员资格门禁）——原「PDF 下载（中/英）」那两行入口已撤。
  **线上实测（2026-09-26，本单现场 curl）**：`https://fluxus-masterclass-lab.pages.dev/` 返回 200，页面 footer 印 `由课文源文件生成 · e8f0676 · 2026-09-26`，正文已是「一套交互课件，就是这门课的全部……第 0–26 章……现在能看：开篇 + 第 0–11 章 + 尾声 + 附录 A–H、L」的新文案，与上面①②③一致。`e8f0676` = 书线 `feat/T-0925-49-paid-pdf` 分支 HEAD，是本次上线的部署源 commit。**⚠️ 本单未取到 Cloudflare Pages 的部署 hash**（原 2026-09-25 那条记的是「生产部署 `bbb882b4`」这种 CF 自己的部署号，需要 `wrangler pages deployment list` 才能查，本机未装 `wrangler` CLI、本单未展开这一步）——**这是缺口，不是已核实的数字，别当权威源引用**；要补，下一个碰这条线的会话跑 `npx wrangler pages deployment list --project-name fluxus-masterclass-lab`（在 `~/Documents/SwingMasterclass/_web/cf_pages_deploy` 目录里跑）。
  **⚠️ 一并作废**：2026-09-25 门禁实测那两个数「付费区 80 个文件 / 免费区 92 个文件」是旧范围（CH00–07）下测的，范围扩到 CH00–11 后文件数必然变了——**本单未重新实测，别再引用 80/92 这两个数**，需要就开新任务重测。
  **四、三条待办已登记、不在本单做**：①dashboard 数据搬迁——Andy 2026-09-26 定用 **Whop 登录**，数据拟放 **Cloudflare R2**，公开那半（universe / ticker_events / tickers / baskets / performance / modelbooks）留公开仓；**顺序：先通新地址再撤旧数据**。②⚠️ **搬走 ≠ 历史干净**——旧数据仍在 public repo 的 git 历史里，真要干净得改写历史或换仓，**这条本身就是记这一句，免得以后有人以为搬完就干净了**。③书线两处待改：课程地图「正书 · 七篇 26 章」应为 **27 章**（中英各 3 处）；CH05 正文「特别篇〈Fluxus Playbook〉」按 Andy「特别篇就是 Fluxus Playbook」判**冗余**，待书线处理。
...
- **① `etf_data.json` 之前完全没有日期键**（裸的 168 元素 JSON 列表，任意深度找不到日期串）。已修：`pipeline/adapters/yfinance_adapter.py::fetch_etf_data` 给每一行加 `as_of`（`pipeline.marketcal.last_completed_session().isoformat()`，ET 交易日——和 `run_all.py` 里 `event_date`/`asset_payload`/quality 校验等调用同一个来源；**不是**那 15 个 `timestamp` 字段的来源，那些字段是 `datetime.now(timezone.utc).isoformat()`，抓取时刻的 UTC datetime，两种口径这次审计时才发现一直混用）；同一提交把仓库里现存的 `data/output/etf_data.json` 也按同一交易日（2026-09-25，与同一次跑出的 `market_health.json`/`breadth.json` 一致）回填了这个字段，免得等今晚 cron 才生效。
