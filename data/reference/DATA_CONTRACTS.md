...
- [08-21] **`tickers/` 里有 K 线、没基本面的那一批**(空壳修完之后剩下的另一半)。**08-21 19:xx 量的是 41/188**——写日期是因为我今天刚在"92 个空壳"上栽过:这类数每晚都在动,**它是"每次抓取失败留下的坑"不是一个存量**,所以别把某一次的读数当现状引用。形态:`ohlc_2y` 有值而 `info: {}`,与那 11 个空壳**零重叠**,所以是另一条失败路径不是同一个的余波。样本:APLD APPS CBRG CGNX CLSK CRCL FPS GLW INTC IOT IOVA LIFE …。前端可见面:ticker 页的**估值快照**和**财报**两块吃的正是 `info`,所以那些名字上会出现"有图无基本面"。不急,列在这里备查;要不要修、怎么修归数据端判断。
- [08-22] **(ops→数据端+前端) 大扫除从废弃 worktree 里捞出 6 条含 main 没有的真修复的封存分支**,已全部 push 到 origin,8 个并行 agent 逐条对比过 origin/main 现状(2026-08-22 量的)。**归数据端**的五条:`fix/ohlc-staleness-guard`(ohlc_store 加 through/max_age_days+latest_close 防陈旧价,附 159 行测试)、`claude/keen-germain-696b72` 与 `claude/optimistic-clarke-4c56d0`(**同一个 bug 的两份修复,二选一**:stockbee_ratio 同日重跑双计+归档被截成 5 条,_upsert_entry 方案,各附测试)、`claude/sharp-boyd-7ee3fc`(ticker-store 保鲜守卫 staleness.py+CI step,针对「46 只冻在 05-22 达 2.5 个月」)、`claude/silly-borg-68395f`(非交易日不落零行防护,锁定 08-08 周六写零行 bug,含 test_no_offsession_rows.py)。**归前端**的一条:`claude/wonderful-shannon-b66cdf` 里的 78 行 hookOrder.test.jsx 回归测试(hook 修复本身 main 已有,测试没有)。要不要合、怎么合归各线判断;不合也请在本行后追一句结论。另:`claude/adoring-haibt-ceef68` 已被 v3 sweep 取代、`auto/night-20260819-b` 只是可重算的研究 CSV,两条不用看。
