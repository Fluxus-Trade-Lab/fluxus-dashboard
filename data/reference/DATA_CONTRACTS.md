...
- **[2026-09-26] alex（T-0926-29 晨检）：`quality.json` 的 `groups_stocks` 连续两个交易日（09-23、09-24）报 degraded，174/5281（3.3%）票缺 `excess_1m`/`rs_accel`/`persistence` 等字段，不阻断、此前无人记录。** `pipeline/quality.py` 的 `check_source()` 对 `groups.json.stocks` 逐票测这批字段的 null rate，`data/history/quality/groups_stocks.csv` 显示 09-22 缺失率约 0.7%，09-23 跳到约 3.2–4.0%，09-24 持平未扩大。抽样 20+ 支缺字段的票（AEHL/AIIO/AIXI/AMCI/AMPG 等）均**不在** `universe.json`（244/5607 支主表）里出现过——`groups.json.stocks` 的 5281 支来自比主 universe 更宽的行业成分源，未查这批票是不是刚被该源纳入、还没攒够 1 个月交易历史；来源脚本未定位。**不阻断**：同一天 `audit_archives` 0 violation、`schema_snapshot --check` exit 0，`quality.json` 顶层 `status: degraded` 只由这一项拖累，其余 5 个 source（etf_data/groups_themes/rotation_baskets/breadth_last/signals）全 ok。留给下一班或数据契约复核：定位这批票的来源脚本，确认是新纳入成分股（可考虑归进 `is_sparse_by_design` 降噪）还是真实取数缺口。（alex）
