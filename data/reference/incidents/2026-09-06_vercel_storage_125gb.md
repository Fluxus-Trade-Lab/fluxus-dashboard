...
> ⚠️ **09-13 注（DATA ALEX）**：这个 75 MB 是**反推值**，Vercel 实际给单个部署记多少从没实测过。09-12 拿它反推出「积压约 1,600 个、要 280 天」是错判——API 实拉（09-13）活跃部署只有 500 个，而且容量下降由 30 天恢复期何时期满决定，不是按速度走的斜线。**别拿这个数做乘除**；Vercel 的判断以 [`.claude/skills/vercel-ops/SKILL.md`](../../../.claude/skills/vercel-ops/SKILL.md) 为准。
