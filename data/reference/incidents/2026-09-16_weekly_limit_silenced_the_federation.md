...
- ② 额度预算：Andy 原话「额度预算怎么切的问题，目前只需要做一件事，哨兵改成每2小时做一次检查」。已执行：哨兵 cron `15 * * * *` → `15 */2 * * *`（下一班 2026-09-16 14:15Z），任务书同步。
- ① Actions 心跳：Andy 这次没要，暂不做。
- ⚠️ 本档上文「断档 >2 小时」「>3 小时开 issue」两个阈值都是按每小时一班写的；改成两小时一班后，健康时 `sentinel(` commit 间隔本来就是 2 小时，判据改为 **>4 小时** 才查 run log。
