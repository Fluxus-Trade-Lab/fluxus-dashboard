...
7. **⚠️ 归档能自证自相矛盾，而此前没有任何闸在看这条。**（Nighty Zac 实测，2026-09-06）
   ...
   实测有**三天**不等，三种不同的坏法（第三天只有把恒等式推到**跨文件**才看得见）：
   ...
   | 2026-09-02 | 11（跨文件） | 11/11 共同票 | `leaders_log` 与 `delayed_ep_log` 都记 `close`，当天 11 只共同票全对不上，**10/11 的 delayed_ep 读数逐位等于 leaders_log 前一场(09-01)的读数**。同一对在 09-03/09-04 零分歧。旁证：git 里没有任何 `chore: market data 2026-09-02` |

   **08-14 那条 `change_pct` 看不见，只有 `volume` 看得见** —— 那次修复盯的是大声死掉的那一列，
   旁边安静退化的那一列没有人验收。**空值检查看不见错值。**
   **09-02 那天在文件内部完美自洽**，任何只看一个文件的检查都判绿 ——
   **自相矛盾要能被看见，前提是有两份读数放在一起；闸的边界画在哪里，决定了它能看见哪一类错。**
   （08-14 在跨文件这一层被**独立报了第二次**：6/9 只共同票 `close` 对不上。两个证人。）

   **已建**：`pipeline/tools/audit_event_agreement.py`（+**31** 条测试，**15** 个变异体全部被杀，commit `2793493d` 起）。**两层**：文件内 + 跨文件；后者按较粗一方记录的精度比（`delayed_ep_log` 写 float32 残迹，逐位比会把 71 例里的 54 例判成假分歧）。⚠️ 对照文件不在场时**不判绿**，打印 `cross_checked=False`；且「跑成了」问的是有没有**一对**是齐的，不是有没有任一个文件在。
   查**五个**跨筛子必须相等的字段 `change_pct` `volume` `sector` `atr_ext` `rel_volume`
   （`sector`/`atr_ext` 全库 26,108 / 18,558 例可比、**0 例不一致**，是干净对照）。
   **比法按字段定，不是一刀切也不是拍的容差**：`rel_volume` 按较粗一方记录的精度比
   （`vol_up_gainers` 四舍五入到 2 位、预设写全精度）—— 精确比 42.38% 的「不一致」全是精度，
   按精度比后 **0.19%（13 例），13 例全部落在上面这两天**，**它本来就能独立指认出它们**；
   ⚠️ 同规则**不能**推广到 `change_pct`（实测漏 6 例，全是 `"0.0"` 撞 `"0.0412"`）。
   明确**不查** `group` —— 那是**多义列**（`momentum_97` 写自己的 RS 桶 97/98/99/100，
   `healthy_charts` 写别的），同一个列名两个量，分歧散布在 111 个日期上。
   棘轮形状，两天已具名声明，E2 逼人修好后删声明。
   （⚠️ 本条初版把 `rel_volume` 也归成「定义不同」——**没量过就给理由，量过之后是精度**，已更正。）
   ⚠️ 第一版只查 `change_pct` 时有 **3 天（08-11/12/13）完全看不见**，扩到四字段后盲区 **0/114** ——
   闸自己报覆盖面，因为「有闸」之后还得问「盖住了多少」。

   **仍欠（归 DATA ALEX，本线不碰 `data/history/`）**：⓪**09-02 最近（距今 4 天）**——定位是哪一环把 09-01 的收盘价写进了 `delayed_ep_log` 的 `as_of=2026-09-02` 行；⚠️ `delayed_ep_scan` 的四态判定本身吃 `close`，**那天的态也可疑不只是这一列**；①重算或撤下 08-17 的 604 行 `preset:*`；
...
8. **⚠️ 第 7 条的欠账三天零动作；顺带第三个证人，以及一条比它更该记的边界。**（Nighty Zac 实测，2026-09-09）

   **欠账读数（09-09 04:5x JST 现场跑）**：第 7 条的 ⓪①②③ **一条没动**。
   `audit_progress` 仍是同样 5 条 violation（09-02 整场 36/36、08-17 12/59、08-19 12/57、08-20 8/57、
   CORT 的 `base_high`），`audit_event_agreement` 三条声明原样还在。
   09-06 以来碰过 `data/history/` 的只有 cron 的两条 `chore: market data`。**声明是欠条，第 3 天。**

   **第三个证人（独立于我们另外两份归档，但与扫描器同源于 yfinance）**：Delayed EP 复盘自带的 join check（账本存的 `close` vs 今天重拉的同一根 K 线）
   把偏差**全部**定位在一天——全账本 **31 行偏差 >0.5%，31 行全在 2026-09-02**，其余 16 场 0 行；
   且是**方向性**的：那 36 行 **0 行**配当日的厂商 K 线、**36/36 行**配 09-01 的，其余 16 场全部「配当日＝全部行」。
   **16 场阴性对照 + 1 场满分阳性。** 厂商对这 36 只票的 09-01/09-02 收盘价**全部不同**（0/36 相同），
   所以这不是厂商发了两根一样的 K 线，是我们这边重放的。
   ⚠️ 「独立」只到这个程度：`fetch_bars.py` 与 `delayed_ep_scan.py:219` 是**同一个厂商、同一个函数、
   同一个参数**（`yf.download(..., auto_adjust=False)`）。独立的是**取数的时刻**，不是数据源。
   它能证「账本里那个数不是 09-02 的收盘价」，不能证「yfinance 当晚给的是什么」。
   **巧合已排除**：那 36 只票 09-01→09-02 的真实收盘变动中位 **1.90%**、最小 0.137%，
   **0/36 落在 0.1% 以内**；容差压到浮点全等结论不变。

   **还有第四个证人，而且最便宜——它在文件内部**（复核 agent 补的）：09-01 与 09-02 逐列比，
   17 列里 **15 列 36/36 全等**，其中包括 `days_since`。**一个真实的新交易日，这个计数器必须 +1，它没有。**
   不需要厂商、不需要日历、不需要第二份归档。
   → 四种比法指同一天。**但它不是「文件被整份复制」**：`today_relvol` 36/36 全变、`recent_range_pct` 19/36 变，
   说明**扫描器当晚确实跑了，只是价格侧读的是前一场的 K 线**。这个区别对定性有用。

   **⚠️ 更正我自己 09-06 的措辞**：我写过 09-02 是「`delayed_ep_log` 那天记的是前一场的收盘价」。
   复核 agent 独立算出来的是**整场逐位复制**——两场 ticker 集合 36/36 相同，
   **17 个数据列里 16 列 36/36 完全相等**（含 `stage`、`days_since`、`base_high`、`vs_ep_close_pct`），
   唯一全变的是 `today_relvol`（0/36 相同）。**`days_since` 没有 +1，是复制的直接铁证。**
   症状写对了，病灶写小了：**那一场根本没有产出新快照**，不是某一列写错。

   **新线索，交 DATA ALEX（本线不碰 `data/history/`）**：08-17 / 08-19 / 08-20 那 **32 行**
   `days_since` 少算 1 天的行，**`ep_date` 全部 ≤ 2026-07-30**（07-27:1 / 07-28:8 / 07-29:17 / 07-30:6），
   而同三场里 `ep_date ≥ 07-31` 的行 **0/136 出错**。边界干净得可疑。
   **但不能就此写成「07-31 那根 bar 丢了」**——`ep_date ≤ 07-30` 的行在 08-13 / 08-14 / 08-18 三场
   是 **0/47 出错**的。所以缺口不是账本层面的、也不是永久的，它只发生在**那三个晚上的取数帧里**，
   且不是全体（ALVO / CAKE 在 08-17 与 08-19 都是对的，CLS 在 08-17 是对的）。
   查它需要那三晚扫描器实际看到的 bar，账本本身答不了。

   **顺带扫了一次「还有没有别的归档重放过整场」**：7 个归档 / 188 对相邻场
   （`ticker_events` 回到 2026-03），站得离自身基线特别远的**只有** `delayed_ep_log` 09-01→09-02
   （逐字相同 90.5% vs 自身中位 60.2%）；其余六个归档最高一对都在自身中位 +10pp 以内。
   ⚠️ **但这句话的边界必须一起写**：该扫描的分辨率是「一场重放里最多允许几列是新的」，
   实测 **0~3 列**，`delayed_ep_log` 上是 1（09-02 正好卡在边上，**能看见是运气**），
...
9. **⚠️ 第 8 条说「`ticker_events` 上带宽为 0」，这个口子现在有一把尺子了。它首扫就逮到一天以前没人报过的。**（Nighty Zac 实测，2026-09-11）
   ...
   **没动的 15 天与理由**：04-02 / 05-22 / 06-18 / 07-02 旧行对厂商 K 线本来就是 1.000，只换 preset 行会让同一天分属两份快照（`audit_event_agreement` 实测新增 4 天 E1），要改就两个回填一起重跑；08-19 起 11 天是夜间产线当场写的，行数与闸读数都不变。
   ...
   ↳ ✅ **Nighty Zac 复核（2026-09-14）**：通过。08-07 在 change_pct 上「查不了」，另用 `volume` 对厂商 K 线独立核：新 preset 行 **86/86 配 08-07**、冻结坏行 **78/78 配 08-06**；08-17 新行 121/121 配当日、冻结行 0/105（周一盘前的 volume 不配任何一天的收盘量）。08-06 78/78、08-14 95/95。新旧标签差异重算为全史 37 个日期、归档内 **21 个**，与上文逐字一致。
   ↳ **盲区 2 补了一条恒等式（Nighty Zac，2026-09-14，`audit_events_vs_bars`）**：`volume` 对厂商 K 线**当日** bar 量，单边带 [0.90, 1.01]、判定线 0.80（自造，登记待合 `auto/night-20260914-4ef60f-metric`）。08-07 与 08-11/12/13 由「查不了」变为判绿（77/77、74/74、74/74、77/77）；两条都判不了的是 03-12/03-13/03-26/03-27/03-30 五天（后三天是 change_pct 也改成按票计票后露出来的：此前重复行把它们抬过了 n≥15，那三天的「判绿」是同几只票数了几遍）。冻结坏行 08-07 在 volume 上 7/56、帧 = 08-06（两条不共用算术的尺子说同一句话），08-17 = 23/106 无帧。**分辨率**：干净日最差 0.929、噪声底最高 0.304；**一场里被换掉的票少于约两成（中位 23%）看不见**。两个 verifier 复核（复算 + 反驳），13 个变异体见晨报。
