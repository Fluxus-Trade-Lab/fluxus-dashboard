...
| `market_light.spy.light` / `checks[]` | 课程 **L6 交通灯**（SwingMasterclass） | SPY 日线 10 vs 20：10 在 20 上 + 两条都上倾 = 🟢，**其余一律 🔴**（`L6:183`）；均线类型课文只写「MA」 | ⚠️ **部分自造**（2026-09-11 落地）。均线 **EMA 做参数**（NEEDS_ANDY 缺口一）；**「上倾」= 今日 > 昨日是自造操作化**（课文无数值判据，Studio Q 裁 L106，禁「N 天前」参数）。复算：课文印的「SPY 2015-26 绿 54.5」与两段「最长连续」日期**全部逐字复现**，但属于**另一套**口径——**SMA 10/20、上倾＝比 3 天前高**（Studio Q 09-11 查出；我首轮只扫了网格两条边、误报「任何口径都复现不出」，已更正）。本页用的 EMA＋1 天在同窗口读 56.55/19.30。**同一课两套口径＝NEEDS_ANDY 缺口一**，已成课程仓单选题 |
| `market_light.spy.plus_n` | 课程 **L6B.4 +N/−N 周期计数** | 收盘 vs 21 日 SMA，连续同号为一段，**≤4 天的段剔除（不并入邻段）** | ✅ **一致**（2026-09-11）。照 `_pdf/cycle_bench.json`；**SPY 与 QQQ 各 7 项逐字复现**（均值 21.4/12.7、段数 125/82、最长 68 起于 2025-04-24、32 起于 2022-04-11）。21 日线类型做参数（缺口二）。实时读数给**原始游程**（课程梯子从 +1 起算），≤4 另给 `plus_n_noise` 标志 |
...
| `market_light.brightness.setups` | 课程 **L7-Q1** setup 数 | 10+ → 🔥、1-3 → 暗绿、0 → 当红（`L7:49-50`）；**4-9 课文没说** | ⚠️ **自造映射，provisional**（待 Studio Q 验收）。= watchlist `entries` 区五个面板去重后的名字数；面板按 L10-11 战法打标（PB/URAR/EP），**`ma_reclaim` 不算**（配方是「昨收线下、今收线上」＝从下方爬回，与课程 PB「绿灯趋势中回踩到均线」方向相反；09-10 含它 43、去掉 21）；**BO 与 HTF 无对应面板**；面板被截断时标 `count_is_floor`；4-9 报 `unspecified` 不自填 |
...
| `market_light.verdict` | 课程 **L7 判决** 🔥/暗绿/回避 | 红灯 = 回避（「sit still」）；**绿灯时三问如何合成，课文没给** | ⚠️ **红灯日由课文独定 `avoid`；绿灯日为 null** 直至合成规则与 Studio Q 在 §七 定下，前端不自合 |
