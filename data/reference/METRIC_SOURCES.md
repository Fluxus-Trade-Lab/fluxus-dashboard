...
| Short List「Sugar Babies 名册 ≥5 次＝反指」 | 名册本身挂 Stockbee **Sugar Babies**（一手定义未找到，见审计 C 表 #46） | — | ⚠️ **自造，2026-09-21 登记**（C 表 #25）：`roster` = 最近 10 个事件日里命中 `P:sugar_babies` 的天数（`name_cards.py:135`），`≥5` 时名片加「⚠Sugar Babies 名册 N 连(反指)」（`:112-113`）。「反指」的依据已留档：2026-08 扫描器验证里该预设 20 日前瞻 −9.5%（`data/research/case_mrna_2026-08-19/six_names_2026-08-20.md:20`；半字母表重算后 −11.4%，`half_alphabet_reach_2026-09-17/README.md:20`），且 heat 不给它权重。**5 这个门槛理由未留档**。⚠️ 标签写「连」，代码数的是 10 天里的次数，不要求连续 |
| preset `Sugar Babies`（`bo_count_1y` ≥10 ∧ `bo_count_3m` ≥2） | 挂 Stockbee **Sugar Babies**；一手来源无数字（Investors Underground 访谈只说「半年或一年里 9M EP 次数多」、盯 25–30 只） | 每次 bo＝c/c1≥4% ∧ v>v1 ∧ v≥100,000（他的 4% 扫描原文） | ⚠️ **自造，2026-09-21 登记**（C 表 #46）：单日 bo 判定照他 4% 扫描原文；按票滚动计数、窗口与 10/2 阈值是我们的，**理由未留档**。⚠️ **计数单位可能和原作者不同**——他说的是 9M 放量 EP 事件，09-04 换成了 4% breakout；待 Andy 裁 |
