...
| `mcclellan_osc_ndx` / `mcclellan_osc`（legacy） | McClellan Oscillator | RANA = net/(adv+dec)×1000，19 与 39 日 EMA 之差；原始口径统计 **NYSE** 上涨/下跌家数 | ✅ **池子已换标准（2026-09-23，T-0923-03，Andy「我们的MCO应该用纳指100或者纽交所」）**：查过 NYSE 逐日 A/D 无免费源（yfinance 无 `^ADVN`/`^DECN` 一类 ticker，2026-09-23 现查），改用任务本身允许的 fallback——**Nasdaq-100**（`in_ndx` 成分表，`pipeline.screeners.run_all.attach_ndx_membership`）。`advances_ndx`/`declines_ndx`/`mcclellan_osc_ndx` 是新的标准读数，投票（`votes.mcclellan`）与页面图表已切过去；**Nasdaq-100 台账从 2026-09-23 起才建立，之前的每一行都是 NULL，不倒填**（没有历史成分股名单可回放，倒填只能拿今天的成分股冒充过去，那是编造历史，见下方 M12 判例同类问题）。旧 `mcclellan_osc`（Finviz 全池 ~5,600 只，不对应任何发布指数）**保留在归档里供连续性引用，不再是标准读数**，`verdict.context` 百分位与 Market Conditions 0-100（`conditions.today`，Andy 已裁「可以直接闭了」的免验项）仍读它，未随本次改动切换——若要一并切换需要单独评估对那两个复合指标的影响，本次任务范围只覆盖 MCO 本体的投票与展示。extreme 阈值 ±70→±100：StockCharts ChartSchool 自己发布的超买超卖线是 "+100 and -100"（2026-09-23 现查确认），±70 只见于二手科普，2026-09-18 审计（`metric_audit_2026-09-18/B_market_layer.md` M12）已判无一手出处 |
...
| `adr_pct` | ADR% (Qullamaggie / Deepvue / TradingView) | `100×(mean(High_i/Low_i, 20根)−1)`：纯日内、算术平均、每根除自己的 low | ✅ **一致**（2026-09-04 落地 `843d527d`）。此前发的是 ATR%。闸阈值 3.5–10 当时写作「从 Qullamaggie 借的」——**2026-09-21 更正：查无出处**，[qullamaggie.com/faq](https://qullamaggie.com/faq/) 只给 ADR 定义、没有阈值，3.5 是我们的数（见下方「ADR 全局下限 3.5」行）。口径切换当时实测生产闸 ≥3.5，我们过 226 / 标准过 205，**24 支（10.6%）只因读数偏高才进来** |
...
| `ep_industry_seasons.json`（T-0921-85，课程「Rotation」§5） | 三部分自造/代理，逐条已声明：① EP hit 代理「财报后跳空」② 「财报季」窗口 ③ industry 用 Finviz | ①**代理，非过滤**：EP hit = `ep_stockbee`∪`ep_qullamaggie`（原作者口径，2026-09-18 起）∪退役的 `episodic_pivot`（≤2026-09-17，用于回补历史，已知不匹配三个发表口径之一，见 `audit 2026-09-18 #44`）；宇宙级无逐票财报日期字段（覆盖仅 `ticker_data_fetcher.py` 的约 244 支 tracker 相关票），不做真「财报后」过滤——Stockbee 原文本身把 EP 扫描定性为待人工读「game-changing earnings」的候选名单（`ep_stockbee.UNMEASURED`），并购/FDA/指引修正等同样会产生 EP 形态的 bar。②**「财报季」无 SEC/交易所官方日期**（仅 45 天披露死线，2026-09-21 检索 FINRA Investor Insights 确认），采用 FINRA/Yahoo Finance/CFI 引用的通行惯例（季末+约2周 → 次月末），窗口边界是我们对这句惯例的操作化，非任何机构发布的数字；档案里 EP 命中密度确实在 04-20..05-08、08-04..08-21 出现尖峰，与该窗口吻合。③industry 用 `data/output/universe.json` 的 Finviz `industry` 字段（09-21 Andy 裁：GICS 不可得，见 `DATA_CONTRACTS.md` §七），**按今日快照回填历史命中**，不随命中当日的分类走；退市/改名/被收购后不在当前 universe 的 ticker 计入 `unmapped_tickers`，不丢弃不误分类 | ⚠️ **三处均已在代码注释、本表、输出 JSON 里明写；不得当标准读数上页**，页面呈现须带「代理指标，非官方财报季/GICS」字样。方法：`pipeline/tools/ep_industry_seasons.py` |
| `mcclellan_summation_ndx`（MCSI） | McClellan Summation (Ratio-Adjusted) Index | Ratio-Adjusted 版 McClellan 振荡器逐日累加，从振荡器第一个有值的交易日起算，不含 10 日均线的均线本身也存了一份（`mcclellan_summation_ndx_ma10`）（mcoscillator.com/learning_center/kb/market_data/ratio_adjusted_summation_index/，2026-09-23 现查确认公式与 ±500 强趋势阈值的一手出处） | ✅ **新增（2026-09-23，T-0923-03）**，池子同上一行 Nasdaq-100，同样从 2026-09-23 起才有值，不倒填历史。extreme 注记 ±500：原文原话「a strong uptrend can be signified by index values going from below -500 to well above +500」，只作 `verdict.notes` 的强趋势提示，不是一票投票 |
| — | Arms Index (TRIN) | (adv/dec)÷(上涨量/下跌量) | 🔲 我们没有 |
| — | Bullish Percent Index | P&F 买入信号占比 | 🔲 我们没有 |
| `advancers`/`decliners`（A-D 线的**解读**，非计算） | William J. O'Neil, *How to Make Money in Stocks*，小节 "Interpret the Overrated Advance-Decline Line"（中译本《笑傲股市》；该小节标题**无手边官方中译，复盘里用的是自译并附英文原名**） | 计数口径本身是标准的；O'Neil 给的是**解读口径**：市场方向不是单纯数人头的游戏，知道真正的领涨力量在哪、怎么走，比数清有多少平庸股票在涨跌更有用。例证：纽交所 A-D 线 1998 年 4 月见顶，而牛市六个月后才开始，该线一路向下、错过几十年里最强的几波。他的定位是「价值有限的次级指标」 | ✅ 出处由 Andy 2026-09-24 提供（书页扫描），已写进 09-23 复盘的教育段正文。**这条管的是怎么读，不改任何计算**；广度读数仍按 Stockbee/标准口径算 |
...
| VCP 第一层 = Trend Template（`vcp_detector.layer1_finviz_filter`） | Minervini **Trend Template**（《Trade Like a Stock Market Wizard》，8 条） | ①价在 150 日（30 周）与 200 日线之上 ②150 日线在 200 日线之上 ③200 日线上升至少 1 个月 ④50 日线在 150 与 200 日线之上 ⑤价在 50 日线之上 ⑥距 52 周低 ≥30% ⑦距 52 周高 25% 以内 ⑧RS ranking 不低于 70 | ⚠️ **2026-09-21 补齐并声明**（审计 C 表 #43 小尾巴）：①②④⑧ 此前缺，现已实现；⑤⑥⑦ 原有。**偏离，自造，逐条声明**：150 日线读 `wk_sma30`（30 根周五收盘均线，他自己写「150-day (30-week)」，行上没有日线 150 SMA）；**③ 未实现**（行上无 200 日线斜率，不造代理）；⑧ 读 `rs_rating`（IBD 式社区重建，见 `rs_rating` 行），不是 IBD 原值；**close >$10、市值 ≥$1B、perf_1m >0、`abs(perf_1w) < abs(perf_1m)` 四条是我们的**（把约 2,400 行压到第二层可抓取的量，不是他的条件，理由未另留档）。整列缺失（fallback 宇宙）时跳过该条并记 warning，单行缺值即不过。09-21 用线上 universe 复算第一层 270 → 194 |
...
| `min_vol_3d_1` · Anticipation 流动性 | Stockbee **minv3.1** | 截至昨日 3 根的最小量 `>=100000`——[2019-10](https://stockbee.blogspot.com/2019/10/anticipation-scans-that-can-make-you.html)（作者评论释义 minv3.1） | ✅ 一致（2026-09-18 新列）。vcs≥60、adr≥3 为自造；原文 Buyout 条件未做；强度阈值 1.05 / 1.8 / 1.19 出自另一篇：[2016-04](https://stockbee.blogspot.com/2016/04/how-to-find-bullish-anticipation-setups.html)——TI65 `avgc7/avgc65>1.05`、MDT `c/avgc126>1.19` 为严格大于，Double Trouble `c/minl252>=1.8` 为大于等于，2026-09-21 起逐条照原文比较符（此前 TI65/MDT 用 ≥） |
...
| 主题／行业聚合的成分闸（`themes.in_theme_content`，`FULL_CONTENT_BASKETS`） | *(查过，无标准)*；对标 TSF 自己的主题篮子 | 2026-09-23 起：只排除 `industry == "Shell Companies"`，无市值、无成交额门槛；此前是 `is_tradeable`（$1B 市值 ∧ $2M 日成交额 ∧ 非空壳） | ⚠️ **自造，2026-09-23 登记**。Andy 原话「主题篮子放开市值闸，采纳，但交易 screener 和实际操作我们会有市值的 filter」。**证据**：TSF 26 个同名主题快照（2026-09-22，私有仓 `fluxus-ops/data/tsf_members/`）里我们的票 96% 在他篮子里、他的票只有 72% 在我们篮子里，缺的 352 席 95% 市值 <$1B；放开后覆盖 72%→93%。空壳排除保留——他多出来的 352 席里空壳为 0。**交易层不受影响**：`is_tradeable` 仍定义 run_all 的打分域，`universe_gate.cap_floor` 仍管每张筛子单。副作用：参与聚合的票 2,524→5,280，主题成员中位 61→81，可打分行业 121→139 |
