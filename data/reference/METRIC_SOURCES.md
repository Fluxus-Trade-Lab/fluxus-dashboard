...
| thrust 投票（`THRESHOLDS['thrust']`：`fraction 0.113`、`min_count 60`；分子 `up_4pct`/`down_4pct`） | Stockbee **4% breadth**（「300+ 家涨 4% 为 thrust day」的经验门槛） | 当日 `up_4pct` ≥ max(60, 0.113 × universe_size) | ⚠️ **自造，2026-09-18 补登记**（Nighty Zac 查出零登记）。①`0.113` 是把 08-09 前「300 家 / 约 2,650 只」的操作点固化成比例（代码注释自称 tuned，558 场中位 11.3%），不是任何出处的数；②**分子是自造列 `up_4pct`（只看涨幅），不是上一行登记在案的 `up_4pct_stockbee`**（加量能两条件）——09-16 两者 292/580 对 159/246，差约一倍；③06-26 人口换了（≥$1B → 全市场，小票 4% 日基础频率高 1–3 倍，未独立复核），同一个 0.113 在新人口上不是同一个操作点。**未改门槛也未换分子**：属口径变更，要 Andy 裁。页面散文里写死的「300+」改为按当日宇宙算出的门槛，在分支 `feat/alex-universe-era-percentiles` 待合 |
