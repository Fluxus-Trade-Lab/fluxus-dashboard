...
| `market_light.spy.plus_n` | 课程 **L6B.4 +N/−N 周期计数** | 收盘 vs 21 日线，连续同号为一段，≤4 天的段剔除（不并入邻段） | ⚠️ **弃用待删**（2026-09-11，Andy：trend day 计数「标上标注，等待删除」；缺口二裁 21 日线＝EMA）。字段保留、带 `plus_n_deprecated: true`，**仍按复算过的 21 SMA 计**（`cycle_bench.json` SPY/QQQ 各 7 项逐字复现），课程删那天随清单一起下，期间不迁移 |
...
| `market_light.brightness.setups` | 课程 **L7-Q1** setup 数 | 10+ → 🔥、1-3 → 暗绿、0 → 当红（`L7:49-50`） | ⚠️ **自造映射，provisional，未校准**。= watchlist `entries` 区五个面板去重名字数（`ma_reclaim` 不算，Studio Q 裁五验收）；**4–9 归 dim 并标 `band_default`**（裁二）。**校准未完成**（裁三：阈值 10 不动、池子收窄，锚＝红灯日中位 1–3 且最强绿灯段到 10+）：`watchlist_hits.csv` 17 天（13 红/4 绿）现映射红灯日中位 **68**；收窄到主题龙头 14、RS 新高区 25、两者都要 7——都到不了 1–3。池子怎么收由 Studio Q 定 |
...
| `market_light.verdict` | 课程 **L7 判决** 🔥/暗绿/回避 | 红灯 = 回避（「sit still」）；**绿灯时三问如何合成，课文没给** | ⚠️ **红灯日课文独定 `avoid`；绿灯日按 Studio Q 裁一合成，标 `verdict_synthetic`**：每问归 good/mid/bad（Q1 10+/1–9/0；Q2 破位占比 ≤20%/中间/≥50%；Q3 confirm/mixed/negate），任一 bad→avoid、全 good→full、其余 dim。任一问答不出时绿灯日判决为 null（「未测」不是「中」）。⚠️ Q1 未校准时几乎每天读 good，绿灯日它投不出 mid/bad |
