...
| ~~`market_light.spy.gear`~~ | 课程 **L6B.2 油门七档**（Webster） | 21 EMA、用**最低价**读线；七条规则逐条覆盖、编号大者胜；「急刹」= 今收破且昨收上、前 30 天 ≥22 天低点在线上 | 🗑 **已删**（2026-09-20，Andy「L6B.2 --L6B.6全部删除」；课程只剩 L6B.1「三条线，三群人」）。存活期间逐字照抄 `_pdf/charts.py::throttle_gears`、复现过课文对 MA 2025 图的三句话（急刹 02-21、最大防御 03-05、第 1 档从不触发）；market_light 同日下掉该字段（`instrument_block(..., include_gear=False)`），`audit_ledger.EVIDENCE['market_light']` 同步摘掉 `gear` 证据要求。`qqq.gear` 仍在算但只是遗留计算，前端不读、本表不再登记 |
