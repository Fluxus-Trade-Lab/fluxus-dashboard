...
口径不是自造的（宪法「先找口径，别自己造」）：
  ...
  * 本文另报 **有向包含度** C(A→B) = |A∩B| / |A|，
    ...
    有向这一步是自造的，明写在此。
⚠️ 2026-09-10 补一条边界：**本脚本对每一对只输出中位数**
（median_jaccard / median_containment / median_lift），既没有「最低的那一天」，
也没有 C=1.00 / C≥0.9 的天数占比。而本目录 results.md §3 的裁决恰恰是
**中位数在这里会骗人**：中位 0.812 的那一对只有 24% 的天数 ≥0.9、最低一天 0.031；
中位 1.000 的真嵌套是 110/110 天都 1.000、最低也是 1.000。
两者中位只差 0.19，**性质上一个是「相关」一个是「嵌套」**。
所以拿本脚本的输出判「这两个筛子重复」是不够的 —— 必须连着最差的那一天一起看。
