...
## 5 · 半路撞见的：归档有三天自己跟自己打架

§1 那条「100 天里有 1 天不成立」的线索拉出来的是一个数据问题，不是重叠问题：
**同一天同一只票被多个筛子记下时，读数必须相等**，而 2026-08-17 与 2026-08-14 不等。
把同一条恒等式推到**跨文件**（两个归档都记了同一个量），又掉出**第三天 2026-09-02** ——
`delayed_ep_log` 那天记的是前一场的收盘价（10/11 逐位等于 `leaders_log` 的 09-01 读数），
**而它在 `ticker_events` 内部完全自洽，文件内那一层一个字都报不出来**。
机制、证据与修法在事故档
