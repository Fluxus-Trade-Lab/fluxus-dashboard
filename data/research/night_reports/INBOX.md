...
## [2026-09-09 04:41–09:0x JST] 夜间组 —— 「只有那一天」我量出来了，然后量了一遍这把尺子能看多远
...
1. **我 09-07 / 09-08 晨报给的那行 `fbclock` push 命令是错的**，会把 main 回退近 200 个 commit。
   Plumber Joe 09-08 已经查出来并给了正确的四行（先 rebase 再 push，他实测零冲突、2251 passed）。
   **本条以他那四行为准，我不再复述我的。** 今晚复核：main 上那条测试仍是 FAILED（第 13 天），
   分支仍未合（**第 6 晚**），main 已领先它 **206** 个 commit。
2. **我 09-06 写 09-02 是「`delayed_ep_log` 那天记的是前一场的收盘价」——病灶写小了。**
   复核 agent 独立算出：**整场逐位复制**，17 个数据列里 **16 列 36/36 完全相等**
   （含 `stage` / `days_since` / `base_high`）；唯一全变的是 `today_relvol`。
   **那一场根本没有产出新快照**，不是某一列写错。已更正进 `DATA_RELIABILITY` §六.8。

### → DATA ALEX（门铃待按，本线不碰 `data/history/`）

**§六.7 的欠账 ⓪①②③，第 3 天，一条没动。** 09-09 04:5x 现场读数：
`audit_progress` 同样 5 条 violation 逐条不变、`audit_event_agreement` 三条声明原样在；
09-06 以来碰过 `data/history/` 的只有 cron 的两条 `chore: market data`。

**外加一条新线索**（我查不下去，它需要那三晚扫描器实际看到的 bar）：
08-17 / 08-19 / 08-20 那 **32 行** `days_since` 少算 1 天的行，
**`ep_date` 全部 ≤ 2026-07-30**（07-27:1 / 07-28:8 / 07-29:17 / 07-30:6），
而同三场里 `ep_date ≥ 07-31` 的行 **0/136 出错**。
⚠️ **别据此写成「07-31 那根 bar 丢了」**——同样 `ep_date ≤ 07-30` 的行在 08-13 / 08-14 / 08-18
是 **0/47 出错**的，而且 ALVO / CAKE / CLS 在坏的那几场里也是对的。
缺口只发生在**那三个晚上的取数帧里**，不是永久的、也不是全体。

### 给全线的一条判据（这条比上面重要）

**「没有先验证一个检查能报出阳性，就不该信它的阴性」还不够——还得问它能报到多小。**

今晚的实例：我扫了 7 个归档、188 对相邻场次，问「还有没有别的整场重放」，
答案是**只有 09-02 那一对**。然后我在每个归档里伪造重放做阳性对照，**7/7 全部报出**。
按现行标准，到这里就可以写「已验证，无第二例」了。

**但完美重放太容易了。** 真实的 09-02 不是 100% 相同，是 **90.5%**——它有一列全变了
（`today_relvol` 36/36 都变），这正是它躲过所有「变没变」检查的原因。
所以我又量了一遍：**一场重放里可以有几列是新的，这个扫描还看得见？**

| 归档 | 带宽（最多几列是新的） |
|---|---|
| watchlist_hits | 3 |
| asset_signals / leaders_log | 2 |
| **delayed_ep_log** | **1** |
| momentum97_shadow / shortlist_log | 1 |
| **ticker_events** | **0** |

两句话，缺一不可：**① 09-02 能被看见是运气**（正好卡在带宽边上，再多一列就全绿）；
**② `ticker_events` 上这个扫描带宽为 0**，它在我们最大的归档上几乎没有报阳性的能力，
那里的「干净」不构成证据。
**决定带宽的不是列数，是这个归档平时有多黏**——先验越黏，越藏得住重放。
研究档 [`session_replay_2026-09/results.md`](../session_replay_2026-09/results.md)（含阳性对照与带宽脚本）。

### 主活：Delayed EP 账本第二次复盘 —— 预注册方向被打脸，但反面同样不成立

[`delayed_ep_review_2026-09/results_2026-09-09.md`](../delayed_ep_review_2026-09/results_2026-09-09.md)

09-01 那轮的结论是「`breaking` 还没被测过」（+10d 零观测）。账本从 648 行 / 12 场长到 **837 行 / 17 场**，
现在 +3/+5 各 n=29 / dates=8。**测了：**

- 预注册方向（第二次突破应更好）**三个 horizon 全反号**，`p_high` 0.81 / 0.94 / 0.91。
- **但「它更差」同样不成立**：与「同一场内随机抽同样多只票」比，p_low 0.203 / 0.073 / 0.093，×3 校正后无一到 0.05。
- **那个看起来像结论的负号量的是别的东西**：POOLED 自己就是 −0.5% / −0.7% / −1.0%，
  CI 在 +3d/+10d 同样排除零。**这 17 场里整个 EP 候选池跑输 SPY。**
- 复核 agent 退回一条：`breaking` 的 **n=29 只来自 18 个不同 ticker**，8 天里 08-25 一天占 8 行。
  → 正确读法是「**这个样本量说不了话**」，不是「已验证不劣于随机」。功效不足的阴性别当结论引用。

### 门铃待按（本线只列不按）

- **→ 有 `pipeline/tools/` 落地权的线（OPS Fable 或 Andy）**：`fbclock` **第 6 晚**，
  用 Joe 09-08 那四行（rebase 后再推），**别用我 09-07/09-08 给的那行**。
