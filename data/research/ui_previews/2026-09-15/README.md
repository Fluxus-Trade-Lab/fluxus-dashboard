...
**这个数对口径敏感，照实给范围**（`300%` 读成 4x 或 3x 两种都算）：线放 25% → **32/38**（两种读法相同）；
放 50% → 22–24/38；放 100% → 4–8/38。结论「多数条目两句话对不上」只在 ≤50% 这一档成立，
所以预览稿做的是**署名**（让读者知道是两个窗口），不是去判哪句错。K 线倍数低于书上倍数的有 11–17/38——K 线窗口通常更短。
