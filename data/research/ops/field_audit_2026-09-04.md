...
## 附录（OPS Fable 事后补，2026-09-04）：汇总时漏掉的两格 + 一条被实测推翻的断言

上面第三节说「这轮只查了四格」。**实际查了七格**——`is_tradeable` 和 `bo_count` 两格的检索结果完整、
都找到了标准，只是没被并进正文（`ticker_heat` 那格确实没找到标准，属实）。原始返回在
`subagents/workflows/wf_9d267c22-f53/journal.jsonl`。补记如下，两格的结论都比正文那四格更硬。

### A.1 `bo_count_*` — 借错了那个扫描的成交量

| | Stockbee 4% Breakout（一手） | 我们 |
|---|---|---|
| 涨幅 | `100×(C−C1)/C1 ≥ 4` | 同 ✅ **唯一对上的一格** |
| 成交量地板 | **100,000 股** | **9,000,000 股 — 高 90 倍** |
| 放量条件 | **`V > V1`**（今日量 > 昨日量） | **没有** |
| universe | US Common Stocks（原文明写排除 ETF） | 全 universe，无类型过滤 |
| 聚合 | 单日**横截面广度**（今天全市场几只） | 逐票**纵向滚动计数**（近 N 日几天）— 标准里没有这个 |

9,000,000 这个数来自 Pradeep 的**另一个**扫描（EP 9 Million），在那里它是 `maxv65` 而不是当日量，
配的也不是单日 4%。**我们把两个扫描各取一半拼起来，得到一个两边都不成立的量。**

后果是方向性的：9M 地板对大盘股恒真，等于放量条件完全失效，只剩「今天涨了 4%」；
而一只 2M 量的小盘股永远不可能被记一次 breakout——**Pradeep 原帖明说这个扫描是给小中盘和低价股用的，
我们把它的适用面翻了个方向。** 现在的注释写着「Pradeep's literal rule」，那句话得删。

⚠️ 与今天另一件事扣上了：标准要求的 `V > V1` 需要 `prev_volume`，而 `prev_volume` 直到今天
（commit `79b34cf6`）才第一次进 universe.json。**修 bo_count 的输入，今天刚到位。**

建议：①「4% breakout day」照抄一手公式 `≥4% AND V≥100000 AND V>V1`，universe 限普通股；
② 21/63/126/252 的逐票滚动计数保留可以，但必须明写是自造聚合；
③ 真想要 9M 那个量就另建一格、照 EP 9 Million 的原公式建，别和 `bo_count` 混。

### A.2 `is_tradeable` — 建议保留，但要写下三条偏离

标准侧找到的是 S&P Dow Jones 的 **FALR（年美元成交额 / 流通调整市值）**，辅以 Amihud ILLIQ、
SEC Rule 22e-4、Trade-Ideas / Deepvue 的 Dollar Volume 厂商口径。

**不建议整体照抄 S&P**：FALR 是换手率，会把 BRK 这类低换手但极易交易的票判负；
而我们问的问题是「这个账户规模一天之内能不能建仓」，不是「够不够格进指数」。
绝对美元成交额地板本身有厂商先例。

要写下的三条：
1. **构造是绝对量而非比值**（$1B、$2M 两个常量随市场漂移失准）——与 08-31 Record High Percent 的教训同形。
2. **无证券类型过滤**（标准靠类型过滤解决的事，我们赌上游不返基金）——08-31 事故的同形复发。
3. **两个常量的相对松紧从未对齐**：市值比 S&P SmallCap 600 入选线严，流动性比同档指数门槛松约 1.7×。
   不一致是无意的，因为两个数是分别拍的。

从业者方向的留痕（宪法要求）：Minervini 与 Qullamaggie 的最低流动性口径**一手源不存在**——
Qullamaggie 本人被问到扫描参数时明确拒绝给数，Minervini 的数字全是二手博客。

### A.3 ❌ 被实测推翻：`avg_volume` 没有窗口混用

检索结论里有一条写着「`avg_volume` 一列里 63 天和 20 天两种均值共存，喂同一个 $2M 闸——
**这条要修，不是要记**」，依据是 `finviz_adapter.py:43` 把 `'Average Volume'` 映射进了 `avg_volume`。

**实测不成立。** 取 55 只本地有 2 年日线的票，用它们自己的 bar 复算 20 日与 63 日滚动均量，
与 `data/output/universe.json` 的 `avg_volume` 对照：
...
| 更接近 20 日 | **55** |
| 更接近 63 日 | 0 |
| 相对误差（20 日） | **0.000**（AA / AAOI / ACMR / ACOG / AEHR 逐点完全相等） |

原因：Finviz 免费版只吐 Overview 七列，**`Average Volume` 不在其中**，那条映射永远不触发。
`avg_volume` 100% 来自 `yfinance_adapter.py:789` 的 20 日滚动均量。

这是「**读到了源码，当成了行为**」——映射表里写着 ≠ 那条路径跑得到。联邦里同形第三次
（前两次：CI 日志里的命令回显被当成真输出；`git diff A...B` 回答了另一个问题）。
**留在这里当病例**：口径检索给出的「这是 bug」不能直接进工单，得先用数据落一次地。

真正成立的是它旁边那半句：**窗口从来没被声明过**。20 日是对的，但代码里没有一个字说它是 20 日，
而所有查到的标准口径无一例外都声明窗口（S&P 365 日历日、Trade-Ideas 10 日、Deepvue 20/50 日、
Amihud 年内 >200 天）。这条该做的是**补注释和契约行**，不是修代码。
