...
| **Studio Mia**（暂定名，待 Andy 定） | 内容制作 · digital product：课程整理、视频工作流、Substack 信成稿、数据艺术素材 | `Fluxus_Substack/`、`Fluxus_Brand/templates\|record\|copybook/`、课程仓库（`~/Documents/SwingMaster*`） | 成稿小改直推 main；大改 `feat/*` |
...
## Studio Mia ↔ Marketing Steve 的审稿流程（拆线的原因就是防抢笔）
...
- **Studio Mia**：课程整理和设计、课程自动视频生成工作流、Digital writing (memo/Substack/X 成稿)、2026H1交易数据艺术可视化
...
- 每个数据文件只有一条线有写入权：
  ...
  - `Fluxus_Brand/` 按二级目录分笔：`templates|record|copybook` → Studio Mia；`research|voice|ops|visual` → Marketing Steve
