<!--
留档副本 · 2026-08-19。DESIGN.v2.md 是 DESIGN.md 在被 v3 重写之前的**完整原样**
（75,821 字节 / 1,242 行，sha256 3663dca4…，与当时的 DESIGN.md 逐字节一致）。

为什么留：这份文件里有大量**一次性的实测记录**——明暗两套调色板的 ΔE 表、
灰度承重列的证据、八条拒绝各自的触发率、四页落地时才暴露的三条通则。
那些是测量不是设计意图，重跑一遍要花掉整轮的工夫；而按成品倒推重写会把
没体现在当前代码里的历史读数整段挤掉。Andy 2026-08-19 明确要求不能丢。

要改设计系统请改 DESIGN.md，不要改这里。
-->
