...
KNOWN_UNWIRED: dict[str, tuple[str, str, str]] = {
    ...
    # 第五条「作者当晚给自己的工具打的欠条」，理由与前四条相同，同样说在明处。
    # 它**没有任何形式在跑**（连测试里都只跑合成数据：真实历史那一跑要逐个 `git show`
    # 139 份快照，几分钟，放进 CI 会把每次 push 拖垮）。所以它现在是一把**只在有人手敲时
    # 才响的闸**，这句话必须说清楚，别被读成「测试绿＝它在看着」。
    # 接线的正确位置是每晚写完 `data/output/universe.json` 之后自查一次 —— 那是
    # DATA ALEX 的文件；接的时候建议改成**只比今天与近 20 场的滚动中位**（把那 20 场的
    # 三个数存成一行账，别每次重走 git 历史），否则它会越跑越慢。
    # W2 会在有人接上的那天逼我删掉这条。
    "audit_universe_population": (
        "DATA ALEX", "2026-09-18",
        "字母那一维有人看（audit_universe_shape），市值这一维没有：2026-06-26 那一晚两维"
        "一起动，字母闸抓到了截断，而「≥$10 亿那道筛选条件不再生效、1,613 只中位市值 1.43 亿"
        "的小票一次性进宇宙」没有任何东西抓到 —— 截断 08-10 修好了，人口至今没回去，"
        "十二周零记账。真实历史 --since 2026-03-01 跑 139 场，首个违规正是 2026-06-26",
    ),

    # 第六条「作者当晚给自己的工具打的欠条」，理由与前五条相同，同样说在明处。
    # 它与上一条同族同主人，接线位置也同一处（每晚写完 universe.json 后自查一次），
    # 但**它比上一条便宜得多**：一份 payload 只需要两个加总，不需要回看任何历史，
    # 所以接进去是真正的三行，不会像人口闸那样越跑越慢。
    # 它现在**只在有人手敲时才响**：测试里跑的全是合成数据（真实那一跑要逐个
    # `git show` 156 份快照），别把测试绿读成「它在看着」。
    # W2 会在有人接上的那天逼我删掉这条。
    "audit_universe_freshness": (
        "DATA ALEX", "2026-09-20",
        "数量维有人看、内容维有人看、市值维有人看，**时点维没有**：2026-08-10 05:31 那份"
        "盘前 payload 字段齐全、volume 缺失率 0.0，管线自己盖的章是 quality.status=ok，"
        "而同周一份真收盘（d2337e85）盖的是 degraded；它在 universe_quality.csv 里与相邻"
        "四场收盘的最大差只有 0.33 个百分点 —— 不是 null 率账本漏看了，是那一维上本来就"
        "没什么可看。全历史 156 份快照里 5 份盘前、8 份 avg_volume 断供，9 场（含 08-10）"
        "一份可用的货都没有",
    ),
    ...
    "audit_progress": (
        ...
        "复制品却答得出「不一样」：十列状态与价格 36/36 逐位相同，而 today_relvol 36/36 都"
        "变了（最大 +46.4%，HTHT 0.5643→0.8262）—— 一根已经收掉的 bar 被重抓时带着盘后修订"
        "回来。计数器 days_since 跟价格无关，必须等于交易日历上 (ep_date, as_of] 的交易日数；"
        "按这把尺子另有 08-17 / 08-19 / 08-20 三天共 32 行帧缺失，其中 10 行修正后落在"
        "delayed_ep_scan 的 --max-days 15 之外，本不该在归档里",
    ),
