...
`volume` 独立复述同一句话（77/77 配 08-06、2/77 配自己 —— 09-11 首报时的对称 ±2% 口径；
按现行单边带与计票规则，冻结坏行是 56/56 配 08-06、7/56 配自己）。整场偏了一天。
...
2. **change_pct 恒为 0 的筛子测不了**：`healthy_charts` `ema21_watch` `momentum_97` `vcp`
   ...
   * **带是单边的，而且是自造的**：Finviz 快照量 ÷ yfinance 当日量 ∈ [0.90, 1.01]。
     ...
     08 月起中位 1.000。对称 ±2% 会把 04-29 判成 0.567。比值 >1.01 的全史 7 行，其中 3 行在最新一根 bar
     （2026-09-14 实测；最新一根 bar 次日会被厂商上调约 0.1%）。偏低的**成因没证实**：历史 bar 量在三份库快照间
     中位比值 1.0000（不是厂商事后修订），8 月那一跳更像取数时刻变了。**所以带依赖运行时刻**——
     cron 若提前到盘中、或 K 线库刷新早于 Finviz 快照，下沿/上沿会被吃掉，届时重量。
   * **计票**：同一只票同一天、**同一个量**只算一票（五个筛子抄同一个数不许投五票）；
     **不同的量各算一票**，不按行序取舍——08-14 的 `gainers_4pct` 是已知的盘前坏快照（中位量 987 股），
     按「首行」去重会让读数随文件行序在 0.925 和 0.975 之间翻（反驳 verifier 实测）。量须是有限正数。
   * **分辨率**（全史 113 个可判日，2026-09-14 实测）：干净日最差 **0.929**（08-14，含上面那族坏快照的 6 票），
     次差 0.931（05-18），中位 1.000；非自身日的噪声底中位 0.17、最高 **0.304**（候选日若要求票在当日也有 bar，是 0.318）；
     冻结坏行 08-07 = **0.125**（帧 = 08-06，1.000）、08-17 = **0.217**（配不上任何单日，最高 0.047）。
     **判定线 `VOL_OK_RATE = 0.80`**：比最差干净日低 0.13，比噪声最高高 0.50。
     **部分重放的分辨率**（随机把一部分票的量换成前一日 bar 量，求跌破线所需的最小比例，113 天）：
     线 0.80 → 最小 14%、中位 23%；线 0.70 → 26% / 34%；线 0.85 → 10% / 17% 但离最差干净日只剩 0.08。
     **一场里被换掉的票少于约两成，这条线看不见。**
     **时间切分复核**：带只用 06-15 前 66 天选，06-15 起 52 天上干净日最差 0.925 / 噪声最高 0.304，站得住。
   * 看不见的：①只偏了成交量级别（不是换帧）的错；②部分重放 <~20%（上一条）；
     ③**拆股**——yfinance 拆股后整体改写历史量，该票此前每天都失配（CRWD 4:1：07-02 前比值恒 0.249，
     19 天各拖 1/n）。一只票拖不过线，但**审计读数会随 K 线库重抓回溯改变**；④一张 DECLARED 声明同时静音两条恒等式。
...
# 下沿宽、上沿紧：实测 Finviz 那一侧只偏低不偏高（全史 >1.01 的 7 行，3 行在最新一根 bar）；成因未证实，带依赖运行时刻。
VOL_LO = 0.90
VOL_HI = 1.01
# 干净日最差 0.929 / 噪声底最高 0.304 / 冻结坏行 0.125 与 0.217。
# 0.80 而不是空档中点：中点 0.62 要换掉三成多的票才报；0.80 约两成，且离最差干净日还有 0.13。
VOL_OK_RATE = 0.80
...
def _comparable_volume(rows: Sequence[Mapping[str, str]], day: str) -> List[Tuple[str, float]]:
    """day 这一场的 (票, 量) 读数。同票同量只算一次（五个筛子抄同一个数不许投五票）；
    同票不同量各算一次 —— 不按行序挑一个，否则读数随文件行序变，且会把一族坏快照藏起来。
    量须是有限正数：空、0（「本列不填」）、nan、inf 都跳过。"""
    seen = set()
    out: List[Tuple[str, float]] = []
    for r in rows:
        if r.get("date") != day:
...
def _vol_rate(pairs: Sequence[Tuple[str, float]], store, cal_index, cal, day: str,
    ...
    for ticker, v in pairs:
        ...
        try:
            ...
            continue
        if not (math.isfinite(bv) and bv > 0):
            continue
        n += 1
        if lo <= v / bv <= hi:
            hit += 1
