...
Type is IBM Plex Mono 14.25 px (13.14 px for `.small`), so a Latin glyph is ~0.6 em wide; CJK falls back to a
...
def font_size(cls: str) -> float:
    """Must match the print sizes in recap_local.css (.tell text 14.25px / .tell .small 13.14px)."""
    return 13.14 if "small" in (cls or "").split() else 14.25
