...
def main() -> None:
    ...
    data = yf.download(B.REQUIRED, period=args.period, group_by="ticker",
    ...
    for ticker in B.REQUIRED:
        ...
        (args.out / f"{ticker}.json").write_text(
            json.dumps({"ticker": ticker, "bars": bars}))
        written += 1

    log.info("stored %d/%d baskets in %s", written, len(B.REQUIRED), args.out)
    if failed:
