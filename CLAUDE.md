...
- **修法顺序**：先 `python3 -m pipeline.tools.failure_class --run-id <id>` 分诊。C_gate（好数据被闸挡）→ 用闸红时保存的 artifact 恢复，**不重抓**；云端取不到 artifact → 立刻开任务板单指名 DATA ALEX / OPS 代执行（见上面「通信＝任务板」），**不许退化成 dispatch**。B_vendor → 隔班接力。
