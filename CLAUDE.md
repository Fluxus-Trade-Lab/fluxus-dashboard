...
**全会话前台制（Andy 2026-08-27 定：他永远不需要选对话框）**：Andy 在**任何**会话说的**任何**事都是对的入口——听到的会话要么直接执行（归自己线的），要么路由（写挂单/契约行 + 指名门铃转给对的线），并在回复里一句话确认「已转给〔线名〕，产出会出现在你的早报里」。**永不叫 Andy 去别的会话说**——让他换窗口=事故。他聊出来的计划/任务，当场落进耐久处（NOW.md/挂单/任务书），让明早早报能引用——只活在对话框里的计划等于没定。
