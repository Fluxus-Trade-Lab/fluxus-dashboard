...
对照 posts.csv + verdicts.jsonl 出三清单：**keep**（重复奏效且仍符策略）/ **test**（有苗头，需再来一次受控尝试）/ **stop**（重复地弱、重复的格式、贵而无果）。每条提案**点名支持它的帖子**。Andy 批了才写回 BRAIN.md / Voice Bible / 角色契约——一次好结果是假设，不是规则。
