...
- ⛔ **具体物闸（2026-09-08 维修单修 2，逐卡硬闸）**：逐条变体查——至少占**一笔具体交易 / 一个能查的数字 / 一个能查的时间戳**之一？三样全无 → 退回分发站，判定里指名哪条。四张卡三条 rejected 之后加的这道闸；它拦的是「说了几句话不知道在说什么」。
- ⛔ **判决账闸（2026-09-08 维修单修 3，逐卡硬闸）**：读 `Fluxus_Brand/voice/verdicts.jsonl` **全账**，逐条变体对——撞没撞任何历史判例的形状（对仗收口 / 造词式自嘲 / 格言收口 / AI 味整体 / 已毙题目族）？撞了 → 退回并**指名撞的是哪一条判例**；判定里必须有一节「判决账对照」，写「逐条查过，零撞」也要落字。此前本账只进每周 keep/test/stop，不进逐卡——08-24 那把刀隔 15 天原样复出就是这么漏的。
...
⚠️ **过渡期缺 `writing` / `visual` 两节不算「缺字段」**——那两站没有执行者（无 routine），
「缺字段=退回上一站」对它们不适用。按毛坯审，过闸时在 APPROVAL_QUEUE 注明「未经 Mia 成稿 / Vera 配图」。
