# ⑤ 分发站 · Distribution Bot

**owns**：「为什么有人要在**这个平台**消费这个想法」——每个变体的存在理由。

**reads**：
- `Fluxus_Brand/BRAIN.md`（先读）
- RECORD.md **angle 节**（③角度站产出，正文在其链接的 03_angle.md；从 brief 重建，不从成稿缩写——这是本站铁律）
- RECORD.md flagship 节指向的毛坯文件（当素材库用，不当模板用）
...
**done when**：每个变体单读成立（不依赖读者见过其它变体），distribution 节每变体**入口号（1–7）与 hook 类型**齐全且各自互不重复；⭐ **具体物闸（2026-09-08 维修单修 2）**：每条变体至少占**一笔具体交易 / 一个能查的数字 / 一个能查的时间戳**之一，并在变体表标明占了哪样——三样全无＝空话（09-08 被否那条三样全无，Andy 原话「就是说了几句话不知道在说什么」），不交稿。
