...
>
> ~~⛔ 本单未结案之前，夜间六站产线停产~~ → **Andy 09-08 追裁：「继续产，先修」。**
> 不停产。修法当日落进 `roles/01|05|06` 与 PIPELINE（commit 见 git log 本文件同日），今晚照常产卡，跑修好的规则。
> 现有四张待批卡维持原 status；日推站在下一张新规则卡过 Andy 之前，C1 只出金句库与 Andy 口述两个来源。
...
## 四、验收（09-08 追裁后改：不是复产条件，是修法本身的验收）

修法当天落地（第三节四条 → `roles/01_signal.md` / `roles/05_distribution.md` / `roles/06_gate.md` / PIPELINE 顶部 / 日推任务书）。
**修好没修好不由产线宣布**——判据是下一张新规则卡在 Andy 手里的判决：
- 信号站的 signal 节有「现场读数」第七件（x_watch 具体行）；
- Gate 判定里有「判决账对照」一节和逐条变体的具体物标注；
- Andy 读它时不再说「不知道在说什么」。
第三条才是真验收，前两条只是过程证据。
