...
1. ✅ **底盘 = 现有的 `Copybook` List**(`2083551367399182754`,30 人,私密)。Andy 09-06 定:「用 copybook,成员名单你调整。」**不新建 List。**

   **其余五张 Andy 已授权删**(原话「删 List 我授权,都是很久不用的」):Core `2005911734494826499`(5)· Top `1639985272384356356`(7)· Wizards `2005914198073827644`(8)· 机构 `2010371377032003689`(4)· 预测(1)。
   ⚠️ **删之前必须先把这 25 个成员抓下来存进 `data/content/x_watch/README.md`** —— 删 List 不可逆,而这 25 个人里可能正有我们要找的「不同视角」(机构那张里已看到 @ConnorJBates_ / @TedHZhang,Wizards 8 人一个没看到)。**没存下名单就删 = 扔掉还没看过的候选。** 存完再删,删完在 README 记一行。
2. **调整 Copybook 成员**:先读出现有 30 人,和 roster「定稿 v3」的 30 人对账(交集 / 只在 List 里 / 只在 roster 里),**产出对账表给 Andy 看一眼再动手加删**。加 @TheOneLanceB / @LindaRaschke 时认精确 handle(两者都有仿冒号)。

   ⚠️ **09-06 抓不到成员:X 限速。** 我反复拉列表把这个 session 打限速了 —— List 成员弹窗永久转圈、following 只吐 16/533。**跑手首件事就是重抓**(隔几小时,慢速,一次一张)。加完记下 List URL(`https://x.com/i/lists/<id>`),写进 `data/content/x_watch/README.md`。
3. **建目录**:
   ```
