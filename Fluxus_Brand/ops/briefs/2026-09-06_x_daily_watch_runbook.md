...
1. **用现有 List,不新建**(Andy 09-06)。他账号里六张私密 List:Copybook `2083551367399182754`(30 人)· Core `2005911734494826499`(5)· Top `1639985272384356356`(7)· Wizards `2005914198073827644`(8)· 机构 `2010371377032003689`(4)· 预测(1)。**用哪张当底、其余是否删,等 Andy 指定**;删 List 不可逆,跑手不删,只列。
...
**权重:** 名单里有几个人的话要加权(Andy 定名单,`weight` 列写在 roster,默认 1,加权的 2)。榜单同时报**原始提及数**和**加权提及数**,两个数都要在。
...
30 人不是终点,**最后 20–25 人**。砍谁、加谁,按**发帖密度 / 有效性 / 质量**三个数说话,不凭印象 —— 首跑两天的数据正好是精简的依据。**新加的 5 人必须提供现有 30 人没有的信息和视角**(不是再来 5 个成长股图表号):候选视角 = 交易心理 · 跨市场/宏观 · 期权/波动率 · 机构 PM · 系统化/量化。来源是 Andy 自己的 following(533 人)—— 09-06 抓的时候 X 只吐了 16 个就停,**这一步归跑手,等限速过了再抓一次**。
