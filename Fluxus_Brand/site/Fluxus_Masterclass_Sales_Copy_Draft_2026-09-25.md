# Fluxus Swing Masterclass — 销售文案更正草稿（T-0919-67，Writer Mia）

> ⚠️ **草稿，待 Andy 批准。09-25 之前、Andy 点头之前不得外发或上线。**
> 只读参照：`~/Documents/SwingMasterclass/_marketing/sales_copy.md`（Q 线仓库，本次未改动那份文件，改动全部落在这份草稿里）。
> 出处：任务 T-0919-67，事实源见 `~/Documents/fluxus-ops/tasks/T-0919-67.md` 「已定事实」节（Andy 2026-09-19 22:10 JST 前后原话）。
> Andy 批准后，交 **UI Claire** 落地（渠道：Vercel dashboard 新 landing page，见同目录 `Fluxus_Masterclass_Landing_Page_Copy_2026-09-25.md`）；若最终仍走 Squarespace，本文内容同样适用，字段一一对应旧版 `sales_copy.md` 的三节。

## 改动清单（对照旧版，逐条给理由）

| 字段 | 旧版（$1,399 版本） | 新版 | 依据 |
|---|---|---|---|
| 价格 | $1,399 一次性 | **$1,499 一次性** | Andy 2026-09-19 原话「我定价1499」|
| 录播 | 「20+ recorded video lessons, subtitled」 | **「10+ 录播视频，分批放出」**，去掉「带字幕」的承诺 | Andy 原话「10+录播视频」；字幕未定，不写死 |
| 直播 | 「Occasional live streams」 | **「不定期直播」** | 用词对齐 Andy 原话「不定期直播」 |
| 新增交付物 | 无 | **交互课件**（L1–L5 + 首个附录已做完，其余分批放出）+ **中英文 PDF** | Andy 原话「交互课件」；task「已定事实」节 |
| 开放日期 | 「Enrollment opens August 7」 | **发布日 09-25**（Andy 只说「9/25」未注明时区，按 JST 排；若他改期顺延） | Andy 原话「发布时间推迟到9/25日」 |
| 渠道 | 未指定 / 站内 | **Whop**（不变） | Andy 原话「销售渠道还是用whop」 |
| 章节/页数细节 | 无 | **不写死**——PDF 仍在 Andy手上校对，内容可能还有改动 | task「已定事实」末行 |

---

## 1) Enrollment / Sales page — 主文案

Bundle: **one-time $1,499 USD · lifetime access · not sold individually.**
*一次性 $1,499 美元 · 永久访问 · 不单独售卖单节。*

### Headline
**The setups are a commodity. The edge is subjective.**
*技术是大路货 —— edge 是主观的。*
（沿用旧版定位，未改——这是课程一直以来的锚点，不属于本次更正范围）

### Sub-headline
A complete swing trading system — plus the four disciplines almost no one teaches, because you can only teach them once you've lived them.
*一套完整的 swing 交易系统 —— 外加四门几乎没人教的功夫，因为只有交易过的人才讲得出来。*

### Body
Most trading courses sell you setups — breakouts, VCPs, episodic pivots. That part is everywhere; it gets you to the starting line, not the finish. This masterclass teaches the setups properly, but its heart is the four subjective disciplines that actually separate a Year-2 trader from a Year-5 trader: **position sizing, risk management, progressive exposure, and situational awareness.**

If you've traded 1–3 years, taken real losses, and you're done chasing signals — this is the bridge from Year 2 toward Year 5.

> *大多数交易课在卖战法——突破、VCP、EP。那部分到处都有，只让你站到起跑线。这门课把战法讲透，但核心是真正区分"第二年"和"第五年"交易者的四门主观功夫：**仓位、风控、渐进敞口、情境感知。**如果你交易了 1–3 年、亏过真钱、不想再追信号——这就是从"第二年"通往"第五年"的桥。*

### What you get / 你会得到

- **The full written system + operations manual** — built into the lessons, always the latest version *（整套系统 + 操作手册，内建在课程里，永远最新）*
- **Bilingual PDF (EN / 中文)** — the written reference, done *（中英文 PDF，已成）*
- **Interactive courseware** — Lessons 1–5 plus the first appendix are ready now; the rest unlocks as it's finished *（交互课件——L1–L5 + 首个附录已做好，其余随进度分批放出）*
- **10+ recorded video lessons**, released in batches as they're finished *（10+ 录播视频，分批放出）*
- **Full written notes inside every lesson** *（每课完整讲义，都在课程页内）*
- **One printable one-page checklist** — the 60-second pre-trade self-check, yours to keep and pin above your desk *（一页可打印清单：60 秒交易前自检）*
- **Optional weekly homework** *（每周作业，选做）*
- **Self-paced** — watch on your own schedule *（自定进度）*
- **Live streams, held as needed** — not on a fixed calendar *（不定期直播手把手）*

### What it won't do / 它不会
No signals to copy. No holy grail. No get-rich promise. Year 5 is called Year 5 for a reason — this gives you the map so those years build an edge instead of wandering.
*不发信号照抄，没有圣杯，不承诺暴富。Year 5 之所以叫 Year 5 是有原因的——这门课给你地图，让那几年花在打造 edge 上。*

### Primary button / 主按钮
**Enroll — full bundle · $1,499**  ·  下方小字：*One-time payment · lifetime access / 一次付款 · 永久访问*

---

## 2) Pricing section / 定价区块

**Fluxus Swing Trading Masterclass — Full Bundle**
**$1,499** *(one-time / 一次性)*

Everything, one price. The complete course is sold as a single bundle — lessons are not available individually.
*全部内容，一个价格。整套课程按 bundle 出售——不单独售卖单节。*

Included / 包含：
- Bilingual written system + operations manual, inside the lessons
- Interactive courseware (L1–5 + first appendix live now; rest unlocks as it's finished)
- 10+ video lessons with full written notes on-page, released in batches
- One printable pre-trade checklist (the only download — pin it above your desk)
- Optional weekly homework
- Lifetime access + all future updates to the course *（永久访问 + 课程后续所有更新）*
- Live streams as needed

> Enrollment opens **September 25, 2026**. *报名 2026 年 9 月 25 日开放。*

*(Refund policy — 待 Andy 定；旧版留白，本次未新增)*

---

## 3) Purchase confirmation email / 购买确认邮件

**Subject / 主题:** Welcome to the Fluxus Swing Masterclass — you're in.

**Body / 正文:**

Hey {First Name},

You're in. Welcome to the Fluxus Swing Trading Masterclass.

Here's how to start:
1. **Sign in** to your member account and open the Masterclass area.
2. **Open the member area** — start with the bilingual PDF and the interactive courseware for Lessons 1–5. More unlocks as it's finished; you won't need to ask, it'll just show up.
3. **Start with Module 1, Lesson 1.** Do the drills. Don't rush ahead — everything builds on the basics.
4. Watch for **live-stream announcements** by email — those happen as needed, not on a fixed schedule.

Two things before you begin:
- This isn't a signals service. There's no shortcut. The edge you're paying for is in the four disciplines almost no one teaches — position sizing, risk management, progressive exposure, situational awareness. Read those the deepest.
- Go slow. Twelve weeks is fast; six months is normal. The people who rush blow up.

Any access issues, just reply to this email.

See you inside,
Tom · Fluxus Trade Lab

---
*中文版正文：*

嘿 {名字}，

你已加入 Fluxus Swing 交易大师课。

开始方式：
1. 登录会员账号，打开 Masterclass 区域。
2. **进入会员区**——先看中英文 PDF 和 L1–L5 的交互课件。后面的内容会随进度陆续解锁，不用你去问。
3. **从模块一 Lesson 1 开始**，把 drill 做了。别急着往后跳——一切都建在基础上。
4. 留意邮件里的**直播通知**——直播不定期，看需要开。

开始前两句话：
- 这不是信号服务，没有捷径。你付费买的 edge，在那四门几乎没人教的功夫上——仓位、风控、渐进敞口、情境感知。这四门读最深。
- 慢慢来。12 周算快，6 个月是常态。急的人会爆仓。

访问有问题，直接回复本邮件。

课里见，
Tom · Fluxus Trade Lab
