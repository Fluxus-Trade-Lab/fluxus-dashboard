...
>
> 有一题问你能扛多大回撤。我今年跑赢了指数，但中间最深的一段是 17.9%。那个坑不是每个人都该踩，我需要知道你站在哪。
...
>
> One of them asks how much drawdown you can sit through. I'm well ahead of the index this year, but the deepest stretch along the way ran 17.9%. Not everyone should take that ride. I need to know where you stand.
...
**开场白原稿第三句的业绩数字换成了「跑赢指数」。** Andy 09-26 原话：「**YTD不用给准确的 就说我outperform 就行**」。原稿写「我自己 YTD 是 130%，SPY 是 11%」——这两个数在仓库里找不到权威源：`PERFORMANCE_TRUTH.md` 最新一期（Period 2）只到 2026-08-30，口径是已实现 **+114.36%**；SPY **+9.60%** 是 H1 到 07-22 那个窗口的数。发给会员的数字不能是转抄来的。
...
✅ **不必跑 `truth_snapshot.py` 了**：Andy 09-26 裁定业绩只说「跑赢指数」、不给准确数，所以不需要把权威源更新到 09-26。保留的 **17.9%** 是回撤不是收益，有权威源且是那道题本身要问的东西。
