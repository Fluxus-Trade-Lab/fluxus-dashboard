...
**减法**：Industries / Provisional 两个页签整页下线（数据保留在 groups.json；
Industries 层由 Dashboard 的行业/板块卡承担，Provisional 的诚实归宿是页眉那个计数）。
排名默认折叠：头 8 + 尾 4 + 每个选中主题 ±1 邻域，隐藏段印计数可展开。

**两个「好看」的哲学裁定**（Andy 问 TSF 的曲线和渐变是否容于我们的理念——答案是各守一法则）：

- **曲线**：直线段和曲线段同样是「两个测量点之间的假设」。被禁的从来不是弯，
  是**过冲**——自由样条会画出从未测得的极值。改用 Fritsch–Carlson 单调三次插值：
  曲线不越过相邻样本的范围，测量点仍画在曲线上。和折线同等诚实，好看得多。
