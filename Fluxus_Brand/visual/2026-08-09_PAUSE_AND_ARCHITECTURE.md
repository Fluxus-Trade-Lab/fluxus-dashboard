...
## 五之五、宇宙扩容留下的断点(2026-08-09 记 · 已验证)

`2f782b5` 把 Finviz 抓取从 150 页提到 600 页,宇宙从 ~3000 → **5615**(M–Z 回来了,
含 NVDA/MSFT)。修复是对的,但它在两处留下断点,都验证过了:

### 事实(2026-08-09 在 558 行归档上重放)

1. **归档里恰好 `universe_size=3000` 的有 21 行,2026-06-26 → 08-06** —— 那不是宇宙
   正好三千,是 150 页上限咬住、字母表切在中间的行。**这六周的 up_4pct / NH / NL
   等全宇宙计数是系统性低估**(缺了 M–Z 的一半)。断点不只在扩容那天 ——
   扩容前六周的数据本身就是残的。
2. **`thrust` 阈值 300 家的含义变了。** 300 家在历史宇宙里 = 中位 **11.3%**
   (区间 10.0–12.2%,很紧);在 5615 里 = **5.3%**。**阈值悄悄放松了一半** ——
   以后一半强度的推力日就会点亮 thrust 票。
3. **改成按比例的代价极小:** 用历史中位占比(11.3% × 当行 universe_size)重放全部
   558 行,只有 **19 行(3.4%)** 翻票。历史上便宜,向前是必要。

### 设计(待 Andy 过目后实施 —— 它改判决引擎,不是改画法)

- `THRESHOLDS['thrust']['count']` 从绝对数 **300** 改为 **占当行 universe_size 的比例**。
  每行归档本来就带 `universe_size`,不用补数据。
- 校准值是个决定:守历史含义用 **10.7%–11.3%**(5615 上 ≈ 600–635 家);
  或者回 Stockbee 原始定义重校。**这个数由 Andy 定,不由我定。**
- 归档 2026-06-26 → 08-06 那 21 行加数据质量标记(截断样本),
  Time Machine 重放跨过这段时要提示。
- 其余绝对家数阈值(`up_25pct_qtr` 之类的对比票都是同宇宙内的差值,自动免疫;
  **只有对固定家数的比较受影响**,查下来就 thrust 一个)。

### 为什么现在不直接改

改的是**判决引擎**(3.4% 的历史行会翻票,以后每一天都按新义投)。
按「calc engine, not advice」的规矩:我出数,他拍板。
