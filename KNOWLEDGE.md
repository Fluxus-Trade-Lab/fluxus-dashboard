...
> Andy 2026-08-27 立项：「目前还没有完整 absolute truth 的资料库」——对。真理**不集中迁移**（搬家=砸断全仓引用），但从今天起有这张地图。每个会话开工遇到「我该信哪份文件」时，答案从这里出。无人值守读权威版一律 `git show origin/main:<path>`（例外仅剩 receipts.md 与 voice/raw/——Andy 09-11 批「同意收窄」，见 CLAUDE.md 主树保护第 3 条）。
...
| 关卡进度 | 日推第一行 / NOW.md 🎮 节 | |
| **波段大师课 ver1 两份 PDF 的页数/字节/md5（⚠️ 2026-09-26 起内部留档、不对外发行）** | `data/content/RELEASE_ver1.md`「六、2026-09-26 修订」· 契约行 `data/reference/DATA_CONTRACTS.md` §七 [2026-09-26] 修订行 | 数字仍是 **b5** 定版：中 **194 页** / 13,568,448 B / md5 `c7302ad14f7a84d00dcd7d9f80e17b6e`；英 **210 页** / 12,390,358 B / md5 `6c56c6f22c19e738601132edafd03f40` ——**页数/字节/md5 本身没变**，变的只是身份：**不再是买家下载路径，退居内部留档**（书线 commit `d6f4e33`，Andy「让pdf退居二线，不提供了。做1」「但我们内部保留pdf」）。⚠️ **「中文全本 390 页」与「中 199 页 / 英 213 页」都是已作废的旧数**（同一个数作废过两次），见到就按本行更正。PDF 本体只在本机 `~/Documents/SwingMasterclass/_pdf/release_2026-09-25/`（该仓无 git remote，`git show` 不适用）——要重新量：`md5 -q <文件>` 与 `mdls -name kMDItemNumberOfPages -raw <文件>` |
| **波段大师课交互课件全集范围/页数（现行对外交付形态，2026-09-26 起）** | `data/content/RELEASE_ver1.md`「六、2026-09-26 修订」· 契约行 `data/reference/DATA_CONTRACTS.md` §七 [2026-09-26] 修订行 | **52 页 = 中英各 26 页**：前言 + 目录 + 模块一五课+附录 MU + **CH00–CH11（十二章）** + 尾声 + 附录 A/B/C/D/E/F/G/H/L（九篇）。书线 commit `d4c6e58`（T-0925-10）。**附录 I/J/K 不在内，走 2026-09-27**——这条 09-25 定的，没变。线上验证：`https://fluxus-masterclass-lab.pages.dev/` footer `由课文源文件生成 · e8f0676 · 2026-09-26`（2026-09-26 现场 curl）。⚠️ CF Pages 部署 hash 与门禁文件数「80/92」未重测，见契约行 [2026-09-26] 修订行末尾两条缺口 |
